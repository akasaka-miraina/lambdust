%\vfill\eject
\section{Lambdust Formal Denotational Semantics}
\label{lambdustformalsemanticssection}

\bgroup

% R7RS notation (inherited)
...
\copy1{} (handle \E{} (\I{} \arbno{(\I{} \arbno{\I} \E)})) & effect handler \\
...
\copy1{} (receive (\arbno{(\E{} \E)})) & pattern-based message reception \\
\copy1{} (future \E) & asynchronous computation \\
\copy1{} (await \E) & synchronization \\[0.5em]

\multicolumn{4}{l}{\textbf{FFI:}} \\
\copy1{} (foreign-call \I{} \T{} \arbno{\E}) & foreign function call \\
...
$\nu$      & \elem & \NAT & &          & natural numbers \\
           ...
           &       &      & &          & miscellaneous \\
$\phi$     & \elem & \FUN &=& $\LOC\times(\arbno{\EXP} \to \DP \to \EC \to \CC)$ & procedure values \\
...
$\chi$     & \elem & \text{Mailbox} &=& $\arbno{\text{Message}}$ & actor mailboxes \\
...
$\Csem:$ & $\arbno{\Com}\to\ENV\to\DP\to\CC\to\CC$ \\[0.5em]
...
\begin{semfun}
\Esem\sembrack{(\textbf{handle} \E\:(\I\:\arbno{(\I_i\:\arbno{\I}_i\:\E_i)}))} =$\\
\go{1}$\lambda\rho\omega\kappa\:.\:
  \Esem\sembrack{\E}\rho$\\
\go{2}$(\fun{extend-handler}\:\omega\:(\fun{make-handler}\:\arbno{(\I_i\:\arbno{\I}_i\:\E_i)}\rho))$\\
\go{2}$\kappa
\end{semfun}
...
\begin{semfun}
\Esem\sembrack{(\textbf{foreign-call} \I\:\T\:\arbno{\E})} =$\\
\go{1}$\lambda\rho\omega\kappa\:.\:
  \arbno{\Esem}\sembrack{\arbno{\E}}\rho\omega\:($\\
\go{2}$\lambda\arbno{\epsilon}\:.\:
    \fun{check-capability}\:\I\:($\\
\go{3}$\lambda\text{cap}\:.\:
      \fun{marshal-call}\:\I\:\T\:\arbno{\epsilon}\:\text{cap}\kappa))
\end{semfun}

\begin{semfun}
\fun{marshal-call} : \SYM \to \TYP \to \arbno{\EXP} \to \text{Capability} \to \EC \to \CC$\\$
\fun{marshal-call} =$\\
\go{1}$\lambda\text{fname}\:\text{ret-type}\:\arbno{\epsilon}\:\text{cap}\kappa\:.\:
  \fun{convert-args}\:\arbno{\epsilon}\:($\\
...
$$\vdash P : \tau \implies \forall \sigma\:.\: \Esem\sembrack{P}\:\emptyset\:\textit{root}\:(\lambda\epsilon\:.\:\epsilon) \neq \wrong{\cdot}$$

\textbf{Effect Safety:} Effects are properly contained by handlers:
$$\vdash P : \tau \text{ in } \Psi \implies \text{effects}(\Esem\sembrack{P}) \subseteq \Psi$$

\textbf{Memory Safety:} FFI operations cannot access unauthorized memory:
$$\text{safe}(\text{capability}) \implies \text{safe}(\Isem\sembrack{\text{ffi-call}}\:\text{capability})$$

\textbf{Proof Soundness:} Extracted programs satisfy their specifications:
$$\vdash P : \text{proof-of}(\phi) \implies \Esem\sembrack{\text{extract}(P)} \models \phi$$
