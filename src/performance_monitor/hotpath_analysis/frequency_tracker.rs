...
impl FrequencyTracker {
    /// Create a new frequency tracker with default configuration
    #[must_use]
    pub fn new() -> Self {
        Self {
    ...
    /// Record execution of an expression with performance metrics
    /// 
    /// Tracks execution time, memory usage, and return value type
    /// for performance analysis and hotpath detection.
    ...
    #[must_use] pub fn get_execution_record(&self, expr_hash: &str) -> Option<&ExecutionRecord> {
        self.execution_counts.get(expr_hash)
    }
    
    /// Get all tracked expressions
    #[must_use] pub fn get_tracked_expressions(&self) -> Vec<String> {
        self.execution_counts.keys().cloned().collect()
    }
    
    /// Calculate overall execution statistics
    #[must_use] pub fn calculate_statistics(&self) -> FrequencyStatistics {
...
impl TemporalFrequencyAnalysis {
    /// Create a new temporal frequency analysis with default time windows
    #[must_use] 
    pub fn new() -> Self { 
        Self { 
            time_windows: vec![
    ...
    pub fn record_execution(&mut self, expr_hash: &str, _execution_time: Duration) {
        for &window in &self.time_windows {
            let window_map = self.windowed_counts.entry(window).or_default();
            let expr_counts = window_map.entry(expr_hash.to_string()).or_default();
            
            // Add current execution
            expr_counts.push_back(1);
            
            // Keep only recent executions within the time window
            let cutoff_time = std::time::Instant::now().checked_sub(window).unwrap();
    ...
    #[must_use] pub fn get_frequency_in_window(&self, expr_hash: &str, window: Duration) -> u64 {
...
impl ContextualFrequencyTracker {
    /// Create a new contextual frequency tracker with empty contexts
    #[must_use] 
    pub fn new() -> Self { 
        Self { 
    ...
    pub fn record_call_context(&mut self, expr_hash: &str, call_stack: &[String], execution_time: Duration) {
        if !call_stack.is_empty() {
            ...
            context.context_avg_time = Duration::from_nanos(
                ((context.context_avg_time.as_nanos() * u128::from(context.context_frequency - 1)
                 + execution_time.as_nanos()) / u128::from(context.context_frequency)) as u64
            );
        }
    }
    ...
    pub fn record_binding_context(&mut self, expr_hash: &str, bindings: &HashMap<String, String>) {
        if !bindings.is_empty() {
            let context_key = format!("{expr_hash}@bindings");
    ...
    #[must_use] pub fn get_call_context(&self, expr_hash: &str) -> Option<&CallStackContext> {
