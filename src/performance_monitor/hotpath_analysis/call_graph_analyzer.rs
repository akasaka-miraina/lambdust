...
impl CallGraphAnalyzer {
    /// Create a new call graph analyzer with empty graphs
    #[must_use] 
    pub fn new() -> Self { 
        Self { 
    ...
    /// 
    /// Adds an edge from caller to target and updates call frequency weights.
    pub fn record_call(&mut self, caller: &str, target: &str) -> Result<()> {
        self.call_graph.entry(caller.to_string()).or_default().insert(target.to_string());
        self.reverse_call_graph.entry(target.to_string()).or_default().insert(caller.to_string());
        *self.call_weights.entry((caller.to_string(), target.to_string())).or_insert(0) += 1;
        Ok(())
    }
    
    /// Get call graph information for a specific expression
    /// 
    /// Returns information about callers, callees, and call frequencies
    /// for the given expression hash.
    #[must_use] pub fn get_call_info(&self, expr_hash: &str) -> Option<CallGraphInfo> {
    ...
    /// Calculate overall complexity metrics for the call graph
    /// 
    /// Returns metrics including node count, edge count, and depth analysis.
    #[must_use] pub fn calculate_complexity(&self) -> CallGraphComplexity {
        CallGraphComplexity {
            ...
            avg_out_degree: if self.call_graph.is_empty() { 
                ...
                self.call_graph.values().map(std::collections::HashSet::len).sum::<usize>() as f64 / self.call_graph.len() as f64 
            },
        }
    }
    ...
    #[must_use] pub fn get_call_hotspots(&self, threshold: u64) -> Vec<CallHotspot> {
    ...
    #[must_use] pub fn find_strongly_connected_components(&self) -> Vec<Vec<String>> {
    ...
    #[must_use] pub fn find_call_path(&self, source: &str, target: &str) -> Option<Vec<String>> {
        let mut visited = HashSet::new();
