...
use super::{Module, ModuleError, ModuleId, ModuleMetadata, ModuleNamespace, ModuleSource};
use crate::ast::Expr;
...
impl SchemeLibraryLoader {
    ...
    pub fn compile_library(
        ...
        let tokens = lexer.tokenize().map_err(|e| {
            Error::parse_error(
                format!("Lexing error in {}: {}", id.components.join("/"), e),
                Span::new(0, 0),
            )
        })?;
        ...
        let program = parser.parse().map_err(|e| {
            Error::parse_error(
                format!("Parsing error in {}: {}", id.components.join("/"), e),
                Span::new(0, 0),
            )
        })?;
    ...
    fn compile_module_expressions(
        ...
        for expr in exprs {
            match &expr {
                ...
                Expr::DefineLibrary {
                    ...
                    if actual_name != id.components {
                        return Err(Box::new(Error::parse_error(
                            format!(
                                "Library name {:?} does not match expected {:?}",
                                actual_name, id.components
                            ),
                            Span::new(0, 0),
                        )));
                    }
                ...
                Expr::Application { operator, operands }
                    if self.is_export_declaration(operator) =>
                {
                ...
                Expr::Application { operator, operands }
                    if self.is_import_declaration(operator) =>
                {
        ...
        Ok(Module {
            ...
            exports,
            dependencies,
            source: Some(ModuleSource::File(PathBuf::from(format!(
                "{}.scm",
                id.components.join("-")
            )))),
            metadata,
    ...
    fn process_import_expression(
        &self,
        import_expr: &crate::diagnostics::Spanned<Expr>,
    ) -> Result<Vec<ModuleId>> {
                    ...
                    match &component.inner {
                        ...
                        Expr::Literal(literal) if literal.is_number() => {
                            if let Some(n) = literal.to_f64() {
                                module_parts.push(
                                    n.to_string().split('.').next().unwrap_or("0").to_string(),
                                );
                            } else {
                            ...
                            "rename" => {
                                ...
                                if components.len() == 3 {
                                        ...
                                        (&components[1].inner, &components[2].inner)
                                    {
                                        ...
                                        let value = if let Some(primitive_value) =
                                            context.available_primitives.get(local)
                                        {
                                            primitive_value.clone()
                                        } else if let Some(env_value) =
                                            context.environment.lookup(local)
                                        {
                                            env_value
                                        } else {
                                            Value::Unspecified
