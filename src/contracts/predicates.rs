...
impl PredicateRegistry {
    ...
    fn register_builtin_predicates(&mut self) {
        ...
        self.register_builtin("rational?", rational_predicate(), "tests for rational numbers");
        ...
        self.register_builtin("primitive?", primitive_predicate(), "tests for primitive procedures");
        ...
        self.register_builtin("positive?", positive_predicate(), "tests for positive numbers");
        self.register_builtin("negative?", negative_predicate(), "tests for negative numbers");
        ...
        // String predicates
        self.register_builtin("non-empty-string?", non_empty_string_predicate(), "tests for non-empty strings");
        
        // Collection size predicates
        self.register_builtin("empty?", empty_predicate(), "tests for empty collections");
        self.register_builtin("non-empty?", non_empty_predicate(), "tests for non-empty collections");
    }
...
pub fn number_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::InexactReal(_)) | Value::Literal(Literal::Number(_)) | 
...
pub fn integer_predicate() -> PredicateFn {
    Arc::new(|value| {
        match value {
            Value::Literal(Literal::InexactReal(n)) | Value::Literal(Literal::Number(n)) => n.fract() == 0.0,
...
pub fn exact_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::ExactInteger(_)) | Value::Literal(Literal::Integer(_)) |
                       Value::Literal(Literal::Rational(_)))
    })
}
...
pub fn complex_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::Complex(_)) |
...
pub fn string_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::String(_)) | 
...
pub fn char_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::Character(_)))
    })
}
...
pub fn boolean_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Literal(Literal::Boolean(_)))
    })
}
...
pub fn hash_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Hashtable(_) | Value::AdvancedHashTable(_))
    })
}
...
pub fn primitive_predicate() -> PredicateFn {
    Arc::new(|value| {
        matches!(value, Value::Primitive(_))
    })
}
...
pub fn even_predicate() -> PredicateFn {
    Arc::new(|value| {
        match value {
            Value::Literal(Literal::ExactInteger(n)) | Value::Literal(Literal::Integer(n)) => *n % 2 == 0,
...
pub fn non_empty_predicate() -> PredicateFn {
    Arc::new(|value| {
        ...
        (list_predicate()(value) || string_predicate()(value) || 
         vector_predicate()(value) || hash_predicate()(value))
    })
}
...
mod tests {
    ...
    fn test_string_predicates() {
        ...
        let string_val = Value::Literal(Literal::String("hello".to_string()));
        let empty_string_val = Value::Literal(Literal::String("".to_string()));
    ...
    fn test_collection_predicates() {
        ...
        let non_empty_string = Value::Literal(Literal::String("hello".to_string()));
