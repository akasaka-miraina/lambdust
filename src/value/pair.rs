...
impl PairData {
    ...
    #[must_use] pub fn new(car: Value, cdr: Value) -> Self {
        PairData { car, cdr }
    }
}
...
impl Value {
    ...
    #[allow(clippy::similar_names)]
    #[must_use] pub fn cons(car: Value, cdr: Value) -> Value {
    ...
    #[must_use] pub fn as_pair(&self) -> Option<(Value, Value)> {
        match self {
            Value::Pair(pair_ref) => {
    ...
    #[must_use] pub fn car(&self) -> Option<Value> {
        match self {
            Value::Pair(pair_ref) => {
                let pair = pair_ref.borrow();
                Some(pair.car.clone())
    ...
    /// Returns a cloned value to avoid borrow checker issues
    #[must_use] pub fn cdr(&self) -> Option<Value> {
        match self {
            Value::Pair(pair_ref) => {
                let pair = pair_ref.borrow();
                Some(pair.cdr.clone())
