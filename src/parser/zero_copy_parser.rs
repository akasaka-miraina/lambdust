#![allow(missing_docs)]
//! Zero-Copy Parser for Phase 8 Performance Optimization
//!
...
use crate::eval::string_interning_system::{InternedString, intern_string};
...
impl<'source> ZeroCopyParser<'source> {
    ...
    fn convert_token(source: &'source str, token: Token) -> BorrowedToken<'source> {
        ...
        let kind = match token.kind {
            ...
            | TokenKind::ComplexNumber => BorrowedTokenKind::Number(source_slice),
            TokenKind::Boolean => BorrowedTokenKind::Boolean(source_slice),

            // Comments
            TokenKind::LineComment | TokenKind::BlockComment => {
                BorrowedTokenKind::Comment(source_slice)
            }
    ...
    pub fn materialize(self) -> Result<Expr> {
        match self {
            ...
            ExprView::Quote(expr) => {
                ...
                Ok(Expr::Quote(Box::new(Spanned::new(
                    expr.materialize()?,
                    Span::new(0, 0),
                ))))
            }
            ...
            ExprView::Quasiquote(expr) => {
                use crate::diagnostics::{Span, Spanned};
                Ok(Expr::Quasiquote(Box::new(Spanned::new(
                    expr.materialize()?,
                    Span::new(0, 0),
                ))))
            }
            ...
            ExprView::Unquote(expr) => {
                use crate::diagnostics::{Span, Spanned};
                Ok(Expr::Unquote(Box::new(Spanned::new(
                    expr.materialize()?,
                    Span::new(0, 0),
                ))))
            }
            ...
            ExprView::UnquoteSplicing(expr) => {
                use crate::diagnostics::{Span, Spanned};
                Ok(Expr::UnquoteSplicing(Box::new(Spanned::new(
                    expr.materialize()?,
                    Span::new(0, 0),
                ))))
            }
            ...
            ExprView::List(elements) => {
                let materialized: Result<Vec<Expr>> =
                    elements.into_iter().map(|e| e.materialize()).collect();
                ...
                if exprs.is_empty() {
                    ...
                    let operands = exprs[1..]
                        .iter()
                        .map(|e| Spanned::new(e.clone(), Span::new(0, 0)))
                        .collect();
            ...
            ExprView::DottedList(elements, tail) => {
                let materialized_elements: Result<Vec<Expr>> =
                    elements.into_iter().map(|e| e.materialize()).collect();
                ...
                if elements.is_empty() {
                    ...
                    Ok(Expr::Pair {
                        car: Box::new(Spanned::new(elements[0].clone(), Span::new(0, 0))),
                        cdr: Box::new(Spanned::new(materialized_tail, Span::new(0, 0))),
                    })
                }
            }
        }
...
impl<'source> ProgramView<'source> {
    ...
    pub fn materialize(self) -> Result<Program> {
        let expressions: Result<Vec<Expr>> = self
            .expressions
            .into_iter()
            .map(|e| e.materialize())
            .collect();
        ...
        Ok(Program {
            expressions: exprs
                .into_iter()
                .map(|e| crate::diagnostics::Spanned::new(e, crate::diagnostics::Span::new(0, 0)))
                .collect(),
        })
    }
}
