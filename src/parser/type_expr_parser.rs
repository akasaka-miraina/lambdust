...
use crate::ast::{TypeExpr, TypeConstraint, VariantCase};
...
impl Parser {
    ...
    fn parse_forall_type(&mut self, start_span: Span) -> Result<Spanned<TypeExpr>> {
        ...
        self.consume(&TokenKind::LeftParen, "Expected opening parenthesis for type variables")?;
        ...
        self.consume(&TokenKind::RightParen, "Expected closing parenthesis for type variables")?;
        
        // Parse body type
        let body = self.parse_type_expression()?;
        
        self.consume(&TokenKind::RightParen, "Expected closing parenthesis for forall type")?;
        let end_span = self.current_span();
        let span = start_span.combine(end_span);
    ...
    fn parse_exists_type(&mut self, start_span: Span) -> Result<Spanned<TypeExpr>> {
        ...
        self.consume(&TokenKind::RightParen, "Expected closing parenthesis for exists type")?;
        let end_span = self.current_span();
        let span = start_span.combine(end_span);
    ...
    fn parse_recursive_type(&mut self, start_span: Span) -> Result<Spanned<TypeExpr>> {
        ...
        self.consume(&TokenKind::RightParen, "Expected closing parenthesis for recursive type")?;
        let end_span = self.current_span();
        let span = start_span.combine(end_span);
    ...
    fn parse_variant_type(&mut self, start_span: Span) -> Result<Spanned<TypeExpr>> {
        ...
        while self.check(&TokenKind::Pipe) && !self.is_at_end() {
            ...
            cases.push(VariantCase { constructor, payload });
        }
        ...
        self.consume(&TokenKind::RightParen, "Expected closing parenthesis for variant type")?;
        let end_span = self.current_span();
        let span = start_span.combine(end_span);
    ...
    fn parse_record_type(&mut self, start_span: Span) -> Result<Spanned<TypeExpr>> {
        ...
        self.consume(&TokenKind::RightBrace, "Expected closing brace for record type")?;
        let end_span = self.current_span();
        let span = start_span.combine(end_span);
