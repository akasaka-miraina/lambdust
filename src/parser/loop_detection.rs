...
mod tests {
    ...
    fn test_simple_circular_dependency() {
        ...
        let result = check_for_infinite_loops(&[expr]);
        assert!(result.is_err());
    }

    #[test]
    fn test_mutual_circular_dependency() {
        // (define x y)
        // (define y x)
        let expressions = vec![
            Expr::List(vec![
        ...
        let result = check_for_infinite_loops(&expressions);
        assert!(result.is_err());
    }
    ...
    #[test]
    fn test_valid_function_with_base_case() {
