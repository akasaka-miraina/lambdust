...
impl Parser {
        ...
        F: FnMut(&mut Self) -> Result<T>,
    {
        ...
        match result {
            Ok(value) => {
                self.consume(
                    &close,
                    &format!("Expected {}", Self::token_kind_name(&close)),
                )?;
                Ok(value)
            }
            Err(err) => {
    ...
    pub fn parse_comma_separated_expressions(&mut self) -> Result<Vec<Spanned<Expr>>> {
        ...
        if self.looks_like_expression_start() {
            ...
            while self.check(&TokenKind::Unquote) {
                // Using Unquote as comma for now
...
mod tests {
    ...
    fn test_token_kind_name() {
        assert_eq!(
            Parser::token_kind_name(&TokenKind::LeftParen),
            "opening parenthesis '('"
        );
        assert_eq!(
            Parser::token_kind_name(&TokenKind::Identifier),
            "identifier"
        );
