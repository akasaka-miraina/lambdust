...
impl Primitives {
    ...
    pub fn tag<'a>(expected: &'static str) -> impl Fn(Input<'a>) -> ParseResult<'a, &'a str> {
        move |input: Input<'a>| {
            if let Some(remaining) = input.strip_prefix(expected) {
                ...
                Err(Box::new(
                    ParseError::new(format!("Expected '{expected}'"), Span::new(0, actual_len))
    ...
    pub fn char<'a>(expected: char) -> impl Fn(Input<'a>) -> ParseResult<'a, char> {
        move |input: Input<'a>| {
            ...
            match chars.next() {
                ...
                None => Err(Box::new(
                    ParseError::new("Unexpected end of input".to_string(), Span::new(0, 0))
                        .with_expected(expected.to_string()),
                )),
            }
        }
    ...
    pub fn char_range<'a>(start: char, end: char) -> impl Fn(Input<'a>) -> ParseResult<'a, char> {
        move |input: Input<'a>| Self::satisfy(|ch| ch >= start && ch <= end)(input)
    }
    ...
    pub fn eof<'a>() -> impl Fn(Input<'a>) -> ParseResult<'a, ()> {
        |input: Input<'a>| {
            if input.is_empty() {
                ...
                Err(Box::new(
                    ParseError::new("Expected end of input".to_string(), Span::new(0, 1))
...
impl SchemePreimitives {
    ...
    pub fn identifier_start<'a>() -> impl Fn(Input<'a>) -> ParseResult<'a, char> {
        Primitives::satisfy(|ch| ch.is_alphabetic() || "!$%&*+-./:<=>?@^_~".contains(ch))
    }

    /// Scheme識別子継続文字
    pub fn identifier_continue<'a>() -> impl Fn(Input<'a>) -> ParseResult<'a, char> {
        Primitives::satisfy(|ch| ch.is_alphanumeric() || "!$%&*+-./:<=>?@^_~".contains(ch))
    }
    ...
    pub fn scheme_delimiter<'a>() -> impl Fn(Input<'a>) -> ParseResult<'a, char> {
        Primitives::satisfy(|ch| ch.is_whitespace() || "()[]{}\"';#|\\".contains(ch))
    }
...
impl ErrorUtils {
        ...
        P: Fn(Input<'a>) -> ParseResult<'a, T>,
    {
        move |input: Input<'a>| {
            parser(input)
                .map_err(|err| Box::new(err.as_ref().clone().with_context(context.to_string())))
        }
    }

    /// カスタムエラーメッセージ
    pub fn custom_error<'a, T>(message: String, span: Span) -> ParseResult<'a, T> {
    ...
    pub fn expected_error<'a, T>(
        ...
        Err(Box::new(
            ParseError::new(format!("Expected {expected}, found {actual}"), span)
