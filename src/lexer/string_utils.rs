...
pub fn validate_character_literal(content: &str, span: Span) -> Result<()> {
    ...
    match content {
        "alarm" | "backspace" | "delete" | "escape" | "newline" | "null" | "return" | "space"
        | "tab" | "vtab" => {
            return Ok(());
        }
        _ => {}
                ...
                Some((_, escape_ch)) => {
                    match escape_ch {
                            ...
                            let code_point =
                                u32::from_str_radix(&hex_digits, 16).map_err(|_| {
                                    Error::internal_error(
                                        "Invalid Unicode escape during unescaping",
                                    )
                                })?;
                            let unicode_char = char::from_u32(code_point).ok_or_else(|| {
                                Error::internal_error(
                                    "Invalid Unicode code point during unescaping",
                                )
                            })?;
                        ...
                        ch if ch.is_ascii_digit() && ('0'..='7').contains(&ch) => {
                            ...
                            let octal_value =
                                u8::from_str_radix(&octal_digits, 8).map_err(|_| {
                                    Error::internal_error("Invalid octal escape during unescaping")
                                })?;
                            result.push(octal_value as char);
                        }
                        _ => {
                            return Err(Box::new(Error::internal_error(format!(
                                "Unknown escape sequence during unescaping: \\{escape_ch}"
                            ))));
                        }
                    }
                }
...
pub fn parse_character_literal(content: &str) -> Result<char> {
    ...
    if let Some(hex_part) = content.strip_prefix('x') {
        let code_point = u32::from_str_radix(hex_part, 16).map_err(|_| {
            Error::internal_error("Invalid Unicode character literal during parsing")
        })?;
        return char::from_u32(code_point)
            .ok_or_else(|| {
                Error::internal_error("Invalid Unicode code point during character parsing")
            })
            .map_err(Box::new);
    }
    ...
    if chars.next().is_some() {
        return Err(Box::new(Error::internal_error(
            "Multiple characters in character literal during parsing",
        )));
    }
