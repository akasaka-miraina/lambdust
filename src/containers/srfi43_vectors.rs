...
use crate::containers::{Container, Capacity, ContainerError, ContainerResult};
...
impl Srfi43Vector {
    ...
    pub fn pop(&self) -> Option<Value> {
        *self.modification_count.write() += 1;
        ...
        self.elements.write().pop().map(|nan_boxed| nan_boxed.to_value())
    }
    ...
    pub fn slice(&self, start: usize, end: usize) -> ContainerResult<Self> {
        ...
        let slice_values: Vec<Value> = slice_elements.iter()
            .map(|nb| nb.to_value())
            .collect();
        ...
        F: Fn(f64) -> f64,
    {
        if let Some(ref cache) = self.homogeneous_cache {
            if cache.element_type() == HomogeneousVectorType::F64 {
                ...
                let elements = self.elements.read();
                let mapped: Vec<Value> = elements.iter()
    ...
    pub fn fill(&self, value: Value, start: Option<usize>, end: Option<usize>) -> ContainerResult<()> {
        *self.modification_count.write() += 1;
        ...
        if start_idx > end_idx || end_idx > len {
            return Err(ContainerError::IndexOutOfBounds {
                index: if start_idx > end_idx { start_idx } else { end_idx },
                length: len,
            });
        }
...
pub mod constructors {
    ...
    /// 
    /// Time Complexity: O(n)
    pub fn unfold<F, G>(length: usize, seed: Value, generator: F, next_seed: Option<G>) -> Srfi43Vector
    where
...
pub mod iterators {
        ...
        F: Fn(Value, Value) -> Value,
    {
        ...
        elements.into_iter().reduce(|acc, elem| combiner(acc, elem)).unwrap_or(identity)
    }
