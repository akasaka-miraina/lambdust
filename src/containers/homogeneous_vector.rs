#![allow(missing_docs)]
//! SRFI-4 Homogeneous Numeric Vectors - Memory-Optimized Implementation
//!
...
impl RawHomogeneousStorage {
    ...
    unsafe fn ptr_at(&self, index: usize) -> *mut u8 {
        ...
        unsafe {
            self.data
                .as_ptr()
                .add(index * self.element_type.element_size())
        }
    }
    ...
    unsafe fn as_mut_slice<T>(&mut self) -> &mut [T] {
        unsafe { std::slice::from_raw_parts_mut(self.data.as_ptr() as *mut T, self.length) }
    }
}
...
impl HomogeneousVector {
    ...
    pub fn new(element_type: HomogeneousVectorType) -> Self {
        Self {
            storage: Arc::new(RwLock::new(RawHomogeneousStorage::new(
                element_type,
                MIN_CAPACITY,
            ))),
        }
...
mod tests {
    ...
    fn test_memory_layout_optimization() {
        ...
        assert!(
            vec.capacity() >= 1000,
            "Vector should have at least requested capacity"
        );
