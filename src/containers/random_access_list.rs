...
impl RandomAccessList {
    ...
    pub fn cons_front(self, value: Value) -> Self {
        ...
        if digits.len() >= 2 && digits[0].weight == digits[1].weight {
            ...
            let new_tree = Arc::new(Tree::node(value, first.tree, second.tree));
...
impl ThreadSafeRandomAccessList {
    ...
    /// Gets element by index
    pub fn get(&self, index: usize) -> Option<Value> {
        self.inner.try_read().unwrap().get(index).cloned()
    }
...
mod tests {
    ...
    #[test]
    fn test_append() {
        let list1 = RandomAccessList::from_vec(vec![Value::number(1.0), Value::number(2.0)]);

        let list2 = RandomAccessList::from_vec(vec![Value::number(3.0), Value::number(4.0)]);
    ...
    fn test_take_drop_split() {
        ...
        assert_eq!(left.to_vec(), vec![Value::number(1.0), Value::number(2.0),]);
        assert_eq!(
            right.to_vec(),
    ...
    fn test_cons_back_uncons_back() {
        ...
        assert_eq!(rest.len(), 2);
        assert_eq!(rest.to_vec(), vec![Value::number(1.0), Value::number(2.0),]);
    }
}
