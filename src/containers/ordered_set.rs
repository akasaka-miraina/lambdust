...
impl RedBlackTree {
    ...
    fn search_node(&self, node: &Option<Arc<Node>>, value: &Value) -> Option<Arc<Node>> {
        match node {
            None => None,
            Some(n) => match self.comparator.compare(value, &n.value) {
            ...
            Some(n) => {
                match self.comparator.compare(value, &n.value) {
                    Ordering::Less => {
                        ...
                        if !is_red(&n.left)
                            && n.left.as_ref().map(|l| !is_red(&l.left)).unwrap_or(true)
                        {
                    ...
                    _ => {
                        if is_red(&n.left) {
                            ...
                            if self.comparator.compare(value, &n.value) == Ordering::Equal
                                && n.right.is_none()
                            {
                                return None;
                            }
                            ...
                            if !is_red(&n.right)
                                && n.right.as_ref().map(|r| !is_red(&r.left)).unwrap_or(true)
                            {
...
impl ThreadSafeOrderedSet {
    ...
    /// Checks if the set contains a value
    pub fn contains(&self, value: &Value) -> bool {
        self.inner.try_read().unwrap().contains(value)
    }

    /// Gets the minimum value
    pub fn min(&self) -> Option<Value> {
        self.inner.try_read().unwrap().min()
    }

    /// Gets the maximum value
    pub fn max(&self) -> Option<Value> {
        self.inner.try_read().unwrap().max()
    }

    /// Gets all values in a range
    pub fn range(&self, low: &Value, high: &Value) -> Vec<Value> {
        self.inner.try_read().unwrap().range(low, high)
    }
    ...
    pub fn to_vec(&self) -> Vec<Value> {
        self.inner.try_read().unwrap().to_vec()
    }

    /// Union with another set
    pub fn union(&self, other: &Self) -> Self {
        let self_set = self.inner.try_read().unwrap();
        let other_set = other.inner.try_read().unwrap();
        Self {
...
mod tests {
    ...
    fn test_functional_operations() {
        ...
        // Fold
        let sum = set.fold(0.0, |acc, v| acc + v.as_number().unwrap_or(0.0));
        assert_eq!(sum, 15.0);
