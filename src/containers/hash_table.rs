...
use super::{Container, ContainerError, ContainerResult, capacities, load_factors};
...
impl HashTable {
    ...
    pub fn with_comparator(comparator: HashComparator) -> Self {
        Self::with_capacity_and_comparator(capacities::DEFAULT_HASH_TABLE_CAPACITY, comparator)
    }
    ...
    fn shift_back(&mut self, mut pos: usize) {
        ...
        while let Some(entry) = self.buckets[pos].take() {
            ...
            self.buckets[new_pos] =
                Some(Entry::new(entry.key, entry.value, entry.hash, new_distance));
...
impl ThreadSafeHashTable {
    ...
    /// Gets a value by key
    pub fn get(&self, key: &Value) -> Option<Value> {
        self.inner.try_read().unwrap().get(key).cloned()
    }
    ...
    /// Checks if the hash table contains a key
    pub fn contains_key(&self, key: &Value) -> bool {
        self.inner.try_read().unwrap().contains_key(key)
    }
    ...
    /// Returns all keys
    pub fn keys(&self) -> Vec<Value> {
        self.inner.try_read().unwrap().keys()
    }
    ...
    /// Gets statistics
    pub fn stats(&self) -> HashTableStats {
        self.inner.try_read().unwrap().stats()
    }
...
mod tests {
    ...
    fn test_srfi_125_operations() {
        ...
        assert_eq!(
            table.hash_table_ref(&key1, Some(default.clone())),
            Ok(default.clone())
        );

        // Test hash-table-set! with multiple pairs
        table.hash_table_set(&[
        ...
        table
            .hash_table_update(
                &key1,
                |v| Value::number(v.as_number().unwrap() + 10.0),
                None,
            )
            .unwrap();
