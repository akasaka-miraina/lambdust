...
impl PriorityQueue {
    ...
    pub fn change_priority(&mut self, target: &Value, new_priority: Value) -> bool {
        if let Some(index) = self.find_value_index(target) {
            let old_priority =
                std::mem::replace(&mut self.heap[index].priority, new_priority.clone());
    ...
    fn find_value_index(&self, target: &Value) -> Option<usize> {
        self.heap.iter().position(|entry| entry.value == *target)
    }
    ...
    fn compare_priorities(&self, a: &Value, b: &Value) -> Ordering {
        ...
        if self.is_max_heap { cmp } else { cmp.reverse() }
    }
...
impl ThreadSafePriorityQueue {
    ...
    pub fn contains(&self, target: &Value) -> bool {
        self.inner.try_read().unwrap().contains(target)
    }

    /// Returns the number of elements
    pub fn len(&self) -> usize {
        self.inner.try_read().unwrap().len()
    }

    /// Checks if the queue is empty
    pub fn is_empty(&self) -> bool {
        self.inner.try_read().unwrap().is_empty()
    }
    ...
    /// Returns all values
    pub fn values(&self) -> Vec<Value> {
        self.inner.try_read().unwrap().values()
    }

    /// Returns all priorities
    pub fn priorities(&self) -> Vec<Value> {
        self.inner.try_read().unwrap().priorities()
    }

    /// Returns all entries
    pub fn entries(&self) -> Vec<(Value, Value)> {
        self.inner.try_read().unwrap().entries()
    }
    ...
    pub fn to_sorted_vec(&self) -> Vec<(Value, Value)> {
        self.inner.try_read().unwrap().to_sorted_vec()
    }

    /// Gets the current capacity
    pub fn capacity(&self) -> usize {
        self.inner.try_read().unwrap().capacity()
    }
...
impl PriorityQueue {
    ...
    pub fn min_max(&self) -> Option<((Value, Value), (Value, Value))> {
        ...
        for entry in &self.heap[1..] {
            match self
                .comparator
                .compare(&entry.priority, &min_entry.priority)
            {
                Ordering::Less => min_entry = entry,
                Ordering::Greater => {
                    if self
                        .comparator
                        .compare(&entry.priority, &max_entry.priority)
                        == Ordering::Greater
                    {
                        max_entry = entry;
                    }
...
mod tests {
    ...
    fn test_basic_operations() {
        ...
        assert_eq!(
            pq.peek(),
            Some((&Value::string("high"), &Value::number(10.0)))
        );
        ...
        assert_eq!(
            pq.extract(),
            Some((Value::string("medium"), Value::number(5.0)))
        );
        assert_eq!(
            pq.extract(),
            Some((Value::string("low"), Value::number(1.0)))
        );
        assert!(pq.is_empty());
    }
    ...
    fn test_large_priority_queue() {
        ...
        for i in 0..1000 {
            pq.insert(
                ...
                Value::number((i * 17) as f64 % 1000.0),
            );
        }
