...
impl CodeCache {
    ...
    pub fn store(&self, expr: Expr, code: NativeCode) -> Result<()> {
        ...
        // Update memory usage
        {
            let mut memory_usage = self.current_memory_usage.lock().map_err(|_| {
                Error::runtime_error("Failed to acquire memory lock".to_string(), None)
            })?;
        ...
        // Store the entry
        {
            let mut entries = self.entries.write().map_err(|_| {
                Error::runtime_error("Failed to acquire entries lock".to_string(), None)
            })?;
        ...
        if self.config.enable_lru_eviction {
            let mut lru_order = self.lru_order.lock().map_err(|_| {
                Error::runtime_error("Failed to acquire LRU lock".to_string(), None)
            })?;
    ...
    fn handle_memory_pressure(&self) -> Result<()> {
        let current_usage = {
            let memory_usage = self.current_memory_usage.lock().map_err(|_| {
                Error::runtime_error("Failed to acquire memory lock".to_string(), None)
            })?;
            *memory_usage
        };
        ...
        if self.config.enable_compaction {
            let should_compact = {
                let last_compaction = self.last_compaction.lock().map_err(|_| {
                    Error::runtime_error("Failed to acquire compaction lock".to_string(), None)
                })?;
    ...
    fn evict_entries(&self) -> Result<()> {
        ...
        let keys_to_evict = {
            let entries = self.entries.try_read().map_err(|_| {
                Error::runtime_error("Failed to acquire entries lock".to_string(), None)
            })?;
            let lru_order = self.lru_order.lock().map_err(|_| {
                Error::runtime_error("Failed to acquire LRU lock".to_string(), None)
            })?;
    ...
    fn compact(&self) -> Result<()> {
        ...
        {
            let mut last_compaction = self.last_compaction.lock().map_err(|_| {
                Error::runtime_error("Failed to acquire compaction lock".to_string(), None)
            })?;
...
mod tests {
    ...
    use crate::jit::code_generator::{
        CodeMetadata, FunctionSignature, MemoryLayout, MemoryRequirements, SchemeType,
    };
