...
use crate::jit::{
    ...
    jit_primitives::JitPrimitiveRegistry,
};
...
pub enum MemoryAllocation {
    ...
    Variable { base_bytes: usize, per_digit_bytes: usize },
...
impl NumberTowerSupport {
    ...
    fn initialize_standard_specializations(&mut self) -> Result<()> {
        ...
        self.add_specialization("+", NumericSpecialization {
        ...
        self.add_specialization("+", NumericSpecialization {
            ...
            performance: NumericPerformanceProfile {
                ...
                precision_loss: PrecisionLoss {
                    ...
                    precision_loss_conditions: vec!["floating point representation".to_string()],
                },
    ...
    pub fn find_specialization(
        ...
        specializations.iter().find(|&spec| self.types_compatible(&spec.input_types, input_types))
    }
    ...
    fn promote_base_type(&self, current: BaseNumericType, new: BaseNumericType) -> BaseNumericType {
        match (current, new) {
            (BaseNumericType::Complex, _) | (_, BaseNumericType::Complex) => BaseNumericType::Complex,
            (BaseNumericType::Real, _) | (_, BaseNumericType::Real) => BaseNumericType::Real,
            (BaseNumericType::Rational, _) | (_, BaseNumericType::Rational) => BaseNumericType::Rational,
    ...
    fn apply_jit_strategy(
        ...
        match strategy {
            NumericJitStrategy::InlineNative { use_overflow_detection, use_simd } => {
                self.generate_inline_native_code(native_code, *use_overflow_detection, *use_simd, tier)?;
            }
            NumericJitStrategy::LibraryCall { function_name, preserves_exactness } => {
                self.generate_library_call_code(native_code, function_name, *preserves_exactness)?;
            }
            NumericJitStrategy::ArbitraryPrecision { precision_threshold, library } => {
    ...
    pub fn promote_value(&self, value: &Value, target_type: &NumericType) -> Result<Value> {
        match value {
            Value::Literal(Literal::ExactInteger(n)) => {
                match target_type {
                    NumericType::ExactRational => {
                        Ok(Value::Literal(Literal::rational(*n, 1)))
                    }
                    NumericType::InexactReal => {
                        Ok(Value::Literal(Literal::InexactReal(*n as f64)))
                    }
                    NumericType::InexactComplex => {
                        Ok(Value::Literal(Literal::complex(*n as f64, 0.0)))
                    }
                    _ => Ok(value.clone()),
                }
            }
...
impl ExactnessRules {
    ...
    pub fn r7rs_default() -> Self {
        ...
        binary_rules.insert("+".to_string(), BinaryExactnessRule {
        ...
        unary_rules.insert("sqrt".to_string(), UnaryExactnessRule {
        ...
        for &func in &["sin", "cos", "tan", "asin", "acos", "atan"] {
            unary_rules.insert(func.to_string(), UnaryExactnessRule {
...
mod tests {
    ...
    fn test_numeric_type_classification() {
        ...
        assert_eq!(support.get_numeric_type(&exact_int), Some(NumericType::ExactInteger));
        
        // Test inexact real
        let inexact_real = Value::Literal(Literal::InexactReal(3.14));
        assert_eq!(support.get_numeric_type(&inexact_real), Some(NumericType::InexactReal));
        
        // Test rational
        let rational = Value::Literal(Literal::rational(3, 4));
        assert_eq!(support.get_numeric_type(&rational), Some(NumericType::ExactRational));
    }
    ...
    fn test_exactness_rules() {
        ...
        // Exact + exact = exact
        let result = support.apply_exactness_rules("+", &[NumericType::ExactInteger, NumericType::ExactInteger]);
        assert_eq!(result, NumericType::ExactInteger);
        
        // Exact + inexact = inexact
        let result = support.apply_exactness_rules("+", &[NumericType::ExactInteger, NumericType::InexactReal]);
    ...
    fn test_specialization_lookup() {
        ...
        let spec = support.find_specialization("+", &[NumericType::ExactInteger, NumericType::ExactInteger]);
        assert!(spec.is_some());
        let spec = spec.unwrap();
        assert_eq!(spec.output_type, NumericType::ExactInteger);
        assert!(matches!(spec.exactness_behavior, ExactnessBehavior::PreservesExactness));
