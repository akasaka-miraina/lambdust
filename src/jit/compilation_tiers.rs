...
pub struct TierConfig {
    ...
    /// Enable aggressive optimizations at higher tiers
...
impl TieredCode {
    ...
    pub fn record_execution(&mut self, execution_time: Duration) {
        ...
        self.performance_since_tier_change
...
impl PerformanceMetrics {
    ...
    fn trend(&self) -> PerformanceTrend {
        ...
        let second_half: f64 = self.execution_times[mid..]
            ...
            .map(|d| d.as_nanos() as f64)
            .sum::<f64>()
            / (self.execution_times.len() - mid) as f64;
...
impl TierManager {
    ...
    fn should_tier_up(&self, code: &TieredCode, profile: &ExecutionProfile) -> Result<bool> {
        ...
        let execution_threshold_met =
            code.execution_count >= self.config.tier_up_execution_threshold;
        ...
        let performance_ok = matches!(
            code.performance_trend(),
    ...
    pub fn deoptimize(&mut self, expr: &Expr, reason: String) -> Result<CompilationTier> {
        if !self.config.enable_deoptimization {
            return Err(Box::new(Error::runtime_error(
                "Deoptimization disabled".to_string(),
                None,
            )));
        }
