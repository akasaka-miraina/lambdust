...
use crate::diagnostics::{UnifiedError, UnifiedResult, JitError, ErrorSeverity, Span};
use crate::jit::CompilationTier;

// Re-export unified error system for JIT modules
pub use crate::{error_convert, propagate_error, validate_arity, handle_result, error_chain};
...
impl JitUnifiedError {
    ...
    pub fn with_tier(mut self, tier: CompilationTier) -> Self {
        ...
        self.base = self.base.with_context("compilation_tier", format!("{:?}", tier));
        self
    }
    ...
    pub fn with_performance_impact(mut self, impact: PerformanceImpact) -> Self {
        self.base = self.base.with_context(
            "performance_impact", 
            format!("degradation: {:.2}x, fallback: {}", 
                impact.degradation_factor, 
                if impact.fallback_available { "yes" } else { "no" }
            )
        );
...
macro_rules! jit_error {
    ...
    ($kind:expr, $message:expr, impact: $impact:expr) => {
        $crate::jit::unified_jit_errors::JitUnifiedError::new($kind, $message).with_performance_impact($impact)
    };
    ...
    ($operation:expr, tier: $tier:expr) => {{
        ...
        match result {
            ...
            Err(mut error) => {
                ...
                error.base = error.base.with_context("execution_time", format!("{:?}", duration));
                Err(error)
            }
        }
    }};
...
impl JitUnifiedError {
    ...
    pub fn optimization_failed(message: impl Into<String>, expected_benefit: f64) -> Self {
        Self::new(JitErrorKind::Optimization, message)
            .with_performance_impact(PerformanceImpact {
    ...
    pub fn security_failed(message: impl Into<String>) -> Self {
        Self::new(JitErrorKind::Security, message)
            .with_performance_impact(PerformanceImpact {
    ...
    pub fn memory_failed(message: impl Into<String>) -> Self {
        Self::new(JitErrorKind::MemoryManagement, message)
            .with_performance_impact(PerformanceImpact {
    ...
    pub fn hotspot_failed(message: impl Into<String>) -> Self {
        Self::new(JitErrorKind::HotspotDetection, message)
            .with_performance_impact(PerformanceImpact {
...
mod tests {
    ...
    fn test_jit_error_creation() {
        let error = JitUnifiedError::codegen_failed("Test codegen failure", CompilationTier::JitBasic);
    ...
    fn test_performance_impact() {
        ...
        assert_eq!(
            error.performance_impact.as_ref().unwrap().degradation_factor, 
            2.0
        );
    }
}