...
use crate::jit::{
    JitCompiler, DependentHotspotDetector, SpecializedTierManager,
    JitSecurityFramework, DeoptimizationManager, JitConfig,
    SecurityVerificationResult, DeoptimizationReason,
};
use crate::jit::dependent_hotspot_detector::{
    DependentExecutionProfile, DependentCompilationCandidate, TypeExecutionContext
};
...
impl JitMemoryPool {
    ...
    unsafe fn deallocate(&self, ptr: NonNull<JitExecutionContext>) {
        ...
        unsafe {
            ...
            debug_assert!(slot < self.pool_size, "Slot index {} exceeds pool size {}", slot, self.pool_size);
...
impl AtomicRuntimeStats {
    ...
    fn record_execution(&self, execution_time: Duration, used_compiled_code: bool) {
        ...
        self.total_execution_time_nanos.fetch_add(execution_time.as_nanos() as u64, Ordering::Relaxed);
        
        if used_compiled_code {
            self.compiled_executions.fetch_add(1, Ordering::Relaxed);
            self.compiled_execution_time_nanos.fetch_add(execution_time.as_nanos() as u64, Ordering::Relaxed);
        }
    }
    ...
    fn to_runtime_stats(&self) -> JitRuntimeStats {
        JitRuntimeStats {
            ...
            total_execution_time: Duration::from_nanos(self.total_execution_time_nanos.load(Ordering::Relaxed)),
            compiled_execution_time: Duration::from_nanos(self.compiled_execution_time_nanos.load(Ordering::Relaxed)),
...
impl JitRuntime {
    ...
    pub fn new(config: JitConfig) -> Result<Self> {
        ...
        let specialized_config = SpecializedTierConfig {
            specialization_config: crate::jit::specialized_compilation_tiers::SpecializationEngineConfig::default(),
            cache_config: crate::jit::specialized_compilation_tiers::SpecializedCacheConfig,
            performance_config: crate::jit::specialized_compilation_tiers::PerformanceTrackingConfig,
        };
        ...
        Ok(Self {
            ...
            dependent_detector: Arc::new(Mutex::new(DependentHotspotDetector::new(config.hotspot_config.clone())?)),
            specialized_tiers: Arc::new(Mutex::new(SpecializedTierManager::new(specialized_config)?)),
    ...
    pub fn execute_with_jit(
        ...
        if let Some(result) = self.try_execute_compiled_code(identifier, ast, env, &mut execution_context)? {
    ...
    fn trigger_adaptive_compilation(
        ...
        {
            let active = self.active_compilations.read()
                .map_err(|_| Error::runtime_error("Failed to acquire compilations lock".to_string(), None))?;
        ...
        let candidates = {
            let detector = self.dependent_detector.lock()
        ...
        let our_candidate = candidates.iter()
        ...
        if let Some(candidate) = our_candidate {
            ...
            {
                let mut active = self.active_compilations.write()
                    .map_err(|_| Error::runtime_error("Failed to acquire compilations lock".to_string(), None))?;
    ...
    fn perform_compilation(&self, task: CompilationTask) -> Result<()> {
        ...
        let tier_result = {
            let mut specialized_tiers = self.specialized_tiers.lock()
                .map_err(|_| Error::runtime_error("Failed to acquire specialized tiers lock".to_string(), None))?;
        ...
        match tier_result {
            ...
            SpecializedCompilationResult::SpecializedTier { tier, opportunities, .. } => {
    ...
    fn compile_with_specialized_tier(
        ...
        let specialized_code = {
            ...
            specialized_tiers.compile_specialized(
                &task.ast,
                tier,
                opportunities,
                &profile,
    ...
    fn compile_with_base_tier(
        ...
        let base_compiler = self.base_compiler.lock()
            .map_err(|_| Error::runtime_error("Failed to acquire base compiler lock".to_string(), None))?;
    ...
    fn verify_and_store_specialized_code(
        ...
        let verification_result = {
            ...
            security.verify_specialized_code(code, &proofs)?
        };
    ...
    fn store_verified_code(&self, identifier: &str, code: SpecializedNativeCode) -> Result<()> {
        let mut contexts = self.execution_contexts.write()
            .map_err(|_| Error::runtime_error("Failed to acquire contexts lock".to_string(), None))?;
    ...
    fn handle_security_rejection(&self, identifier: &str, reason: String) -> Result<()> {
        ...
        let mut deopt_manager = self.deoptimization_manager.lock()
            .map_err(|_| Error::runtime_error("Failed to acquire deoptimization manager lock".to_string(), None))?;
    ...
    fn create_deoptimization_context(&self, identifier: &str) -> Result<DeoptimizationContext> {
        let contexts = self.execution_contexts.read()
            .map_err(|_| Error::runtime_error("Failed to acquire contexts lock".to_string(), None))?;
    ...
    fn record_execution_for_profiling(
        ...
        let mut detector = self.dependent_detector.lock()
            .map_err(|_| Error::runtime_error("Failed to acquire detector lock".to_string(), None))?;
    ...
    fn execute_with_interpreter_fallback(&self, ast: &Expr, env: &Arc<Environment>) -> Result<Value> {
    ...
    fn create_dependent_profile_from_candidate(
        ...
        Ok(DependentExecutionProfile {
            ...
            type_computation_stats: crate::jit::dependent_hotspot_detector::TypeComputationStats::new(),
            ...
            memory_access_patterns: crate::jit::dependent_hotspot_detector::MemoryAccessPatterns::new(),
        })
    }
    ...
    fn update_runtime_stats(
        ...
        self.atomic_stats.record_execution(execution_time, used_compiled_code);
        Ok(())
    }
...
impl JitExecutionContext {
    ...
    pub fn reset(&mut self, identifier: String, ast: crate::ast::Expr, env: Arc<crate::eval::Environment>) {
...
impl JitRuntimeStats {
    ...
    pub fn record_execution(&mut self, identifier: &str, execution_time: Duration, used_compiled_code: bool) {
        ...
        if used_compiled_code {
            ...
            self.compiled_functions.entry(identifier.to_string()).or_insert_with(Instant::now);
        } else {
    ...
    pub fn average_speedup(&self) -> f64 {
        ...
        let avg_interpreted_time = self.interpreted_execution_time.as_nanos() as f64 / self.interpreted_executions as f64;
        let avg_compiled_time = self.compiled_execution_time.as_nanos() as f64 / self.compiled_executions as f64;
