...
impl InstructionBuffer {
    ...
    unsafe fn as_mut_ptr_at(&mut self, offset: usize) -> *mut u8 {
        ...
        unsafe { self.buffer_ptr.add(offset) }
    }
}
...
impl MemoryLayoutOptimizer {
    ...
    fn optimize_layout(&self, layout: &mut MemoryLayout) {
        ...
        layout.stack_frame_size =
            (layout.stack_frame_size + self.cache_line_size - 1) & !(self.cache_line_size - 1);
...
impl CodeGenerator {
        ...
        match expr {
            ...
            Expr::Lambda { formals, body, .. } => {
                ...
                let body_expr = body
                    .first()
                    .map(|e| &e.inner)
                    .unwrap_or(&Expr::Literal(crate::ast::Literal::Nil));
            ...
            Expr::If {
                ...
                consequent,
                alternative,
            } => self.compile_if_basic(
                builder,
                &test.inner,
                &consequent.inner,
                alternative.as_ref().map(|e| &e.inner),
                type_info,
            ),
            _ => {
    ...
    fn constant_folding(&mut self, expr: Expr) -> Result<Expr> {
        match expr {
            Expr::List(ref exprs) if exprs.len() >= 3 => {
                ...
                if let (
                    Expr::Symbol(op),
                    Expr::Literal(Literal::ExactInteger(a)),
                    Expr::Literal(Literal::ExactInteger(b)),
    ...
    fn compile_literal_optimized(
        ...
        match lit {
            Literal::ExactInteger(n) if *n >= -128 && *n <= 127 => builder.emit_load_immediate(*n),
    ...
    fn compile_list_optimized(
        ...
        if let Expr::Symbol(op_name) = &exprs[0].inner {
            match op_name.as_str() {
                "+" | "-" | "*" | "/" if exprs.len() == 3 => {
                    return self.compile_arithmetic_optimized(
                        builder, op_name, &exprs[1], &exprs[2], type_info,
                    );
                }
                _ => {}
            }
    ...
    fn compile_arithmetic_optimized(
        &mut self,
        builder: &mut CodeBuilder,
        op: &str,
        left: &Expr,
    ...
    fn compile_lambda_basic(
        &mut self,
        builder: &mut CodeBuilder,
        _formals: &crate::ast::Formals,
        body: &Expr,
        type_info: &TypeInfo,
    ) -> Result<()> {
...
impl CodeBuilder {
    ...
    fn emit_load_constant(&mut self, _lit: &Literal) -> Result<()> {
        Ok(())
    }
    fn emit_load_immediate(&mut self, _val: i64) -> Result<()> {
        Ok(())
    }
    fn emit_variable_lookup(&mut self, _name: &str) -> Result<()> {
        Ok(())
    }
    fn emit_integer_variable_lookup(&mut self, _name: &str) -> Result<()> {
        Ok(())
    }
    fn emit_empty_list(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_function_call(&mut self, _arg_count: usize) -> Result<()> {
        Ok(())
    }
    fn emit_generic_call(&mut self, _expr: &Expr) -> Result<()> {
        Ok(())
    }
    fn emit_integer_add(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_integer_subtract(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_integer_multiply(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_integer_divide(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_generic_arithmetic(&mut self, _op: &str) -> Result<()> {
        Ok(())
    }
    fn emit_lambda_prologue(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_lambda_epilogue(&mut self) -> Result<()> {
        Ok(())
    }
    fn emit_branch_if_false(&mut self) -> Result<usize> {
        Ok(0)
    }
    fn emit_jump(&mut self) -> Result<usize> {
        Ok(0)
    }
    fn emit_label(&mut self, _label: usize) -> Result<()> {
        Ok(())
    }
}
