...
use crate::effects::continuation_monad::{ContinuationFunction, ContinuationMonad};
        ...
        U: Send + Sync + 'static + Clone,
    {
        match self.action {
            ...
            _ => {
                ...
                panic!(
                    "bind currently only works with Pure values - complex type conversion needed"
                )
            }
        }
    }
    ...
    W: Into<Value>,
{
    fn from(writer: Writer<W, Value>) -> Self {
        ...
        Value::Pair(Box::new(value), Box::new(output.into()))
    }
}
...
mod tests {
    ...
    #[test]
    fn test_maybe_monad() {
        let result = Maybe::just(21).bind(|x| Maybe::just(x * 2));

        assert_eq!(result, Maybe::just(42));

        let nothing_result = Maybe::<i32>::nothing().bind(|x| Maybe::just(x * 2));
    ...
    fn test_either_monad() {
        let result = Either::<&str, i32>::right(21).bind(|x| Either::right(x * 2));

        assert_eq!(result, Either::right(42));

        let error_result = Either::<&str, i32>::left("error").bind(|x| Either::right(x * 2));
    ...
    fn test_reader_monad() {
        let computation =
            Reader::<String, String>::ask().bind(|env| Reader::pure(format!("Hello, {env}!")));
    ...
    fn test_list_monad_integration() {
        ...
        let result =
            ListMonad::from_vec(vec![1, 2]).bind(|x| ListMonad::from_vec(vec![x * 2, x * 3]));
    ...
    fn test_writer_r7rs_integration() {
        ...
        let writer = Writer::<String, ()>::tell("Debug: ".to_string()).bind(|_| {
            Writer::new(
