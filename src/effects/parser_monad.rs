...
impl Input {
    ...
    pub fn get_state<T: std::any::Any + Send + Sync>(&self) -> Option<&T> {
        self.user_state.as_ref().and_then(|s| s.downcast_ref::<T>())
    }
}
        ...
        match self.parse(input) {
            Ok((result, remaining)) => {
                if remaining.is_empty() {
                    ...
                    Err(
                        ParseError::new(pos, "Unexpected input after parsing".to_string())
...
impl<T> Parser<T> {
    ...
    pub fn fail(message: String) -> Self {
        Parser::new(move |input| Err(ParseError::new(input.position, message.clone())))
    }
        ...
        F: Fn(T, T) -> T + Send + Sync + 'static + Clone,
    {
        ...
        self.bind(move |first| {
            ...
            let op_and_operand =
                op_clone.bind(move |f| self_clone2.clone().map(move |second| (f.clone(), second)));

            op_and_operand.many().map(move |ops| {
                ops.into_iter()
                    .fold(first.clone(), |acc, (op_func, operand)| {
                        op_func(acc, operand)
                    })
            })
        })
    ...
    where
        ...
        U: Send + Sync + 'static,
    {
        self.bind(move |value| other.clone().map(move |_| value.clone()))
    }
...
impl Parser<char> {
    ...
    pub fn any_char() -> Self {
        Parser::named("any_char".to_string(), |input| {
            ...
            match input.advance_char() {
                ...
                None => Err(ParseError::new(pos, "Unexpected end of input".to_string())
    ...
    /// Parse a digit
    pub fn digit() -> Self {
        Parser::satisfy(|ch| ch.is_ascii_digit()).with_name("digit".to_string())
    }

    /// Parse a letter
    pub fn letter() -> Self {
        Parser::satisfy(|ch| ch.is_alphabetic()).with_name("letter".to_string())
    }

    /// Parse an alphanumeric character
    pub fn alpha_num() -> Self {
        Parser::satisfy(|ch| ch.is_alphanumeric()).with_name("alphanumeric".to_string())
    }

    /// Parse whitespace
    pub fn whitespace() -> Self {
        Parser::satisfy(|ch| ch.is_whitespace()).with_name("whitespace".to_string())
    }
}
...
impl Parser<Value> {
    ...
    pub fn value() -> Self {
        Parser::number()
            .map(|n| Value::Literal(crate::ast::Literal::Number(n)))
            .choice(
            ...
            .choice(
                Parser::string("#t".to_string())
                    .map(|_| Value::Literal(crate::ast::Literal::Boolean(true))),
            )
            .choice(
                Parser::string("#f".to_string())
                    .map(|_| Value::Literal(crate::ast::Literal::Boolean(false))),
            )
