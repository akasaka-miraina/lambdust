use super::{Effect, EffectContext, EffectResult, GenerationalEnvManager, LiftingConfig};
...
impl EffectSystem {
    ...
    pub fn should_lift(&self, operation: &str, current_effects: &[Effect]) -> Option<Effect> {
        ...
        if let Some(effect) = self
            .lifting_config
            .check_custom_rules(operation, current_effects)
        {
            return Some(effect);
        }
        ...
        match operation {
            "display" | "write" | "newline" | "read" | "open-input-file" | "open-output-file"
                if self.lifting_config.auto_lift_io() =>
            {
                Some(Effect::IO)
            }
            "set!" | "vector-set!" | "string-set!" if self.lifting_config.auto_lift_state() => {
                Some(Effect::State)
            }
            "error" | "raise" | "throw" if self.lifting_config.auto_lift_error() => {
                Some(Effect::Error)
            }
            _ => None,
        }
