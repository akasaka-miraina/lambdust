...
use super::{Effect, EffectContext, ErrorAction, IOAction, MonadicValue, StateAction};
...
impl EffectLifter {
    ...
    pub fn lift_operation(&mut self, operation: &str, args: &[Value]) -> Option<MonadicValue> {
        if !self.enabled {
            return None;
        }
    ...
    fn rule_applies(
        ...
        if rule_name == operation || rule.conditions.is_empty() {
            return rule
                .conditions
                .iter()
                .all(|cond| self.condition_matches(cond, operation, args));
        }

        // Otherwise, check if any condition explicitly matches
        rule.conditions
            .iter()
            .any(|cond| self.condition_matches(cond, operation, args))
    }
    ...
    fn condition_matches(
        ...
        match condition {
            ...
            LiftingCondition::ArgumentRange(min, max) => args.len() >= *min && args.len() <= *max,
            LiftingCondition::FirstArgType(value_type) => args
                .first()
                .map(|v| self.value_matches_type(v, value_type))
                .unwrap_or(false),
    ...
    fn apply_lifting_rule(
        ...
        match &rule.transformation {
            LiftingTransformation::Direct => {
                ...
                match &rule.target_effect {
                    ...
                    Effect::Mutation => {
                        ...
                        Ok(MonadicValue::pure(Value::Unspecified))
                    }
                    Effect::Custom(name) => {
                        // For custom effects, create a combined computation
                        Ok(MonadicValue::pure(Value::string(format!(
                            "Custom effect: {name}"
                        ))))
                    }
                }
            ...
            LiftingTransformation::Custom => {
                ...
                Ok(MonadicValue::pure(Value::string(
                    "Custom transformation".to_string(),
                )))
            }
...
impl PartialEq for LiftingCondition {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (LiftingCondition::OperationPattern(a), LiftingCondition::OperationPattern(b)) => {
                a == b
            }
...
impl fmt::Display for LiftingRule {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "LiftingRule(effect={}, priority={}, conditions={})",
            self.target_effect,
            self.priority,
            self.conditions.len()
        )
