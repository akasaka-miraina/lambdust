                ...
                func: |args| {
                    ...
                    if let Value::HashTable(ht) = &args[1] {
                        ...
                        for (hash_key, value) in table.iter() {
                            ...
                            result.push(Value::from_vector(vec![
                                hash_key.to_value(),
                                value.clone(),
                            ]));
                        }
                    ...
                    if let Value::HashTable(ht1) = &args[0] {
                        ...
                        for other_arg in &args[1..] {
                            if let Value::HashTable(ht2) = other_arg {
                                ...
                                return Err(LambdustError::type_error("Expected hash table".to_string()));
                            }
                        }
...
mod tests {
    ...
    fn test_hash_table_find() {
        ...
        if let Value::Procedure(Procedure::Builtin { func, .. }) = find_proc {
            ...
            ht.set(Value::String("key1".to_string()), Value::from(42i64)).unwrap();
            ...
            let result = func(&[dummy_proc, hash_table.clone(), Value::String("key1".to_string())]).unwrap();
    ...
    fn test_hash_table_count() {
        ...
        if let Value::Procedure(Procedure::Builtin { func, .. }) = count_proc {
            ...
            let mut ht = HashTable::new();
            ht.set(Value::String("key1".to_string()), Value::from(1i64)).unwrap();
            ht.set(Value::String("key2".to_string()), Value::from(2i64)).unwrap();
            ht.set(Value::String("key3".to_string()), Value::from(3i64)).unwrap();
            let hash_table = Value::HashTable(Rc::new(RefCell::new(ht)));
