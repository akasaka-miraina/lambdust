...
impl SrfiRegistry {
    ...
    pub fn with_standard_srfis() -> Self {
        ...
        registry.register(Box::new(super::srfi_9::Srfi9));
        registry.register(Box::new(super::srfi_45::Srfi45));
        registry.register(Box::new(super::srfi_46::Srfi46));
        registry.register(Box::new(super::srfi_97::Srfi97));
    ...
    pub fn get_srfi_info(&self, id: u32) -> Option<(u32, &str, Vec<&str>)> {
        self.modules.get(&id).map(|module| {
            (module.srfi_id(), module.name(), module.parts())
        })
    }
    
    /// Import functions from a SRFI
    pub fn import_srfi(&self, import: &SrfiImport) -> Result<HashMap<String, Value>> {
        let module = self.modules.get(&import.id)
            .ok_or_else(|| LambdustError::runtime_error(
                format!("SRFI {} is not available", import.id)
            ))?;
                ...
                if let Some(existing) = exports.get(&name) {
                    ...
                    if !values_equivalent(&value, existing) {
                        return Err(LambdustError::runtime_error(
                            format!("Conflicting exports for '{}' from SRFI {}", name, import.id)
                        ));
                    }
                }
