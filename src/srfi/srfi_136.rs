...
mod tests {
    ...
    fn test_record_type_descriptor_creation() {
        ...
        // Test make-record-type-descriptor
        let make_rtd = exports.get("make-record-type-descriptor").unwrap();
        if let Value::Procedure(crate::value::Procedure::Builtin { func, .. }) = make_rtd {
            let name = Value::Symbol("person".to_string());
    ...
    fn test_record_predicates() {
        ...
        let record_pred = exports.get("record?").unwrap();
        if let Value::Procedure(crate::value::Procedure::Builtin { func, .. }) = record_pred {
        ...
        let rtd_pred = exports.get("record-type-descriptor?").unwrap();
        if let Value::Procedure(crate::value::Procedure::Builtin { func, .. }) = rtd_pred {
            // Test with non-descriptor
            let result = func(&[Value::Boolean(true)]).unwrap();
            assert_eq!(result, Value::Boolean(false));
        }
    }
        ...
        }) = make_rtd
        {
            ...
            }) = get_fields
            {
                ...
                if let Value::Vector(field_names) = result {
                    ...
                    assert_eq!(field_names[0], Value::Symbol("field1".to_string()));
                } else {
