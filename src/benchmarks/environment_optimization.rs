...
use crate::eval::{CachedEnvironment, Environment, Value};
use std::rc::Rc;
...
impl BenchmarkResults {
    ...
    pub fn print_results(&self) {
        ...
        println!(
            "Traditional Environment: {:?} ({:.2} ns/lookup)",
            self.traditional_time, self.avg_traditional_ns
        );
        println!(
            "Cached Environment: {:?} ({:.2} ns/lookup)",
...
pub fn run_comprehensive_benchmark(config: BenchmarkConfig) -> BenchmarkResults {
    println!(
        "Setting up environment chain (depth: {}, vars/level: {})...",
    ...
    let (cached_time, cache_hit_ratio) =
        benchmark_cached_environment(&env, &lookup_patterns, config.cache_size);
...
pub fn verify_correctness(config: &BenchmarkConfig) -> bool {
    ...
    for test_case in &test_cases {
        ...
        if traditional_result != cached_result {
            println!(
                "❌ Correctness error for '{test_case}': traditional={traditional_result:?}, cached={cached_result:?}"
            );
            return false;
        }
    }
...
pub fn run_performance_tests() {
    ...
    for (i, config) in configs.iter().enumerate() {
        ...
        if results.speedup_ratio >= target_speedup {
            println!(
                "✅ Performance target achieved ({:.1}x >= {:.1}x)",
                results.speedup_ratio, target_speedup
            );
        } else {
            println!(
                "⚠️  Performance target not met ({:.1}x < {:.1}x)",
                results.speedup_ratio, target_speedup
            );
        }
    }
