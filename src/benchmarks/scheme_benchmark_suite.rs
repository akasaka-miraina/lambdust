...
impl SchemeBenchmarkSuite {
    ...
    pub fn get_categories(&self) -> Vec<String> {
        let mut categories: Vec<String> =
            self.benchmarks.iter().map(|b| b.category.clone()).collect();
        categories.sort();
        categories.dedup();
        categories
    }
    ...
    pub fn run_benchmark(
        ...
        let performance_grade = if let Some(baseline) = benchmark.baseline_ops_per_sec {
            if ops_per_second >= baseline * 1.2 {
                "Excellent".to_string()
            } else if ops_per_second >= baseline {
                "Good".to_string()
            } else if ops_per_second >= baseline * 0.8 {
                "Fair".to_string()
            } else {
                "Poor".to_string()
            }
        ...
        let comparison_to_baseline = benchmark
    ...
    pub fn run_category_benchmarks(
        ...
        category: &str,
        iterations: usize,
    ) -> Vec<SchemeBenchmarkResult> {
        // Collect benchmark indices to avoid borrowing issues
        let benchmark_indices: Vec<usize> = self
            .benchmarks
            .iter()
            .enumerate()
        ...
        println!(
            "Running {} benchmarks in category: {}",
            benchmark_indices.len(),
            category
        );
    ...
    pub fn run_all_benchmarks(
        &mut self,
        iterations: usize,
    ) -> HashMap<String, Vec<SchemeBenchmarkResult>> {
    ...
    pub fn generate_performance_report(&mut self, iterations: usize) -> String {
        ...
        for (category, results) in &all_results {
            ...
            report.push_str(&format!(
                "Average Performance: {avg_ops_per_sec:.0} ops/sec\n"
            ));
            ...
            for result in sorted_results.iter().take(3) {
                report.push_str(&format!(
                    "  • {}: {:.0} ops/sec ({})\n",
                    result.benchmark_name, result.ops_per_second, result.performance_grade
                ));
            }
