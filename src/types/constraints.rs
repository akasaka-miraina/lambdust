...
use super::{Type, TypeConstraint, TypeVar};
...
impl TypeConstraint {
    ...
    pub fn apply_substitution(&self, subst: &Substitution) -> Self {
        match self {
            ...
            TypeConstraint::Instance { class, type_, span } => TypeConstraint::Instance {
                class: class.clone(),
                type_: subst.apply_to_type(type_),
                span: *span,
            },
            TypeConstraint::Subtype { left, right, span } => TypeConstraint::Subtype {
                left: subst.apply_to_type(left),
                right: subst.apply_to_type(right),
                span: *span,
            },
    ...
    pub fn free_vars(&self) -> HashSet<TypeVar> {
        match self {
            ...
            TypeConstraint::Ambiguous { vars, .. } => vars.iter().cloned().collect(),
        }
    }
    ...
    fn solve_constraint(&mut self, constraint: TypeConstraint) -> ConstraintResult {
        match constraint {
            ...
            TypeConstraint::Default {
                ...
                ConstraintResult::Unresolved(TypeConstraint::Default {
                    var,
                    default_type,
                    span,
                })
...
mod tests {
    ...
    fn test_equality_constraint() {
        ...
        assert_eq!(
            result.substitution.apply_to_type(&Type::Variable(var)),
            Type::Number
        );
    }
    ...
    fn test_unsolvable_instance() {
        let constraint =
            TypeConstraint::instance("Eq", Type::function(vec![Type::Number], Type::Number), None);
