...
use crate::runtime::{MinimalPrimitiveRegistry, MinimalPrimitive};
...
impl SchemeIntegration {
    ...
    fn init_primitive_mappings() -> HashMap<String, DependentType> {
        ...
        mappings.insert("exact-integer".to_string(), 
        ...
        mappings.insert("boolean".to_string(), 
            DependentType::Inductive {
        ...
        mappings.insert("string".to_string(), 
            DependentType::Inductive {
    ...
    pub fn value_to_type(&mut self, value: &Value) -> Result<DependentType> {
        ...
        let dependent_type = match value {
            ...
            Value::Nil => DependentType::Inductive {
                ...
                universe_level: 0,
                constructors: vec![
                    ("null".to_string(), DependentType::Universe(0))
                ],
                induction_principle: None,
            },
    ...
    fn literal_to_dependent_type(&self, literal: &Literal) -> Result<DependentType> {
        match literal {
            ...
            Literal::Boolean(b) => Ok(DependentType::Inductive {
                ...
                universe_level: 0,
                constructors: vec![
                    (if *b { "true".to_string() } else { "false".to_string() }, DependentType::Universe(0))
                ],
                induction_principle: None,
            }),
    ...
    pub fn infer_expression_type(&mut self, expr: &Expr, env: &Environment) -> Result<DependentType> {
        match expr {
    ...
    pub fn is_value_compatible(&mut self, value: &Value, expected_type: &DependentType) -> Result<bool> {
