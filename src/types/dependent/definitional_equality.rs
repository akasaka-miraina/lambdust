...
use super::core::{DependentTerm, DependentType, Normalizer};
...
pub enum EqualityJustification {
    ...
    AlphaEquivalence {
        /// Variable renamings performed during α-conversion
        renamed_vars: Vec<(String, String)>,
    },
    /// Equal by β-reduction
    BetaEquivalence {
        /// β-reductions performed to establish equality
        reductions: Vec<String>,
    },
    /// Equal by η-expansion
    EtaEquivalence {
        /// η-expansions performed to establish equality
        expansions: Vec<String>,
    },
    /// Equal by definitional expansion
    DefinitionalExpansion {
        /// Definitions unfolded to establish equality
        unfolded_defs: Vec<String>,
    },
    /// Equal by congruence (structural compatibility)
    Congruence {
        /// Equality justifications for sub-components
        sub_equalities: Vec<EqualityJustification>,
    },
}
...
pub enum EqualityWitness {
    ...
    Transitivity {
        /// Left witness in transitivity chain
        left: Box<EqualityWitness>,
        /// Right witness in transitivity chain
        right: Box<EqualityWitness>,
        /// Middle term in transitivity proof
...
impl DefinitionalEqualityChecker {
    ...
    pub fn types_equal(
        ...
        // Cache the result
        self.equality_cache
            .insert((ty1.clone(), ty2.clone()), congruence_result.is_equal);
    ...
    pub fn terms_equal(
        ...
        if norm_term1 == norm_term2 {
            ...
            self.term_equality_cache
                .insert((term1.clone(), term2.clone()), true);
            return Ok(result);
        }
        ...
        self.term_equality_cache
            .insert((term1.clone(), term2.clone()), false);
        Ok(result)
    }
    ...
    /// - `(x:A) → B(x) ≡_α (y:A) → B(y)`
    /// - `(x:A) × C(x) ≡_α (z:A) × C(z)`
    fn check_alpha_equivalence_types(
        &mut self,
        ty1: &DependentType,
        ty2: &DependentType,
    ) -> Result<EqualityResult> {
    ...
    fn check_alpha_equivalence_terms(
        &mut self,
        term1: &DependentTerm,
        term2: &DependentTerm,
    ) -> Result<EqualityResult> {
    ...
    fn check_eta_equivalence_types(
        &mut self,
        _ty1: &DependentType,
        _ty2: &DependentType,
    ) -> Result<EqualityResult> {
    ...
    fn check_eta_equivalence_terms(
        &mut self,
        term1: &DependentTerm,
        term2: &DependentTerm,
    ) -> Result<EqualityResult> {
    ...
    fn check_definitional_expansion_types(
        &mut self,
        ty1: &DependentType,
        ty2: &DependentType,
    ) -> Result<EqualityResult> {
    ...
    fn check_congruence_types(
        &mut self,
        ty1: &DependentType,
        ty2: &DependentType,
    ) -> Result<EqualityResult> {
        match (ty1, ty2) {
            (DependentType::Universe(l1), DependentType::Universe(l2)) => Ok(EqualityResult {
                is_equal: l1 == l2,
                ...
                Ok(EqualityResult {
                    ...
                    justification: if is_equal {
                        Some(EqualityJustification::Congruence {
                            sub_equalities: vec![
                                domain_result
                                    .justification
                                    .unwrap_or(EqualityJustification::Syntactic),
                                codomain_result
                                    .justification
                                    .unwrap_or(EqualityJustification::Syntactic),
                            ],
                    ...
                    witness: if is_equal {
                        Some(EqualityWitness::Congruence(vec![
                            domain_result
                                .witness
                                .unwrap_or(EqualityWitness::Reflexivity),
                            codomain_result
                                .witness
                                .unwrap_or(EqualityWitness::Reflexivity),
                        ]))
    ...
    fn is_free_in_term(&self, var: &str, term: &DependentTerm) -> bool {
        match term {
            ...
            DependentTerm::Projection { pair, .. } => self.is_free_in_term(var, pair),
    ...
    fn substitute_term_var(
        ...
        match term {
            DependentTerm::Variable(name) => Ok(DependentTerm::Variable(if name == old_var {
                new_var.to_string()
            } else {
                name.clone()
            })),
...
#[cfg(test)]
mod tests {
    use super::super::core::{DependentTerm, DependentType};
    use super::*;
