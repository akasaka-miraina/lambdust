...
use super::arena::{
    TypeArena, TypeRef, TermRef,
    DependentTypeData, DependentTermData,
    MatchBranchData, PatternData,
    ArenaStats
};
...
impl OptimizedTypingContext {
    ...
    pub fn bind_variable(&mut self, name: String, ty: OptimizedDependentType) -> Result<()> {
        ...
        let type_ref = self.arena.borrow().alloc_type(type_data)?;
    ...
    pub fn lookup_variable(&self, name: &str) -> Result<Option<OptimizedDependentType>> {
        if let Some(&type_ref) = self.variables.get(name) {
            let type_data = self.arena.borrow().resolve_type(type_ref)?.clone();
    ...
    pub fn memory_stats(&self) -> ArenaStats {
        self.arena.borrow().memory_stats()
    }
    ...
    fn convert_to_arena_type(&self, ty: &OptimizedDependentType) -> Result<DependentTypeData> {
        match ty {
            OptimizedDependentType::Universe(level) => {
                Ok(DependentTypeData::Universe(*level))
            }
            OptimizedDependentType::Pi { var, domain, codomain } => {
                ...
                let domain_ref = self.arena.borrow().alloc_type(domain_data)?;
                let codomain_ref = self.arena.borrow().alloc_type(codomain_data)?;
            ...
            OptimizedDependentType::Sigma { var, first, second } => {
                ...
                let first_ref = self.arena.borrow().alloc_type(first_data)?;
                let second_ref = self.arena.borrow().alloc_type(second_data)?;
            ...
            OptimizedDependentType::Identity { ty, left, right } => {
                ...
                let ty_ref = self.arena.borrow().alloc_type(ty_data)?;
                let left_ref = self.arena.borrow().alloc_term(left_data)?;
                let right_ref = self.arena.borrow().alloc_term(right_data)?;
            ...
            OptimizedDependentType::Inductive { name, parameters, universe_level, constructors, induction_principle } => {
                ...
                for (param_name, param_type) in parameters {
                    ...
                    let param_ref = self.arena.borrow().alloc_type(param_data)?;
                ...
                for (ctor_name, ctor_type) in constructors {
                    ...
                    let ctor_ref = self.arena.borrow().alloc_type(ctor_data)?;
                ...
                let arena_induction = if let Some(ind_prin) = induction_principle {
                    ...
                    Some(self.arena.borrow().alloc_type(ind_data)?)
                } else {
                    None
                };
    ...
    fn convert_from_arena_type(&self, type_data: &DependentTypeData) -> Result<OptimizedDependentType> {
        match type_data {
    ...
    fn convert_to_arena_term(&self, term: &OptimizedDependentTerm) -> Result<DependentTermData> {
        match term {
            OptimizedDependentTerm::Variable(name) => {
                Ok(DependentTermData::Variable(name.clone()))
            }
            OptimizedDependentTerm::Lambda { param, param_type, body } => {
                ...
                let param_type_ref = self.arena.borrow().alloc_type(param_type_data)?;
                let body_ref = self.arena.borrow().alloc_term(body_data)?;
            ...
            OptimizedDependentTerm::Application { function, argument } => {
                ...
                let function_ref = self.arena.borrow().alloc_term(function_data)?;
                let argument_ref = self.arena.borrow().alloc_term(argument_data)?;
            ...
            OptimizedDependentTerm::Pair { first, second } => {
                ...
                let first_ref = self.arena.borrow().alloc_term(first_data)?;
                let second_ref = self.arena.borrow().alloc_term(second_data)?;
            ...
            OptimizedDependentTerm::Projection { pair, is_first } => {
                ...
                let pair_ref = self.arena.borrow().alloc_term(pair_data)?;
            ...
            OptimizedDependentTerm::Constructor { name, args, result_type } => {
                ...
                for arg in args {
                    ...
                    let arg_ref = self.arena.borrow().alloc_term(arg_data)?;
                    arena_args.push(arg_ref);
                }
                
                let result_type_data = self.convert_to_arena_type(result_type)?;
                let result_type_ref = self.arena.borrow().alloc_type(result_type_data)?;
            ...
            OptimizedDependentTerm::Match { scrutinee, branches, return_type } => {
                ...
                let scrutinee_ref = self.arena.borrow().alloc_term(scrutinee_data)?;
                ...
                let return_type_ref = self.arena.borrow().alloc_type(return_type_data)?;
    ...
    fn convert_from_arena_term(&self, term_data: &DependentTermData) -> Result<OptimizedDependentTerm> {
        match term_data {
    ...
    fn convert_to_arena_pattern(&self, pattern: &OptimizedPattern) -> Result<PatternData> {
        match pattern {
            OptimizedPattern::Variable(name) => {
                Ok(PatternData::Variable(name.clone()))
            }
    ...
    fn convert_from_arena_pattern(&self, pattern: &PatternData) -> Result<OptimizedPattern> {
        match pattern {
            PatternData::Variable(name) => {
                Ok(OptimizedPattern::Variable(name.clone()))
            }
...
impl OptimizedNormalizer {
    ...
    pub fn normalize_type(&self, ty: &OptimizedDependentType) -> Result<OptimizedDependentType> {
        ...
        let type_ref = self.arena.borrow().alloc_type(arena_type)?;
        
        // Check cache first
        if let Some(&cached_ref) = self.normalization_cache.borrow().get(&type_ref) {
            if self.arena.borrow().is_valid_type_ref(cached_ref) {
                let arena_borrow = self.arena.borrow();
        ...
        // Cache the result
        self.normalization_cache.borrow_mut().insert(type_ref, normalized_ref);
    ...
    pub fn normalize_term(&self, term: &OptimizedDependentTerm) -> Result<OptimizedDependentTerm> {
        ...
        let term_ref = self.arena.borrow().alloc_term(arena_term)?;
        
        // Check cache first
        if let Some(&cached_ref) = self.term_cache.borrow().get(&term_ref) {
            if self.arena.borrow().is_valid_term_ref(cached_ref) {
                let arena_borrow = self.arena.borrow();
        ...
        // Cache the result
        self.term_cache.borrow_mut().insert(term_ref, normalized_ref);
    ...
    fn substitute_term_ref(&self, term_ref: TermRef, _var: &str, replacement: TermRef) -> Result<TermRef> {
...
mod tests {
    ...
    fn test_optimized_variable_binding() {
        ...
        ctx.bind_variable("x".to_string(), universe_type.clone()).unwrap();
    ...
    fn test_arena_memory_efficiency() {
        ...
        for (i, ty) in types.iter().enumerate() {
            ...
            ctx_mut.bind_variable(format!("x{}", i), ty.clone()).unwrap();
        }
        
        let stats_final = ctx.arena().borrow().memory_stats();
        
        // Should have far fewer actual type allocations due to deduplication
        assert!(stats_final.types_count <= 5, "Deduplication should limit type count to ~5, got {}", stats_final.types_count);
    }
    ...
    fn test_complex_type_construction() {
        ...
        let mut ctx_mut = ctx.clone();
        ctx_mut.bind_variable("f".to_string(), pi_type.clone()).unwrap();
