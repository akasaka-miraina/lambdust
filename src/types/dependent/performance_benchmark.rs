...
use super::{
    arena::{DependentTermData, DependentTypeData, TermRef, TypeArena, TypeRef},
    core::{DependentTerm, DependentType, Normalizer, TypingContext},
    memory_pool::{AllocationType, MemoryPoolManager, MemoryPoolStatistics},
    optimized_core::{OptimizedDependentType, OptimizedNormalizer, OptimizedTypingContext},
};
...
impl DependentTypeBenchmarkSuite {
    ...
    fn benchmark_identical_type_sharing(&mut self) -> Result<()> {
        let result = self.time_benchmark("Identical Type Sharing", |tracker| {
            ...
            println!(
                "Deduplication: {} refs, {} actual types",
                refs.len(),
                stats.unwrap().types_count
            );
        })?;
    ...
    fn benchmark_structural_sharing(&mut self) -> Result<()> {
        let result = self.time_benchmark("Structural Sharing", |tracker| {
            ...
            let stats = arena.memory_stats();
            println!(
                "Structural sharing: {} Pi types, {} total arena types",
                pi_refs.len(),
                stats.unwrap().types_count
            );
        })?;
    ...
    fn benchmark_large_program_analysis(&mut self) -> Result<()> {
        let result = self.time_benchmark("Large Program Analysis", |tracker| {
            ...
            for i in 0..10000 {
                ...
                match type_complexity {
                    ...
                    _ => {
                        ...
                        let _ =
                            context.bind_variable(format!("complex{}", i), complex_type.clone());
                        let _normalized = normalizer.normalize_type(&complex_type).unwrap();
            ...
            println!(
                "Large program analysis: Context memory: {}KB, Normalizer memory: {}KB",
                stats.map(|s| s.total_memory() / 1024).unwrap_or(0),
...
pub struct BenchmarkSummary {
    ...
    /// Total time taken for all benchmarks
...
impl MemoryTracker {
    ...
    fn record_allocation(&mut self, estimated_size: usize) {
        ...
        let current_memory =
            self.snapshots.last().map(|s| s.memory_usage).unwrap_or(0) + estimated_size;
...
mod tests {
    ...
    fn test_simple_benchmark_run() {
        ...
        let result = suite
            .time_benchmark("Test Benchmark", |tracker| {
                for i in 0..100 {
