...
pub trait Substitution<Type: TypeRepr>: Send + Sync {
...
/// Proof term for dependent systems.
pub trait ProofTerm: Clone + Send + Sync {
    type Type: TypeRepr;
    ...
    /// Applies modus ponens (if available)
    fn modus_ponens(&self, other: &Self) -> UnifiedResult<Self>;
}
...
pub trait MonadStructure<Type: TypeRepr>: Send + Sync {
    ...
    fn natural_transform<Other: MonadStructure<Type>>(&self, other: &Other, m_type: &Type) 
        -> UnifiedResult<Type>;
}
...
impl TypeKind {
    ...
    pub fn arrows(args: Vec<TypeKind>, result: TypeKind) -> Self {
        args.into_iter().rev().fold(result, |acc, arg| Self::arrow(arg, acc))
    }
}
...
impl TypeSystemRegistry {
    ...
    fn supports_capabilities(&self, available: TypeSystemCapabilities, required: TypeSystemCapabilities) -> bool {
...
mod tests {
    ...
    impl TypeSystem for SimpleTypeSystem {
        ...
        fn new() -> UnifiedResult<Self> { Ok(SimpleTypeSystem) }
        fn system_name(&self) -> &'static str { "simple" }
        fn capabilities(&self) -> TypeSystemCapabilities { 
            TypeSystemCapabilities::hindley_milner() 
        }
        fn can_extend_with<Other: TypeSystem>(&self) -> bool { false }
    }
    ...
    fn test_capability_filtering() {
        ...
        let hm_systems = registry.find_systems_with_capabilities(
            TypeSystemCapabilities::hindley_milner()
        );
