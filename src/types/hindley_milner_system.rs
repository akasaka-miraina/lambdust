...
impl TypeRepr for HMType {
    ...
    fn compose_with(&self, other: &Self) -> UnifiedResult<Self> {
        ...
        Ok(HMType::Function(Box::new(other.clone()), Box::new(self.clone())))
    }
    
    fn unit(&self) -> UnifiedResult<Self> {
        // Identity function for this type
        Ok(HMType::Function(Box::new(self.clone()), Box::new(self.clone())))
    }
            ...
            HMType::Function(input, output) => {
                if self == input.as_ref() {
                    ...
                    Err(crate::diagnostics::UnifiedError::new(
                        TypeError,
                        format!("Cannot bind type {} with function type {}", 
                               self.display_type(), f_type.display_type())
                    ))
                }
            }
...
impl HMType {
    ...
    pub fn named_var(name: String, counter: &mut usize) -> Self {
        let id = *counter;
        *counter += 1;
        HMType::Variable(HMTypeVariable { id, name: Some(name) })
    }
}
...
impl Substitution<HMType> for HMSubstitution {
    fn apply(&self, ty: &HMType) -> HMType {
        match ty {
            ...
            HMType::List(elem) => {
                HMType::List(Box::new(self.apply(elem)))
            }
            HMType::Pair(first, second) => {
                HMType::Pair(
                    Box::new(self.apply(first)),
                    Box::new(self.apply(second))
                )
            }
            HMType::Forall(vars, body) => {
                // Don't substitute bound variables
                let filtered_mapping: HashMap<HMTypeVariable, HMType> = self.mapping
                    .iter()
                    .filter(|(var, _)| !vars.contains(var))
                    .map(|(k, v)| (k.clone(), v.clone()))
                    .collect();
                    
                let filtered_subst = HMSubstitution { mapping: filtered_mapping };
    ...
    fn compose(&self, other: &dyn Substitution<HMType>) -> Box<dyn Substitution<HMType>> {
        ...
        for (var, ty) in &other_hm.mapping {
            new_mapping.insert(var.clone(), self.apply(ty));
        }
        
        // Add mappings from self that aren't in other
        for (var, ty) in &self.mapping {
            if !new_mapping.contains_key(var) {
                new_mapping.insert(var.clone(), ty.clone());
            }
        }
    ...
    fn domain(&self) -> HashSet<TypeVariable> {
        self.mapping.keys().map(|var| TypeVariable::new(
            var.name.clone().unwrap_or_else(|| format!("t{}", var.id)),
            var.id,
        )).collect()
    }
...
impl HMSubstitution {
    ...
    pub fn empty() -> Self {
        HMSubstitution { mapping: HashMap::new() }
    }
...
impl ConstraintSystem<HMType> for HMConstraintSystem {
    ...
    fn add_constraint(&mut self, constraint: Self::Constraint) {
        if let Some(simplified) = constraint.simplify() {
        ...
        match (t1, t2) {
            ...
            (HMType::Variable(v), t) | (t, HMType::Variable(v)) => {
                if t.free_variables().contains(&TypeVariable::new(
                    ...
                    Err(crate::diagnostics::UnifiedError::new(
                        TypeError,
                        format!("Occurs check failed: {} occurs in {}", 
                               v.name.clone().unwrap_or_else(|| format!("t{}", v.id)),
                               t.display_type())
                    ))
                } else {
            ...
            _ => Err(crate::diagnostics::UnifiedError::new(
                TypeError,
                format!("Cannot unify {} with {}", t1.display_type(), t2.display_type())
            )),
        }
    }
    
    /// Compose two substitutions
    fn compose_substitutions(&self, s1: &HMSubstitution, s2: &HMSubstitution) -> HMSubstitution {
        let composed = s1.compose(s2);
        *composed.as_any().downcast_ref::<HMSubstitution>()
            .expect("Composition should return HMSubstitution")
    }
}
...
impl InferenceEngine<HMType, HMContext> for HMInferenceEngine {
    ...
    fn check(&self, expr: &impl ExpressionRepr, ty: &HMType, context: &HMContext) -> UnifiedResult<()> {
        ...
        constraints.add_constraint(HMConstraint::Unify(inferred_ty, ty.clone()));
...
mod tests {
    ...
    fn test_hindley_milner_creation() {
        ...
        assert_eq!(hm.system_name(), "hindley-milner");
        assert!(hm.capabilities().polymorphism);
        assert!(!hm.capabilities().dependent_types);
    }
