...
pub fn consistent(type1: &Type, type2: &Type) -> bool {
    match (type1, type2) {
        ...
        (Type::Pair(a1, b1), Type::Pair(a2, b2)) => consistent(a1, a2) && consistent(b1, b2),
        ...
        (Type::Forall { body: b1, .. }, Type::Forall { body: b2, .. }) => consistent(b1, b2),
...
pub fn join_types(type1: &Type, type2: &Type) -> Option<Type> {
    ...
    match (type1, type2) {
        ...
        (Type::Variable(v1), Type::Variable(v2)) if v1 == v2 => Some(Type::Variable(v1.clone())),
...
pub fn gradualize(type_: &Type) -> Type {
    match type_ {
        ...
        Type::Pair(a, b) => Type::pair(gradualize(a), gradualize(b)),
...
pub enum Cast {
    ...
    Upcast {
        /// Source type for the upcast
        from: Type,
        /// Target type for the upcast
        to: Type,
    },
    /// Downcast from dynamic to static (requires runtime check)
    Downcast {
        /// Source type for the downcast
        from: Type,
        /// Target type for the downcast
        to: Type,
    },
    /// Structural cast (for compound types)
    Structural {
        /// Sub-casts for components of the structural type
        casts: Vec<Cast>,
    },
}
...
pub fn is_gradual(type_: &Type) -> bool {
    match type_ {
        ...
        Type::Record(row) | Type::Variant(row) => row.fields.values().any(is_gradual),
...
pub fn approximate_type(type_: &Type) -> Type {
    match type_ {
        ...
        Type::Pair(a, b) => Type::pair(approximate_type(a), approximate_type(b)),
...
mod tests {
    ...
    fn test_join_types() {
        assert_eq!(
            join_types(&Type::Dynamic, &Type::Number),
            Some(Type::Dynamic)
        );
