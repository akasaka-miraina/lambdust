...
use super::{Constraint, Kind, Type, TypeScheme, TypeVar};
...
impl TypeClassInstance {
    ...
    pub fn new(class: impl Into<String>, instance_type: Type, span: Option<Span>) -> Self {
        Self {
            class: class.into(),
            instance_type,
            constraints: Vec::new(),
...
impl TypeClassEnv {
    ...
    pub fn resolve_constraint(
            ...
            for instance in instances {
                if let Some(subst) = instance.matches(constraint) {
                    ...
                    for instance_constraint in &instance.constraints {
                        ...
                        if self
                            .resolve_constraint(&resolved_constraint, context)
                            .is_err()
                        {
                            satisfied = false;
                            break;
        ...
        Err(Box::new(Error::type_error(
            format!(
                ...
                constraint.class, constraint.type_
            ),
            Span::new(0, 0),
        )))
    }
    ...
    fn add_builtin_classes(&mut self) {
        ...
        let eq_class = TypeClass::new("Eq", Kind::Type)
            .with_method(
                "==",
                TypeScheme::monomorphic(Type::forall(
        ...
        // Show type class
        let show_class = TypeClass::new("Show", Kind::Type).with_method(
            "show",
            TypeScheme::monomorphic(Type::forall(
                vec![TypeVar::with_name("a")],
        ...
        // Default type class
        let default_class = TypeClass::new("Default", Kind::Type).with_method(
            "default",
            TypeScheme::monomorphic(Type::forall(
                vec![TypeVar::with_name("a")],
                Type::named_var("a"),
...
mod tests {
    ...
    fn test_builtin_instances() {
        ...
        let number_eq_instance = eq_instances
            .iter()
