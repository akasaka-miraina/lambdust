#![allow(missing_docs)]
//! Feature Flag System for Phase 8 Gradual Optimization Deployment
//!
...
impl OptimizationFlags {
    ...
    pub fn record_success(&self, feature: OptimizationFeature, duration_micros: u64) {
        ...
        if let Some(feature_stats) = stats.get(&feature) {
            ...
            feature_stats
                .total_time_micros
                .fetch_add(duration_micros as usize, Ordering::Relaxed);
        }
    }
        ...
        if let Some(feature_stats) = stats.get(&feature) {
            ...
            if usage >= config.min_sample_size {
                ...
                if failure_rate > config.max_failure_rate {
                    ...
                    eprintln!(
                        "Auto-disabled feature {:?} due to high failure rate: {:.2}%",
                        feature,
                        failure_rate * 100.0
                    );
                }
...
static GLOBAL_OPTIMIZATION_FLAGS: std::sync::OnceLock<OptimizationFlags> =
        ...
        if $crate::feature::optimization_features::global_optimization_flags().is_enabled($feature)
        {
            ...
            match $optimized {
                Ok(result) => {
                    ...
                    $crate::feature::optimization_features::global_optimization_flags()
                        .record_success($feature, duration);
                    Ok(result)
                }
                Err(e) => {
                    $crate::feature::optimization_features::global_optimization_flags()
                        .record_fallback($feature);
                    $fallback
                }
            }
        } else {
...
mod tests {
    ...
    fn test_optimization_statistics() {
        ...
        let stats = flags
            .get_stats(OptimizationFeature::StringInterning)
            .unwrap();
