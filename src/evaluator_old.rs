...
impl Evaluator {
    ...
    fn process_import_spec(&mut self, import_spec: &Expr, env: Rc<Environment>) -> Result<()> {
        ...
        let srfi_import = parse_srfi_import(import_spec)?;

        // Get the SRFI module from registry
        let exports = if srfi_import.imports_all() {
            // Import all exports from the SRFI
            self.srfi_registry.get_exports(srfi_import.id)?
        } else {
            // Import specific parts
            let parts: Vec<&str> = srfi_import.parts.iter().map(|s| s.as_str()).collect();
            self.srfi_registry.get_exports_for_parts(srfi_import.id, &parts)?
        };
