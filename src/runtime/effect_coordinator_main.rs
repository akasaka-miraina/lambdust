...
impl EffectCoordinator {
    ...
    pub fn unregister_thread(&self, thread_id: ThreadId) {
        ...
        self.coordination_system
            .cleanup_thread_transactions(thread_id);
    }

    /// Enters an effect context for the given thread.
    pub fn enter_effect_context(
        &self,
        thread_id: ThreadId,
        effects: Vec<Effect>,
    ) -> Result<EffectContext, String> {
        let mut contexts = self.thread_contexts.write().unwrap();
        ...
        if let Some(state) = contexts.get_mut(&thread_id) {
            ...
            Err(format!(
                "Thread {thread_id:?} not registered with effect coordinator"
            ))
        }
    }

    /// Exits an effect context for the given thread.
    pub fn exit_effect_context(
        &self,
        thread_id: ThreadId,
        effects: Vec<Effect>,
    ) -> Result<EffectContext, String> {
    ...
    pub fn get_thread_context(&self, thread_id: ThreadId) -> Option<EffectContext> {
        let contexts = self.thread_contexts.try_read().unwrap();
    ...
    pub fn coordinate_cross_thread_effect(
        ...
        let dependencies = self
            .ordering_manager
            .compute_dependencies(&effect, sequence)?;

        // Send coordination message to target thread
        if let Some(channel) = self.get_coordination_channel(target_thread) {
            let message =
                super::effect_coordination_message::EffectCoordinationMessage::CoordinateEffect {
                    effect: effect.clone(),
                    sequence,
                    dependencies,
                };
            ...
            return Err(format!(
                "No coordination channel for thread {target_thread:?}"
            ));
        }
        ...
        match self
            .coordination_system
            .wait_for_coordination_completion(transaction_id, timeout)
        {
            Ok(true) => {
                self.coordination_system
    ...
    pub fn coordinate_local_effect(
        ...
        if !dependencies.is_empty() {
            self.ordering_manager
                .wait_for_dependencies(&dependencies, self.policies.coordination_timeout)?;
        }
    ...
    fn get_coordination_channel(&self, thread_id: ThreadId) -> Option<EffectChannel> {
        let channels = self.coordination_channels.try_read().unwrap();
    ...
    pub fn enable_effect_isolation(
        &self,
        thread_id: ThreadId,
        isolation_level: EffectIsolationLevel,
    ) -> Result<(), String> {
    ...
    pub fn can_effect_cross_threads(
        &self,
        effect: &Effect,
        source_thread: ThreadId,
        target_thread: ThreadId,
    ...
    pub fn create_effect_sandbox(
        &self,
        thread_id: ThreadId,
        sandbox_config: EffectSandboxConfig,
    ) -> Result<EffectSandboxHandle, String> {
