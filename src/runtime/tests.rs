...
mod tests {
    ...
    use crate::runtime::{EffectCoordinator, GlobalEnvironmentManager, LambdustRuntime};
    #[test]
    ...
    fn test_evaluator_handle() {
        ...
        let handle = runtime
            .spawn_evaluator()
            .expect("Failed to spawn evaluator");
    ...
    fn test_snapshot_and_rollback() {
        ...
        // Verify changes
        assert_eq!(
            global_env.lookup_global("var1"),
            Some(crate::eval::Value::integer(100))
        );
        assert_eq!(
            global_env.lookup_global("var3"),
            Some(crate::eval::Value::integer(3))
        );
        ...
        assert_eq!(
            global_env.lookup_global("var1"),
            Some(crate::eval::Value::integer(1))
        );
        assert_eq!(
            global_env.lookup_global("var2"),
            Some(crate::eval::Value::integer(2))
        );
    ...
    fn test_effect_isolation() {
        ...
        let isolation_result =
            coordinator.enable_effect_isolation(thread_id, EffectIsolationLevel::Complete);
        ...
        let sandbox_result =
            coordinator.create_effect_sandbox(thread_id, EffectSandboxConfig::default());
    ...
    fn test_effect_ordering_guarantees() {
        ...
        for &sequence in &sequences {
            let completion_result =
                coordinator.complete_effect(sequence, Ok("completed".to_string()));
            assert!(completion_result.is_ok());
        }
    ...
    fn test_integrated_runtime_features() {
        ...
        let transaction_id = global_env
            .start_transaction(thread_id)
    ...
    fn test_lambdust_lib_dir_environment_variable() {
        use crate::runtime::{LibraryPathConfig, LibraryPathResolver};
        use std::fs;
        ...
        fs::write(
            lib_dir.join("r7rs").join("base.scm"),
    ...
    fn test_library_path_fallback_behavior() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("LAMBDUST_LIB_DIR points to invalid directory")
        );
    }
