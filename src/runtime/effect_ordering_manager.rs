...
use super::ordering_constraint::{ConstraintType, OrderingConstraint};
...
impl EffectOrderingManager {
    ...
    pub fn compute_dependencies(&self, effect: &Effect, sequence: u64) -> Result<Vec<u64>, String> {
        let constraints = self.ordering_constraints.try_read().unwrap();
        let pending = self.pending_effects.try_read().unwrap();
