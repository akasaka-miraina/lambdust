...
//!
//! このモジュールはRuntime Executorのメイン実装を提供します。
...
impl RuntimeExecutor {
    ...
    pub fn get_llvm_optimization_level(&self, level: RuntimeOptimizationLevel) -> LLVMOptimizationLevel {
        match level {
    ...
    pub fn clear_jit_caches(&mut self) {
        // TODO: Implement clear_caches method in JitLoopOptimizer
    ...
    pub fn eval_with_execution_context(
        ...
        *self.recursion_depth_mut() += 1;
        ...
        self.send_statistics(&env, StatisticsMessage::EvaluationCompleted {
        ...
        if context.was_macro_expanded() {
            self.stats_mut().macro_expansions += context.macro_expansion_state.expanded_macros.len();
        }
        ...
        for (_name, _value) in &context.constant_bindings {
            ...
            self.stats_mut().constants_used += 1;
        ...
        self.stats_mut().estimated_time_savings_us += static_benefit.time_savings_micros;
        self.stats_mut().estimated_memory_savings_bytes += static_benefit.memory_savings_bytes;
        ...
        if should_use_jit {
            self.stats_mut().jit_compilations_triggered += 1;
        }
        
        if should_use_tail_call_opt {
            self.stats_mut().tail_call_optimizations_applied += 1;
        }
        
        if should_use_continuation_pooling {
            self.stats_mut().continuation_pooling_uses += 1;
        }
        
        if should_use_inline_eval {
            self.stats_mut().inline_evaluations += 1;
        }
        ...
        if complexity_score > 75 {
            self.stats_mut().hot_path_detections += 1;
        }
        ...
        let result = match runtime_level {
            RuntimeOptimizationLevel::None => {
                ...
                // Use mutable access to semantic evaluator for evaluation
                self.semantic_evaluator_mut().eval_pure(expr.clone(), env.clone(), cont.clone())
            }
        ...
        self.stats_mut().total_evaluation_time_us += eval_time.as_micros() as u64;
        ...
        *self.recursion_depth_mut() += 1;
        ...
        let result = if runtime_strategy.should_compile_jit() {
            ...
            match self.jit_compile_and_execute(&context, &opt_level) {
                Ok(jit_result) => {
                    self.stats_mut().jit_compilations += 1;
                    self.stats_mut().jit_execution_time_us += eval_start.elapsed().as_micros() as u64;
                    Ok(jit_result)
                },
                Err(_) => {
                    // JIT失敗時は最適化インタープリタにフォールバック
                    self.stats_mut().jit_fallbacks += 1;
        ...
        self.stats_mut().total_execution_time_us += execution_time.as_micros() as u64;
    ...
    pub fn interpret_with_optimizations(
        ...
        self.stats_mut().expressions_evaluated += 1;
        self.semantic_evaluator_mut().eval_pure(const_optimized_expr, env, optimized_cont)
    }
    ...
    fn apply_conservative_optimizations(
        ...
        if context.static_analysis.is_pure && 
           ...
           context.optimization_hints.use_inline_evaluation {
            ...
            self.stats_mut().inline_evaluation_opportunities += 1;
            self.stats_mut().inline_evaluations += 1;
        }
    ...
    fn apply_balanced_optimizations(
        ...
        if context.optimization_hints.use_tail_call_optimization {
            ...
            // }
            self.stats_mut().tail_calls_optimized += 1;
        }
        ...
        if context.optimization_hints.jit_beneficial && 
           context.static_analysis.complexity_score > 50 {
            ...
            if let Ok(_compiled_fn) = self.llvm_compiler_mut().compile_with_tail_calls(&expr, &tail_call_context) {
                ...
                self.stats_mut().llvm_optimizations_applied += 1;
    ...
    fn eval_with_conservative_optimizations(
        ...
        if let Some(result) = self.try_builtin_optimizations(&expr, &env, analysis)? {
            self.stats_mut().optimizations_applied += 1;
            return Ok(result);
        }
    ...
    fn eval_with_balanced_optimizations(
        ...
        if analysis.optimization_hints.contains(&OptimizationHint::PoolContinuations) {
            if let Some(result) = self.try_continuation_pooling(&expr, env.clone(), cont.clone(), analysis)? {
                self.stats_mut().continuation_pool_hits += 1;
                return Ok(result);
            }
        }
    ...
    fn try_continuation_pooling(
        ...
        if should_pool {
            ...
            if let Some(pooled_cont) = self.continuation_pooler().allocate(cont_type) {
                ...
                let result = self.semantic_evaluator().eval_pure_functional(expr.clone(), env, pooled_cont.clone(), self.recursion_depth + 1);
                ...
                if self.continuation_pooler().deallocate(pooled_cont) {
                    ...
                    self.stats_mut().pooling_memory_saved = memory_saved;
                } else {
                    self.stats_mut().continuation_pool_misses += 1;
                }
        ...
        match expr {
            Expr::List(_) => {
                ...
                let result = self.semantic_evaluator().eval_pure_functional(expr.clone(), env, cont.clone(), self.recursion_depth + 1);
    ...
    fn try_loop_optimization(
        ...
        match expr {
            Expr::List(_) => {
                ...
                Ok(Some(self.semantic_evaluator().eval_pure_functional(expr.clone(), env, cont, self.recursion_depth + 1)?))
            },
            _ => Ok(None),
        }
    }
        ...
        if let Ok(optimized_result) = result {
            ...
            if let Ok(semantic_result) = self.semantic_evaluator().eval_pure_functional(
                ...
                if !self.values_equivalent(optimized_result, &semantic_result) {
                    ...
                    self.stats_mut().verification_failures += 1;
                } else {
                    self.stats_mut().verification_successes += 1;
                }
            }
        }
    }
    ...
    pub fn reset_stats(&mut self) {
        *self.stats_mut() = RuntimeStats::default();
    }
