...
use crate::evaluator::{Continuation, RuntimeExecutor, RuntimeOptimizationLevel, SemanticEvaluator};
...
mod tests {
    ...
    fn test_benchmark_suite_creation() {
        let suite = PracticalBenchmarkSuite::new();
        assert!(suite.results_cache.is_empty());
    }

    #[test]
    fn test_simple_benchmark() {
        let mut suite = PracticalBenchmarkSuite::new();
        let result = suite.benchmark_expression("simple_add", "(+ 1 2 3)");
        assert!(result.is_ok());
        
        let result = result.unwrap();
        assert_eq!(result.name, "simple_add");
        assert!(result.results_equivalent);
    }
    ...
    fn test_performance_stats_calculation() {
        let results = vec![
            BenchmarkResult {
                ...
                semantic_time: Duration::from_millis(100),
                runtime_time: Duration::from_millis(50),
                speedup_factor: 2.0,
            ...
            BenchmarkResult {
                ...
                semantic_time: Duration::from_millis(200),
                runtime_time: Duration::from_millis(100),
                speedup_factor: 2.0,
        ...
        let suite = PracticalBenchmarkSuite::new();
        let stats = suite.calculate_overall_stats(&results);
        
        assert_eq!(stats.average_speedup, 2.0);
        assert_eq!(stats.median_speedup, 2.0);
        assert_eq!(stats.improved_count, 2);
        assert_eq!(stats.regression_count, 0);
    }
}