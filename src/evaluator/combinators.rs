...
impl CombinatorExpr {
    ...
    pub fn reduce_step(&self) -> Option<CombinatorExpr> {
        match self {
            ...
            CombinatorExpr::B | CombinatorExpr::C | CombinatorExpr::W |
    ...
    pub fn free_variables(&self) -> HashSet<String> {
        match self {
            CombinatorExpr::S | CombinatorExpr::K | CombinatorExpr::I |
            CombinatorExpr::B | CombinatorExpr::C | CombinatorExpr::W => HashSet::new(),
...
impl BracketAbstraction {
    ...
    pub fn lambda_to_combinators(expr: &Expr) -> Result<CombinatorExpr> {
        match expr {
            ...
            Expr::Variable(_) | Expr::Literal(_) => {
                Ok(CombinatorExpr::Atomic(expr.clone()))
            }
    ...
    fn bracket_abstract(var: &str, expr: CombinatorExpr) -> Result<CombinatorExpr> {
        match expr {
            // [x] x = I
            CombinatorExpr::Atomic(Expr::Variable(v)) if v == var => {
                Ok(CombinatorExpr::I)
            }
            
            // [x] E = K E (if x not free in E)
            expr if !expr.free_variables().contains(var) => {
...
mod tests {
    ...
    fn test_basic_combinators() {
        ...
        assert_eq!(reduced, CombinatorExpr::Atomic(Expr::Variable("x".to_string())));
    }
    ...
    fn test_bracket_abstraction_constant() {
        ...
        let const_expr = CombinatorExpr::Atomic(Expr::Literal(Literal::Number(SchemeNumber::Integer(42))));
    ...
    fn test_lambda_to_combinators_constant() {
        ...
        let expected = CombinatorExpr::App(
            Box::new(CombinatorExpr::K),
            Box::new(CombinatorExpr::Atomic(Expr::Literal(Literal::Number(SchemeNumber::Integer(42))))),
        );
