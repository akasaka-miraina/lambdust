...
mod tests {
    ...
    use crate::ast::{Expr, Literal, Formals, Binding};
    ...
    mod syntax_object_tests {
        ...
        fn test_syntax_object_properties() {
            ...
            syntax.set_property("type".to_string(), SyntaxProperty::String("identifier".to_string()));
        ...
        fn test_free_identifier_equal() {
            ...
            let mut stx1 = SyntaxObject::new(Expr::Identifier("x".to_string()), span, context.clone());
            let mut stx2 = SyntaxObject::new(Expr::Identifier("x".to_string()), span, context);
    ...
    mod syntax_case_tests {
        ...
        fn test_template_expansion() {
            ...
            if let Expr::List(elements) = &result.expr {
                ...
                assert!(matches!(elements[0].inner, Expr::Identifier(ref name) if name == "lambda"));
            }
        }
        ...
        fn test_ellipsis_template_expansion() {
            ...
            let ellipsis_syntaxes = vec![
                SyntaxObject::new(Expr::Identifier("a".to_string()), test_span(), context.clone()),
                SyntaxObject::new(Expr::Identifier("b".to_string()), test_span(), context.clone()),
                SyntaxObject::new(Expr::Identifier("c".to_string()), test_span(), context.clone()),
            ];
        ...
        fn test_syntax_case_procedures() {
            ...
            let non_id = SyntaxObject::new(Expr::Literal(Literal::Number(42.0)), test_span(), context);
    ...
    mod quasisyntax_tests {
        ...
        fn test_quasisyntax_parser() {
            ...
            let expr = Expr::Identifier("test".to_string());
            let template = parser.parse(&expr).unwrap();
            assert!(matches!(template, QuasisyntaxTemplate::Identifier(ref name) if name == "test"));
        }
        ...
        fn test_quasisyntax_expansion() {
            ...
            let result = template.expand(&mut expansion_context, test_span()).unwrap();
        ...
        fn test_unquote_handling() {
            ...
            let result = unquote_template.expand(&mut expansion_context, test_span()).unwrap();
        ...
        fn test_unquote_splicing() {
            ...
            let result = splice_template.expand(&mut expansion_context, test_span()).unwrap();
    ...
    mod scope_management_tests {
        ...
        fn test_binding_lookup_hierarchy() {
            ...
            let _child_id = manager.push_scope(ScopeType::Function, vec!["test".to_string()]).unwrap();
        ...
        fn test_lambda_scope_creation() {
            ...
            let _lambda_scope = scope_utils::create_lambda_scope(
                &mut manager,
                &formals,
                vec!["test".to_string()],
            ).unwrap();
    ...
    mod integration_tests {
        ...
        fn test_hygiene_application() {
            ...
            let _macro_context = expander.enter_macro_expansion(vec!["test".to_string()]).unwrap();
    ...
    mod property_based_tests {
        ...
        fn prop_mark_set_associativity() {
            ...
            for (marks1, marks2, marks3) in test_cases {
                let set1 = MarkSet::from(marks1.into_iter().collect::<std::collections::HashSet<_>>());
                let set2 = MarkSet::from(marks2.into_iter().collect::<std::collections::HashSet<_>>());
                let set3 = MarkSet::from(marks3.into_iter().collect::<std::collections::HashSet<_>>());
        ...
        fn prop_scope_hierarchy_transitivity() {
            for depth in 1..=5 {
                ...
                // Create hierarchy
                for i in 0..depth {
                    let scope_id = manager.push_scope(
                        ScopeType::Block,
                        vec![format!("level{}", i)],
                    ).unwrap();
