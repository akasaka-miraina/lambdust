...
impl Template {
    ...
    pub fn expand(
        ...
        match self {
            ...
            Template::Literal(lit) => Ok(Spanned::new(Expr::Literal(lit.clone()), span)),

            Template::Identifier(name) => Ok(Spanned::new(Expr::Identifier(name.clone()), span)),

            Template::Keyword(name) => Ok(Spanned::new(Expr::Keyword(name.clone()), span)),

            Template::Nil => Ok(Spanned::new(Expr::Literal(Literal::Nil), span)),
    ...
    fn expand_ellipsis(
        ...
        if expanded_items.is_empty() {
            ...
            Ok(Spanned::new(Expr::List(expanded_items), span))
        }
    }
    ...
    fn expand_nested_ellipsis(
        ...
        if ellipsis_vars.is_empty() {
            return Err(Box::new(Error::macro_error(
                "SRFI-149 error: nested ellipsis template contains no ellipsis variables"
                    .to_string(),
                span,
            )));
        }
        ...
        if !any_bindings_found {
            return Err(Box::new(Error::macro_error(
                format!(
                    "SRFI-149 error: no ellipsis bindings found for variables: {ellipsis_vars:?}"
                ),
                span,
            )));
        }
    ...
    fn expand_extra_ellipsis(
        ...
        if extra_depth > 5 {
            return Err(Box::new(Error::macro_error(
                format!(
                    "SRFI-149 error: extra ellipsis depth {extra_depth} exceeds reasonable maximum (5)"
                ),
                span,
            )));
        }
        ...
        for level in 0..extra_depth {
            result = Spanned::new(Expr::List(vec![result]), span);

            // Optional: Add debugging information for deep nesting
            if level > 3 {
                eprintln!(
                    "SRFI-149 warning: creating deep nesting at level {}",
                    level + 1
                );
            }
        }
    ...
    pub fn ellipsis_depth(&self) -> usize {
        match self {
            ...
            Template::List(templates) => templates
                .iter()
                .map(|t| t.ellipsis_depth())
                .max()
                .unwrap_or(0),
            Template::Pair { car, cdr } => car.ellipsis_depth().max(cdr.ellipsis_depth()),
    ...
    fn resolve_ambiguities_inner(
        &mut self,
        pattern_vars: &std::collections::HashMap<String, usize>,
        current_depth: usize,
    ) {
        match self {
            Template::Variable(name) => {
