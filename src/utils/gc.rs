...
impl GenerationalGc {
    ...
    pub fn maybe_collect(&self) {
        if let Ok(sizes) = self.generation_sizes.try_read() {
    ...
    pub fn collect_generation(&self, generation: GenerationId) {
        ...
        let objects_before =
            if let Ok(generation_objects) = self.generations[generation as usize].try_read() {
        ...
        let memory_in_use = if let Ok(sizes) = self.generation_sizes.try_read() {
            sizes.iter().sum()
        } else {
            0
        };
    ...
    fn mark_from_roots(&self) {
        if let (Ok(roots), Ok(objects)) = (self.roots.try_read(), self.objects.try_read()) {
    ...
    fn mark_from_older_generations(&self, generation: GenerationId) {
        if let Ok(objects) = self.objects.try_read() {
            for gen_idx in (generation + 1)..=MAX_GENERATIONS {
                if let Ok(gen_objects) = self.generations[gen_idx as usize].try_read() {
    ...
    fn sweep_generation(&self, generation: GenerationId) -> (usize, usize, usize) {
        ...
        if let (Ok(mut gen_objects), Ok(objects)) = (
            self.generations[generation as usize].write(),
            self.objects.try_read(),
        ) {
            for &obj_id in gen_objects.iter() {
                if let Some(obj) = objects.get(&obj_id) {
                    if !obj.inner.is_marked() {
                        ...
                        if generation < MAX_GENERATIONS
                            && objects_promoted < self.config.max_promotions
                        {
        ...
        if generation < MAX_GENERATIONS && !to_promote.is_empty() {
            if let (Ok(mut next_gen), Ok(mut objects)) = (
                self.generations[(generation + 1) as usize].write(),
                self.objects.write(),
            ) {
                for obj_id in to_promote {
    ...
    pub fn debug_info(&self) -> GcDebugInfo {
        ...
        for generation in &self.generations {
            if let Ok(gen_objects) = generation.try_read() {
        ...
        let root_count = if let Ok(roots) = self.roots.try_read() {
            roots.len()
        } else {
            0
        };

        let weak_ref_count = if let Ok(weak_refs) = self.weak_refs.try_read() {
...
impl WeakGcPtr {
    ...
    pub fn upgrade(&self) -> Option<GcPtr> {
        self.inner
            .upgrade()
            .map(|inner| GcPtr { inner, id: self.id })
    }
