...
impl MemoryPool {
    ...
    pub fn deallocate(&self, ptr: NonNull<u8>) {
        ...
        loop {
            ...
            if self
                .free_list
                .compare_exchange_weak(current_head, block, Ordering::Release, Ordering::Relaxed)
                .is_ok()
            {
                break;
            }
    ...
    fn grow_pool(&self, count: usize) -> Result<(), String> {
        ...
        for i in 0..count {
            let object_ptr = unsafe { chunk_ptr.add(i * self.object_size) as *mut FreeBlock };
...
pub fn global_pool_manager() -> &'static PoolManager {
    GLOBAL_POOL_MANAGER.get_or_init(|| PoolManager::new(PoolConfig::default()))
}
