...
impl Profiler {
    ...
    fn update_category_stats(&self, entry: &ProfileEntry) {
        if let Ok(mut stats) = self.category_stats.write() {
            ...
            category_stats.average_duration =
                category_stats.total_duration / category_stats.operation_count as u32;
            ...
            category_stats.net_memory_change +=
                entry.memory_allocated as i64 - entry.memory_freed as i64;
            ...
            if entry.start_time >= recent_cutoff {
                ...
                if elapsed_secs > 0.0 {
                    category_stats.ops_per_second =
                        category_stats.operation_count as f64 / elapsed_secs;
                }
            }
        }
    }
    ...
    fn update_system_metrics(&self, entry: &ProfileEntry) {
        if let Ok(mut metrics) = self.system_metrics.write() {
            ...
            metrics.current_memory_usage =
                metrics.current_memory_usage.saturating_add(current_memory);
            ...
            if let Some(interner_stats) = self.get_string_interner_stats() {
                ...
                if total_symbols > 0 {
                    metrics.string_interning_hit_rate =
                        (common_symbols as f64 / total_symbols as f64) * 100.0;
                }
            }
        }
    }
    ...
    pub fn get_category_stats(&self) -> HashMap<ProfileCategory, CategoryStats> {
        if let Ok(stats) = self.category_stats.try_read() {
            stats.clone()
        } else {
            HashMap::new()
        }
    ...
    pub fn get_system_metrics(&self) -> SystemMetrics {
        if let Ok(metrics) = self.system_metrics.try_read() {
            metrics.clone()
        } else {
            SystemMetrics {
                total_cpu_time: Duration::ZERO,
    ...
    fn generate_memory_recommendations(&self) -> Vec<String> {
        ...
        if metrics.memory_pool_efficiency < 0.5 {
            recommendations
                .push("Consider tuning memory pool sizes for better efficiency".to_string());
        }

        if metrics.string_interning_hit_rate < 70.0 {
            recommendations.push(
                "String interning hit rate is low, consider pre-interning more common symbols"
                    .to_string(),
            );
        }
    ...
    fn generate_optimization_suggestions(&self) -> Vec<String> {
        ...
        if metrics.fast_path_hit_rate < 80.0 {
            suggestions.push(
                "Fast path hit rate is low, consider optimizing more common operations".to_string(),
            );
        }
        ...
        for stats in category_stats.values() {
            if stats.average_duration > Duration::from_millis(10) {
                suggestions.push(format!(
                    "Category {:?} has high average duration, consider optimization",
                    stats.category
                ));
            }
        }
        ...
        if metrics.gc_time > metrics.total_cpu_time / 10 {
            suggestions.push(
                "Garbage collection overhead is high, consider tuning GC parameters".to_string(),
            );
        }
...
impl PerformanceReport {
    ...
    pub fn to_json(&self) -> Result<String, Box<dyn std::error::Error>> {
        ...
        Ok(format!(
            "{{\"total_operations\": {}, \"average_duration_ns\": {}}}",
            self.total_operations,
            self.average_op_duration.as_nanos()
        ))
    }
}

/// Global profiler instance.
static GLOBAL_PROFILER: Lazy<Profiler> = Lazy::new(|| Profiler::new(ProfilerConfig::default()));
...
mod tests {
    ...
    fn test_category_stats() {
        ...
        for i in 0..5 {
            let _session =
                profiler.start_profile(ProfileCategory::Parsing, format!("operation_{i}").as_str());
            thread::sleep(Duration::from_millis(1));
        }
    ...
    fn test_memory_tracking() {
        ...
        {
            let mut session =
                profiler.start_profile(ProfileCategory::MemoryAllocation, "alloc_test");
