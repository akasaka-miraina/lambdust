    ...
    pub fn execute(
        ...
        loop {
            if ip >= bytecode.instructions.len() {
                self.state = VmState::Error(Error::runtime_error(
        ...
        match instruction.opcode {
            OpCode::LoadConst => {
                if let Operand::ConstIndex(index) = instruction.operand {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "LoadConst requires constant index operand".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::LoadLocal => {
                if let Operand::LocalIndex(index) = instruction.operand {
                    if let Some(frame) = self.call_stack.last() {
                        ...
                        Err(Box::new(Error::runtime_error(
                            "No call frame for local access".to_string(),
                            None,
                        )))
                    }
                } else {
                    Err(Box::new(Error::runtime_error(
                        "LoadLocal requires local index operand".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::LoadGlobal => {
                if let Operand::Symbol(symbol) = instruction.operand {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "LoadGlobal requires symbol operand".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Add => {
                if self.stack.len() >= 2 {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "Addition requires 2 operands on stack".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Sub => {
                if self.stack.len() >= 2 {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "Subtraction requires 2 operands on stack".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Mul => {
                if self.stack.len() >= 2 {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "Multiplication requires 2 operands on stack".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Cons => {
                if self.stack.len() >= 2 {
                    ...
                    Err(Box::new(Error::runtime_error(
                        "Cons requires 2 operands on stack".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Car => {
                if let Some(pair) = self.stack.pop() {
                    match pair {
                        ...
                        _ => Err(Box::new(Error::runtime_error(
                            "Car requires pair operand".to_string(),
                            None,
                        ))),
                    }
                } else {
                    Err(Box::new(Error::runtime_error(
                        "Car requires 1 operand on stack".to_string(),
                        None,
                    )))
                }
            }
            ...
            OpCode::Cdr => {
                if let Some(pair) = self.stack.pop() {
                    match pair {
                        ...
                        _ => Err(Box::new(Error::runtime_error(
                            "Cdr requires pair operand".to_string(),
                            None,
                        ))),
                    }
                } else {
                    Err(Box::new(Error::runtime_error(
                        "Cdr requires 1 operand on stack".to_string(),
                        None,
                    )))
                }
            }
...
mod tests {
    ...
    fn test_load_const_execution() {
        ...
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::LoadConst,
            Operand::ConstIndex(const_index),
        ));
    ...
    fn test_arithmetic_execution() {
        ...
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::LoadConst,
            Operand::ConstIndex(const1),
        ));
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::LoadConst,
            Operand::ConstIndex(const2),
        ));
