...
use super::{
    BytecodeCompiler, BytecodeOptimizer, BytecodePerformanceStats, CompilationResult,
    CompilerOptions, CompilerStats, ExecutionResult, OptimizationStats, OverallPerformanceMetrics,
    VirtualMachine, VmStats,
};
...
impl BytecodeEngine {
    ...
    pub fn compile_and_execute(&mut self, program: &Program) -> Result<Value> {
        ...
        let execution_result = self
            .vm
            .execute(&optimized_bytecode, &compilation_result.constant_pool)?;

        match execution_result {
            ExecutionResult::Value(value) => Ok(value),
            ExecutionResult::Error(error) => Err(error.boxed()),
            ExecutionResult::Yield(_) => Err(Box::new(Error::runtime_error(
                "Unexpected yield in top-level execution",
                None,
            ))),
        }
    }
    ...
    pub fn get_performance_stats(&self) -> BytecodePerformanceStats {
        ...
        let instructions_per_second = if vm_stats.execution_time_us > 0 {
            (vm_stats.instructions_executed as f64)
                / (vm_stats.execution_time_us as f64 / 1_000_000.0)
        } else {
            0.0
        };
        ...
        let optimization_effectiveness = if optimizer_stats.instructions_before > 0 {
            1.0 - (optimizer_stats.instructions_after as f64
                / optimizer_stats.instructions_before as f64)
        } else {
            0.0
        };
        ...
        let speedup_factor = if vm_stats.optimized_operations > 0 {
            (vm_stats.optimized_operations as f64 / vm_stats.instructions_executed as f64) * 3.0
                + 1.0
        } else {
            1.0
        };
