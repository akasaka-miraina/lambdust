...
use super::instruction::{Bytecode, Instruction, OpCode, Operand};
    ...
    pub fn optimize(&mut self, mut bytecode: Bytecode) -> Result<Bytecode> {
        ...
        while changed && pass_count < self.config.max_passes {
            ...
            if self.config.dead_code_elimination
                && self.apply_dead_code_elimination(&mut bytecode)?
            {
                changed = true;
            }

            if self.config.instruction_combining
                && self.apply_instruction_combining(&mut bytecode)?
            {
                changed = true;
            }

            if self.config.tail_call_optimization
                && self.apply_tail_call_optimization(&mut bytecode)?
            {
                changed = true;
            }
        ...
        self.stats.instructions_eliminated =
            initial_instruction_count.saturating_sub(bytecode.instructions.len());
            ...
            if i + 2 < bytecode.instructions.len() {
                if let (
                    Instruction {
                        ...
                        operand: Operand::ConstIndex(idx1),
                        ..
                    },
                    Instruction {
                        opcode: OpCode::LoadConst,
                        operand: Operand::ConstIndex(idx2),
                        ..
                    },
                    Instruction {
                        opcode: OpCode::Add,
                    ...
                    if let (Some(const1), Some(const2)) = (
                        ...
                        if let (Some(n1), Some(n2)) = (
                            self.constant_to_number(const1),
                            self.constant_to_number(const2),
                        ) {
                            // Fold the addition
                            let result = n1 + n2;
                            ...
                            new_instructions.push(Instruction::with_operand(
                                OpCode::LoadConst,
                                Operand::ConstIndex(result_idx),
                            ));
    ...
    fn apply_dead_code_elimination(&self, bytecode: &mut Bytecode) -> Result<bool> {
        ...
        while let Some(index) = worklist.pop() {
            ...
            match &bytecode.instructions[index] {
                Instruction {
                    opcode: OpCode::Jump,
                    operand: Operand::JumpOffset(offset),
                    ..
                } => {
                ...
                Instruction {
                    opcode: OpCode::Return | OpCode::Halt,
                    ..
                } => {
    ...
    fn apply_tail_call_optimization(&self, bytecode: &mut Bytecode) -> Result<bool> {
        ...
        for i in 0..bytecode.instructions.len().saturating_sub(1) {
            ...
            ) = (&bytecode.instructions[i], &bytecode.instructions[i + 1])
            {
                ...
                bytecode.instructions[i] =
                    Instruction::with_operand(OpCode::TailCall, operand.clone());
    ...
    pub fn generate_report(&self) -> String {
        ...
        if stats.instructions_before > 0 {
            let reduction_percent =
                (stats.instructions_eliminated as f64 / stats.instructions_before as f64) * 100.0;
        ...
        if stats.instructions_eliminated == 0 {
            report.push_str(
                "• No optimizations applied - consider enabling more optimization passes\n",
            );
        }
        if stats.optimization_time_us > 10000 {
            report.push_str(
                "• High optimization time - consider reducing max_passes for faster compilation\n",
            );
        }
...
mod tests {
    ...
    use crate::bytecode::instruction::{
        Bytecode, ConstantPool, ConstantValue, Instruction, OpCode, Operand,
    };
    ...
    fn test_constant_folding() {
        ...
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::LoadConst,
