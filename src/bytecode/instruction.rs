...
impl ConstantPool {
    ...
    pub fn memory_usage(&self) -> usize {
        std::mem::size_of::<Self>()
            ...
            + self.constant_map.capacity()
                * (std::mem::size_of::<ConstantValue>() + std::mem::size_of::<u32>())
    }
}
        ...
        for (index, instruction) in self.instructions.iter().enumerate() {
            if let Operand::ConstIndex(const_index) = &instruction.operand {
                if *const_index >= self.constants.len() as u32 {
                    return Err(format!(
                        "Instruction {index} references invalid constant {const_index}"
                    ));
                }
            }
        }
                ...
                OpCode::Jump | OpCode::JumpIfFalse | OpCode::JumpIfTrue => {
                    if let Operand::JumpOffset(offset) = &instruction.operand {
                        ...
                        if target < 0 || target >= self.instructions.len() as i32 {
                            return Err(format!(
                                "Instruction {index} has invalid jump target {target}"
                            ));
                        }
                    }
                }
