...
use crate::ast::{Binding, Expr, Formals, Literal, Program};
...
impl BytecodeCompiler {
    ...
    pub fn compile_program(&mut self, program: &Program) -> Result<CompilationResult> {
        ...
        for expr in &program.expressions {
            self.compile_expression_internal(
                &expr.inner,
                &mut bytecode,
                &mut constant_pool,
                &mut context,
    ...
    fn compile_expression_internal(
        ...
        match expr {
            ...
            Expr::Quote(quoted) => self.compile_quote(quoted, bytecode, constant_pool, context),
    ...
    fn compile_literal(
        ...
        let constant_value = match literal {
            ...
            Literal::Complex(complex) => {
                ...
                if complex.imaginary != 0.0 {
                    return Err(Box::new(Error::compilation_error(
                        "Complex literals not yet fully supported in bytecode".to_string(),
                    )));
                }
            ...
            Literal::Bytevector(_bytes) => {
                ...
                return Err(Box::new(Error::compilation_error(
                    "Bytevector literals not yet supported in bytecode".to_string(),
                )));
            }
            Literal::Nil => {
    ...
    fn compile_symbol_reference(
        ...
        if let Some(local) = self.find_local(symbol, context) {
            bytecode.add_instruction(Instruction::with_operand(
                OpCode::LoadLocal,
                Operand::LocalIndex(local.index),
            ));
        } else {
    ...
    fn compile_application(
        ...
        if let Expr::Symbol(symbol) = &operator.inner {
            ...
            if let Some(builtin_op) = self.get_builtin_operation(symbol_id) {
                return self.compile_builtin_operation(
                    builtin_op,
                    operands,
                    bytecode,
                    constant_pool,
    ...
    fn compile_define(
        ...
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::StoreGlobal,
            Operand::Symbol(SymbolId::new(symbol_id)),
        ));
        ...
        bytecode.add_instruction(Instruction::with_operand(
            OpCode::LoadConst,
            Operand::ConstIndex(unspec_const),
        ));
        self.push_stack(context);
    ...
    fn compile_let(
        ...
        for binding in bindings {
            ...
            bytecode.add_instruction(Instruction::with_operand(
                OpCode::StoreLocal,
                Operand::LocalIndex(local_index),
            ));
    ...
    fn compile_letrec(
        ...
        for binding in bindings {
            ...
            if let Some(local) = self.find_local_by_name(&binding.name, context) {
                ...
                return Err(Box::new(Error::compilation_error(format!(
                    "Internal error: letrec binding not found: {}",
                    binding.name
                ))));
            }
        }
    ...
    fn compile_quote(
        ...
        match quoted {
            ...
            _ => Err(Box::new(Error::compilation_error(
                "Complex quoted expressions not yet supported in bytecode".to_string(),
            ))),
        }
    }
    ...
    fn exit_scope(&self, context: &mut CompilerContext) {
        ...
        context
            .locals
            .retain(|local| local.scope_depth <= context.scope_depth);
    }
