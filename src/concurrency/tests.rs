...
async fn test_rwlock() {
    ...
    for _ in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            let guard = rwlock_clone.try_read().unwrap();
...
async fn bench_channel_throughput() {
    ...
    println!(
        "Messages per second: {}",
        num_messages as f64 / consumer_time.as_secs_f64()
    );
}
...
async fn bench_future_creation() {
    ...
    println!(
        "Futures per second (creation): {}",
        num_futures as f64 / creation_time.as_secs_f64()
    );
    println!(
        "Futures per second (await): {}",
        num_futures as f64 / await_time.as_secs_f64()
    );
}
