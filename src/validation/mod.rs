...
pub use benchmark_automation::{BenchmarkConfig, BenchmarkReport, BenchmarkRunner};
pub use correctness_validation::{CorrectnessValidator, TestSuite, ValidationError};
pub use performance_validation::{PerformanceThreshold, PerformanceValidator, ValidationResult};
pub use regression_detection::{Baseline, PerformanceRegression, RegressionDetector};
...
impl ValidationPipeline {
    ...
    pub fn run_full_validation(&mut self) -> ValidationPipelineResult {
        ...
        if let Err(e) = correctness_result {
            results
                .errors
                .push(format!("Correctness validation failed: {:?}", e));
        }
        ...
        match self.benchmark_runner.run_all_benchmarks() {
            ...
            Err(e) => {
                results
                    .errors
                    .push(format!("Benchmark execution failed: {:?}", e));
            }
        }
        ...
        if let Some(ref report) = results.benchmark_report {
            ...
            if let Err(e) = perf_result {
                results
                    .errors
                    .push(format!("Performance validation failed: {:?}", e));
            }
        }
        ...
        if let Some(ref report) = results.benchmark_report {
            match self.regression_detector.check_for_regressions(report) {
                ...
                Err(e) => {
                    results
                        .errors
                        .push(format!("Regression detection failed: {:?}", e));
                }
            }
        }
            ...
            if flags.is_enabled(feature) {
                ...
                if let Some(stats) = all_stats.get(&feature) {
                    ...
                    if success_rate >= 0.95 && stats.usage_count >= 100 {
                        ...
                        || (stats.fallback_count > 0 && stats.usage_count > 50)
                    {
    ...
    pub fn run_quick_validation(&mut self) -> ValidationPipelineResult {
        ...
        match self.benchmark_runner.run_quick_benchmarks() {
            ...
            Err(e) => {
                results
                    .errors
                    .push(format!("Quick benchmark failed: {:?}", e));
            }
        }
...
impl ValidationPipelineResult {
    ...
    pub fn summary(&self) -> String {
        ...
        summary.push_str(&format!(
            "Features: {}/{} enabled, {} performing well\n",
...
pub struct FeatureAnalysis {
    ...
    pub problematic_features: Vec<(
        OptimizationFeature,
        crate::feature::optimization_features::FeatureStatsSnapshot,
    )>,
    pub top_performers: Vec<(
        OptimizationFeature,
        crate::feature::optimization_features::FeatureStatsSnapshot,
    )>,
}
...
static GLOBAL_VALIDATION_PIPELINE: std::sync::OnceLock<std::sync::Mutex<ValidationPipeline>> =
