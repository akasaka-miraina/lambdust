...
impl Hash for ComplexLiteral {
    fn hash<H: Hasher>(&self, state: &mut H) {
        // Hash the bit representations of f64 values to handle NaN and signed zero correctly
...
mod tests {
    ...
    fn test_display() {
        ...
        assert_eq!(
            format!("{}", Literal::String(Box::new("hello".to_string()))),
            "\"hello\""
        );
        ...
        assert_eq!(
            format!("{}", Literal::InexactReal(f64::NEG_INFINITY)),
            "-inf.0"
        );
    ...
    fn test_smart_string_heuristics() {
        ...
        let long = Literal::smart_string(
            "this is a very long and uncommon string that should not be interned",
        );
