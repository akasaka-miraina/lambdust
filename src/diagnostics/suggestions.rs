...
impl SuggestionGenerator {
    ...
    fn load_common_patterns(&mut self) {
        ...
        self.function_suggestions.extend([
            ...
            (
                "size".to_string(),
                vec!["length".to_string(), "vector-length".to_string()],
            ),
            (
                "append".to_string(),
                vec!["string-append".to_string(), "vector-append".to_string()],
            ),
            (
                "map".to_string(),
                vec!["map".to_string(), "for-each".to_string()],
            ),
        ]);
...
pub fn generate_suggestions_for_error(error: &Error) -> Vec<Suggestion> {
    ...
    match error {
        ...
        Error::RuntimeError { message, .. } => generator.suggest_for_runtime_error(message),
        Error::TypeError { .. } => generator.suggest_for_type_error(&None, &None),
        _ => Vec::new(),
    }
}
...
mod tests {
    ...
    fn test_suggestion_with_replacement() {
        ...
        let suggestion =
            Suggestion::with_replacement("Fix typo", "correct", span, SuggestionCategory::Syntax);
    ...
    fn test_suggestion_generator_type_errors() {
        ...
        let suggestions = generator
            .suggest_for_type_error(&Some("number".to_string()), &Some("string".to_string()));
    ...
    fn test_custom_corrections() {
        ...
        let custom_suggestion = suggestions.iter().find(|s| s.message.contains("custom"));
