...
use super::{NumericValue, constants, functions, tower};
...
pub fn value_to_numeric(value: &Value) -> Result<NumericValue> {
    match value {
        Value::Literal(literal) => NumericValue::from_literal(literal)
...
pub fn extract_numeric_args(args: &[Value]) -> Result<Vec<NumericValue>> {
    args.iter().map(value_to_numeric).collect()
}
...
pub fn extract_n_numeric_args<const N: usize>(args: &[Value]) -> Result<[NumericValue; N]> {
    if args.len() != N {
        return Err(Box::new(Error::arity_error(
            "extract_n_numeric_args",
            N,
            args.len(),
        )));
    }
    ...
    numeric_args.try_into().map_err(|_| {
...
pub fn primitive_subtract(args: &[Value]) -> Result<Value> {
    ...
    let result = if args.len() == 1 {
        ...
        numeric_args
            .iter()
            .skip(1)
            .fold(numeric_args[0].clone(), |acc, arg| {
                tower::subtract(&acc, arg)
            })
    };
...
pub fn primitive_numeric_equal(args: &[Value]) -> Result<Value> {
    ...
    let all_equal = numeric_args
        .iter()
        .skip(1)
        .all(|arg| matches!(tower::compare(first, arg), Some(std::cmp::Ordering::Equal)));
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Cannot compute exp of this type",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Complex(c) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Cannot compute log of this type",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Cannot compute sin of this type",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Cannot compute cos of this type",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Cannot compute tan of this type",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Gamma function requires real argument",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Error function requires real argument",
                    Span::default(),
                )));
            }
        }
    ...
    let result = match arg {
        ...
        _ => {
            ...
            if let NumericValue::Real(r) = promoted {
                ...
                return Err(Box::new(Error::type_error(
                    "Bessel function requires real argument",
                    Span::default(),
                )));
            }
        }
...
pub fn primitive_list_constants(args: &[Value]) -> Result<Value> {
    ...
    let values: Vec<Value> = constant_names.into_iter().map(Value::string).collect();
