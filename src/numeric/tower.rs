...
pub fn promote_to_bigint(value: &NumericValue) -> NumericValue {
    match value {
        ...
        NumericValue::Complex(c)
            if c.imaginary == 0.0 && c.real.fract() == 0.0 && c.real.is_finite() =>
        {
            NumericValue::BigInteger(BigInt::from_i64(c.real as i64))
        }
        NumericValue::Vector(v) => NumericValue::Vector(v.iter().map(promote_to_bigint).collect()),
...
pub fn promote_to_real(value: &NumericValue) -> NumericValue {
    match value {
        ...
        NumericValue::Vector(v) => NumericValue::Vector(v.iter().map(promote_to_real).collect()),
        _ => value.clone(),
    }
}
...
pub fn promote_to_complex(value: &NumericValue) -> NumericValue {
    match value {
        ...
        NumericValue::Vector(v) => NumericValue::Vector(v.iter().map(promote_to_complex).collect()),
    }
}
...
pub fn add(left: &NumericValue, right: &NumericValue) -> NumericValue {
    ...
    match (&left_promoted, &right_promoted) {
        ...
        (NumericValue::Rational(a), NumericValue::Rational(b)) => NumericValue::Rational(*a + *b),
        (NumericValue::Real(a), NumericValue::Real(b)) => NumericValue::Real(a + b),
        (NumericValue::Complex(a), NumericValue::Complex(b)) => NumericValue::Complex(*a + *b),
...
pub fn divide(left: &NumericValue, right: &NumericValue) -> Result<NumericValue, String> {
    ...
    match (&left_promoted, &right_promoted) {
        ...
        (NumericValue::Complex(a), NumericValue::Complex(b)) => Ok(NumericValue::Complex(*a / *b)),
        _ => unreachable!("Type promotion should ensure matching types"),
    }
}
...
pub fn negate(value: &NumericValue) -> NumericValue {
    match value {
        ...
        NumericValue::Vector(v) => NumericValue::Vector(v.iter().map(negate).collect()),
    }
}
...
pub fn power(base: &NumericValue, exponent: &NumericValue) -> NumericValue {
    match (base, exponent) {
        ...
        _ => {
            ...
            if let (NumericValue::Complex(b), NumericValue::Complex(e)) =
                (&base_complex, &exp_complex)
            {
...
pub fn make_exact(value: &NumericValue) -> NumericValue {
    match value {
        ...
        NumericValue::Complex(c) if c.is_real() => make_exact(&NumericValue::Real(c.real)),
        NumericValue::Complex(_) => value.clone(), // Cannot make complex exact in general
        NumericValue::Vector(v) => NumericValue::Vector(v.iter().map(make_exact).collect()),
    }
}
