    ...
    pub fn search(&self, text: &Text) -> Vec<usize> {
        ...
        while i <= n - m {
            ...
            if j == 0 {
                ...
                let bad_char_shift =
                    if let Some(&pos) = self.bad_char_table.get(&text_chars[i + j - 1]) {
...
impl RabinKarp {
    ...
    pub fn search(&self, text: &Text) -> Vec<usize> {
        ...
        for i in 1..=n - m {
            ...
            text_hash = (self.base
                * (text_hash + self.modulus - (text_chars[i - 1] as u64 * h) % self.modulus)
                + text_chars[i + m - 1] as u64)
                % self.modulus;
...
impl StringSimilarity {
    ...
    pub fn cosine_similarity(text1: &Text, text2: &Text, n: usize) -> f64 {
        ...
        let all_ngrams: std::collections::HashSet<_> =
            ngrams1.keys().chain(ngrams2.keys()).collect();
...
impl TextOperations {
    ...
    pub fn squeeze(text: &Text, chars_to_squeeze: Option<&Text>) -> Text {
        ...
        let squeeze_set: Option<std::collections::HashSet<char>> =
            chars_to_squeeze.map(|t| t.chars().into_iter().collect());
    ...
    pub fn common_prefix(texts: &[Text]) -> Text {
        ...
        let char_vectors: Vec<Vec<char>> = texts.iter().map(|t| t.chars()).collect();

        let min_length = char_vectors.iter().map(|v| v.len()).min().unwrap_or(0);
...
fn bind_search_algorithms(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "text-search-boyer-moore".to_string(),
...
fn bind_similarity_functions(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "text-levenshtein-distance".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "text-jaccard-similarity".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn primitive_text_search_boyer_moore(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "text-search-boyer-moore expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
fn primitive_text_levenshtein_distance(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "text-levenshtein-distance expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
fn primitive_text_jaccard_similarity(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "text-jaccard-similarity expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
fn primitive_text_join(args: &[Value]) -> Result<Value> {
    ...
    let texts: Result<Vec<Text>> = text_list.into_iter().map(|v| Text::try_from(&v)).collect();
