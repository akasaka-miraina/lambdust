...
use crate::eval::value::{Port, PrimitiveImpl, PrimitiveProcedure, ThreadSafeEnvironment, Value};
use crate::stdlib::{advanced_io, async_io, network_io, platform_io, security_io, streaming_io};
...
static IO_SYSTEM_CONFIG: LazyLock<Mutex<IoSystemConfig>> =
    LazyLock::new(|| Mutex::new(IoSystemConfig::default()));
static IO_PERFORMANCE_METRICS: LazyLock<Mutex<IoPerformanceMetrics>> =
    LazyLock::new(|| Mutex::new(IoPerformanceMetrics::default()));
...
fn bind_system_config_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "set-io-system-config".to_string(),
    ...
    env.define(
        "get-io-system-config".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "io-feature-available?".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_migration_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "port->advanced-handle".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "advanced-handle->port".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "migrate-io-operations".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_performance_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "get-io-performance-metrics".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "reset-io-performance-metrics".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        "benchmark-io-systems".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_compatibility_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "ensure-backward-compatibility".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
pub fn primitive_set_io_system_config(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "set-io-system-config expects 1 argument, got {}",
                args.len()
            ),
            None,
        )));
    ...
    match &args[0] {
        Value::Hashtable(config_table) => {
            let table = config_table.try_borrow().unwrap();
            ...
            if let Some(Value::Literal(crate::ast::Literal::Boolean(enable))) = table.get(
            ...
            if let Some(size_val) = table.get(&Value::Symbol(crate::utils::intern_symbol(
                "default-buffer-size",
            ))) {
...
pub fn primitive_io_feature_available_p(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "io-feature-available? expects 1 argument, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_unified_open_file(args: &[Value]) -> Result<Value> {
    if args.len() < 2 || args.len() > 4 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "unified-open-file expects 2 to 4 arguments, got {}",
                args.len()
            ),
            None,
        )));
    ...
    let _options = if args.len() > 3 { Some(&args[3]) } else { None };
...
mod tests {
    ...
    fn test_io_system_config() {
        ...
        let args = vec![Value::Hashtable(Rc::new(RefCell::new(config_map)))];
        ...
        if let Ok(Value::Hashtable(config)) = get_result {
            let config_map = config.try_borrow().unwrap();
            assert!(
                config_map.contains_key(&Value::Symbol(crate::utils::intern_symbol("version")))
            );
        } else {
    ...
    fn test_unified_file_operations() {
        ...
        let args = vec![Value::string(file_path), Value::string("write".to_string())];
    ...
    fn test_performance_metrics() {
        ...
        if let Ok(Value::Hashtable(metrics)) = result {
            let metrics_map = metrics.try_borrow().unwrap();
            assert!(
                metrics_map.contains_key(&Value::Symbol(crate::utils::intern_symbol(
                    "legacy-operations"
                )))
            );
            assert!(
