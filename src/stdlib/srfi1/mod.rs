...
pub use constructors::{
    srfi1_cons, srfi1_list, srfi1_xcons, srfi1_cons_star, 
    srfi1_list_copy, srfi1_make_list, srfi1_list_tabulate,
    srfi1_circular_list, srfi1_iota
};
pub use fold::{srfi1_fold, srfi1_fold_right, srfi1_reduce, srfi1_reduce_right};
pub use map::{srfi1_map, srfi1_for_each, srfi1_append_map, srfi1_map_in_order};
pub use filtering::{srfi1_filter, srfi1_remove, srfi1_partition};
pub use misc::{srfi1_length, srfi1_append, srfi1_reverse, srfi1_take, srfi1_drop};
...
impl SRFI1Core {
    ...
    pub fn validate_arity(args: &[Value], min: usize, max: Option<usize>, proc_name: &str) -> Result<()> {
        let arg_count = args.len();
        
        if arg_count < min {
            return Err(Box::new(crate::diagnostics::Error::runtime_error(
                format!("{} requires at least {} arguments, got {}", proc_name, min, arg_count),
                None,
            )));
        }
        ...
        if let Some(max_count) = max {
            if arg_count > max_count {
                return Err(Box::new(crate::diagnostics::Error::runtime_error(
                    format!("{} accepts at most {} arguments, got {}", proc_name, max_count, arg_count),
                    None,
                )));
            }
        }
...
mod tests {
    ...
    fn test_srfi1_core_fast_length() {
        ...
        let multi_list = Value::list(vec![Value::boolean(true), Value::boolean(false), Value::Nil]);
