...
impl StandardLibrary {
    ...
    pub fn populate_environment(&self, env: &Arc<ThreadSafeEnvironment>) {
        ...
        // Box (mutable cell) operations (SRFI-111)
        crate::stdlib::r#box::create_box_bindings(env);

        // Hash table operations (SRFI-125)
        crate::stdlib::hashtable::create_hashtable_bindings(env);

        // Sorting operations (SRFI-132)
        crate::stdlib::sorting::create_sorting_bindings(env);
    ...
    pub fn builtins(&self) -> Vec<(&'static str, super::BuiltinProcedure)> {
        vec![
            ...
            (
                "jiffies-per-second",
                super::BuiltinProcedure::JiffiesPerSecond,
            ),
            ("command-line", super::BuiltinProcedure::CommandLine),
            (
                "get-environment-variable",
                super::BuiltinProcedure::GetEnvironmentVariable,
            ),
            (
                "get-environment-variables",
                super::BuiltinProcedure::GetEnvironmentVariables,
            ),
