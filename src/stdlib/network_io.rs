...
fn bind_network_utilities(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "network-interface-list".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
pub fn primitive_tcp_connect(args: &[Value]) -> Result<Value> {
    if args.len() < 2 || args.len() > 4 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "tcp-connect expects 2 to 4 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_tcp_listen(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 3 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "tcp-listen expects 1 to 3 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
        ...
        match TcpListener::bind(&addr).await {
            Ok(listener) => {
                let network_listener =
                    NetworkListener::Tcp(Arc::new(std::sync::Mutex::new(Some(listener))));
...
pub fn primitive_tcp_accept(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "tcp-accept expects 1 or 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
                        ...
                        runtime.block_on(async move {
                            ...
                            if let Some(listener) = listener_guard.as_ref() {
                                ...
                                let (stream, addr) = if let Some(timeout_val) = timeout_ms {
                                    match timeout(Duration::from_millis(timeout_val), accept_future)
                                        .await
                                    {
                                ...
                                let socket = NetworkSocket::Tcp(Arc::new(std::sync::Mutex::new(
                                    Some(stream),
                                )));
                                let addr_str = addr.to_string();

                                Ok(Value::Pair(
                                    Box::new(Value::opaque(Box::new(socket))),
                                    Box::new(Value::string(addr_str)),
                                ))
                            } else {
                                Err(Box::new(DiagnosticError::runtime_error(
...
pub fn primitive_tcp_read(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 3 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "tcp-read expects 1 to 3 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
                        ...
                        runtime.block_on(async move {
                            ...
                            if let Some(stream) = stream_guard.as_mut() {
                                ...
                                let bytes_read = if let Some(timeout_val) = timeout_ms {
                                    match timeout(Duration::from_millis(timeout_val), read_future)
                                        .await
                                    {
                                        Ok(Ok(n)) => n,
                                        Ok(Err(e)) => {
...
pub fn primitive_tcp_write(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "tcp-write expects 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_resolve_hostname(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "resolve-hostname expects 1 or 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
mod tests {
    ...
    fn test_url_parsing() {
        ...
        if let Ok(Value::Hashtable(hashtable)) = result {
            let table = hashtable.try_borrow().unwrap();
