...
fn bind_character_predicates(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "char-general-category".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn primitive_char_general_category(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "char-general-category expects 1 argument, got {}",
                args.len()
            ),
            None,
        )));
...
fn primitive_char_titlecase(args: &[Value]) -> Result<Value> {
    ...
    Ok(Value::Literal(crate::ast::Literal::Character(
...
mod tests {
    ...
    fn test_char_comparison() {
        ...
        let result =
            primitive_char_equal(&[char_a.clone(), char_a.clone(), char_a.clone()]).unwrap();
        assert_eq!(result, Value::boolean(true));

        let result =
            primitive_char_equal(&[char_a.clone(), char_a.clone(), char_b.clone()]).unwrap();
        assert_eq!(result, Value::boolean(false));
        ...
        // Test transitive
        let result =
            primitive_char_less(&[char_a.clone(), char_b.clone(), char_c.clone()]).unwrap();
        assert_eq!(result, Value::boolean(true));

        let result =
            primitive_char_less(&[char_a.clone(), char_c.clone(), char_b.clone()]).unwrap();
        assert_eq!(result, Value::boolean(false));
    ...
    fn test_unicode_case() {
        ...
        assert!(matches!(
            result,
            Value::Literal(crate::ast::Literal::Character(_))
        ));
    }
