...
pub fn primitive_make_bytevector(args: &[Value]) -> Result<Value> {
    match args.len() {
        ...
        _ => Err(Box::new(Error::runtime_error(
            format!(
                "make-bytevector expects 1 or 2 arguments, got {}",
                args.len()
            ),
            None,
        ))),
...
pub fn primitive_bytevector_copy(args: &[Value]) -> Result<Value> {
    match args.len() {
        ...
        _ => Err(Box::new(Error::runtime_error(
            format!(
                "bytevector-copy expects 1 to 3 arguments, got {}",
                args.len()
            ),
            None,
        ))),
...
pub fn primitive_bytevector_to_list(args: &[Value]) -> Result<Value> {
    match args.len() {
        ...
        3 => {
            ...
            let values: Vec<Value> = bv[start..end]
                .iter()
                .map(|&b| Value::integer(b as i64))
                .collect();
            Ok(Value::list(values))
        ...
        _ => Err(Box::new(Error::runtime_error(
            format!(
                "bytevector->list expects 1 to 3 arguments, got {}",
                args.len()
            ),
            None,
        ))),
...
pub fn primitive_utf8_to_string(args: &[Value]) -> Result<Value> {
    match args.len() {
        1 => {
            ...
            let s = String::from_utf8(bv).map_err(|_| {
                Error::runtime_error("utf8->string: invalid UTF-8 sequence".to_string(), None)
            })?;
            Ok(Value::string(s))
        }
        2 => {
            ...
            let s = String::from_utf8(bv[start..].to_vec()).map_err(|_| {
                Error::runtime_error("utf8->string: invalid UTF-8 sequence".to_string(), None)
            })?;
            Ok(Value::string(s))
        }
        3 => {
            ...
            let s = String::from_utf8(bv[start..end].to_vec()).map_err(|_| {
                Error::runtime_error("utf8->string: invalid UTF-8 sequence".to_string(), None)
            })?;
            Ok(Value::string(s))
        }
...
mod tests {
    ...
    fn test_bytevector() {
        ...
        let result =
            primitive_bytevector(&[Value::integer(65), Value::integer(66), Value::integer(67)])
                .unwrap();
    ...
    fn test_bytevector_length() {
        let result =
            primitive_bytevector_length(&[Value::bytevector(vec![1, 2, 3, 4, 5])]).unwrap();
