...
use crate::stdlib::exceptions::{ErrorObject, ExceptionObject};
use std::sync::Arc;
...
pub fn enhanced_error_object_message(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "error-object-message expects exactly 1 argument, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn enhanced_error_object_irritants(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "error-object-irritants expects exactly 1 argument, got {}",
                args.len()
            ),
            None,
        )));
