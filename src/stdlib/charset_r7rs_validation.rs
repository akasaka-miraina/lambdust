...
impl CharSetR7RSValidator {
    ...
    pub fn validate_all() -> ValidationReport {
        ...
        report.add_test("Letter+digit characters", Self::validate_letter_plus_digit());
    ...
    fn validate_letter_plus_digit() -> TestResult {
        ...
        if !letter_plus_digit.is_equal(&union) {
            errors.push(ValidationError::SetOperationFailed {
                ...
                description: "Letter+digit set should be union of letter and digit sets".to_string(),
            });
        }
...
impl std::fmt::Display for ValidationError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ValidationError::Inconsistency { character, charset_result, predicate_name, predicate_result } => {
                write!(f, "Inconsistency for '{}' (U+{:04X}): charset={}, {}={}",
                    character, *character as u32, charset_result, predicate_name, predicate_result)
            }
            ValidationError::WrongSize { expected, actual, description } => {
                write!(f, "Wrong size for {}: expected {}, got {}", description, expected, actual)
            }
            ValidationError::MissingCharacter { character, description } => {
                write!(f, "Missing character '{}' (U+{:04X}): {}", character, *character as u32, description)
            }
            ValidationError::UnexpectedCharacter { character, description } => {
                write!(f, "Unexpected character '{}' (U+{:04X}): {}", character, *character as u32, description)
            }
            ValidationError::SetOperationFailed { operation, description } => {
...
impl ValidationReport {
    ...
    pub fn passed_tests(&self) -> usize {
        self.tests.iter().filter(|(_, result)| result.is_success()).count()
    }
    ...
    pub fn total_errors(&self) -> usize {
        self.tests.iter().map(|(_, result)| result.error_count()).sum()
    }
...
mod tests {
    ...
    fn test_validate_standard_character_sets() {
        ...
        if !report.is_fully_compliant() {
            println!("WARNING: Found {} compliance issues that need attention", report.total_errors());
        }
    }
    ...
    fn test_digit_set_basic_requirements() {
        ...
        assert_eq!(digit_set.size(), 10, "Digit set must have exactly 10 characters");
        
        for digit in '0'..='9' {
            assert!(digit_set.contains(digit), "Digit set must contain '{}'", digit);
        }
        
        // Should not contain letters
        for letter in 'a'..='z' {
            assert!(!digit_set.contains(letter), "Digit set must not contain '{}'", letter);
        }
    }
    ...
    fn test_empty_set_requirements() {
        ...
        assert!(!empty_set.contains('a'), "Empty set must not contain any characters");
    }
}