...
impl SecurityManager {
    ...
    pub fn check_path_access(&self, path: &Path, operation: &str) -> Result<()> {
        let policy = self.policy.try_read().unwrap();
        ...
        if !policy.allowed_paths.is_empty() {
            ...
            if !allowed {
                ...
                return Err(Box::new(DiagnosticError::runtime_error(
                    format!(
                        "Access denied to path not in allowed list: {}",
                        canonical_path.display()
                    ),
                    None,
                )));
    ...
    pub fn check_bandwidth_limit(&self, bytes: u64) -> Result<()> {
        ...
        if let Some(max_bandwidth) = policy.max_bandwidth {
            ...
            usage
                .recent_transfers
                .retain(|(timestamp, _)| now.duration_since(*timestamp) <= bandwidth_window);

            // Calculate current bandwidth usage
            let current_usage: u64 = usage.recent_transfers.iter().map(|(_, bytes)| *bytes).sum();

            if current_usage + bytes > max_bandwidth {
                return Err(Box::new(DiagnosticError::runtime_error(
                    format!(
                        "Bandwidth limit exceeded: {current_usage} + {bytes} > {max_bandwidth}"
                    ),
                    None,
                )));
            }
    ...
    pub fn check_open_file_limit(&self) -> Result<()> {
        ...
        if let Some(max_files) = policy.max_open_files {
            if usage.open_files >= max_files {
                return Err(Box::new(DiagnosticError::runtime_error(
                    format!(
                        "Open file limit exceeded: {open_files} >= {max_files}",
            ...
            if let Some(ref path) = chroot_path {
                ...
                chdir(path).map_err(|e| {
                    DiagnosticError::runtime_error(
                        format!(
                            "Cannot change to chroot directory '{}': {e}",
                            path.display()
                        ),
                        None,
                    )
...
fn bind_resource_management_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "reset-resource-counters".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_sandbox_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "create-secure-environment".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
pub fn primitive_set_security_policy(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "set-security-policy expects 1 argument, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
    ...
    match &args[0] {
        Value::Hashtable(policy_table) => {
            let table = policy_table.try_borrow().unwrap();
            ...
            if let Some(max_size_val) =
                table.get(&Value::Symbol(crate::utils::intern_symbol("max-file-size")))
            {
            ...
            if let Some(max_bandwidth_val) =
                table.get(&Value::Symbol(crate::utils::intern_symbol("max-bandwidth")))
            {
            ...
            if let Some(max_files_val) = table.get(&Value::Symbol(crate::utils::intern_symbol(
                "max-open-files",
            ))) {
...
pub fn primitive_get_security_policy(_args: &[Value]) -> Result<Value> {
    let security_manager = get_security_manager();
    let manager = security_manager.lock().unwrap();
    let policy = manager.policy.try_read().unwrap();
...
pub fn primitive_add_allowed_path(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "add-allowed-path expects 1 argument, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_add_forbidden_path(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "add-forbidden-path expects 1 argument, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_check_path_access(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "check-path-access expects 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_enable_sandbox(args: &[Value]) -> Result<Value> {
    if args.len() > 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "enable-sandbox expects 0 or 1 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_enable_audit_logging(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "enable-audit-logging expects 1 argument, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_get_audit_log(args: &[Value]) -> Result<Value> {
    if args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "get-audit-log expects 0 to 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
    ...
    let audit_entries: Vec<Value> = entries_to_return
        .into_iter()
        .rev()
        .map(|entry| {
            #[allow(clippy::mutable_key_type)]
            ...
            Value::Hashtable(Rc::new(RefCell::new(entry_map)))
        })
        .collect();
...
pub fn primitive_secure_file_read(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "secure-file-read expects 1 or 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_secure_file_write(args: &[Value]) -> Result<Value> {
    if args.len() < 2 || args.len() > 3 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "secure-file-write expects 2 or 3 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
pub fn primitive_validate_file_path(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "validate-file-path expects 1 or 2 arguments, got {args_len}",
                args_len = args.len()
            ),
            None,
        )));
...
mod tests {
    ...
    fn test_security_policy() {
        ...
        let args = vec![Value::Hashtable(Rc::new(RefCell::new(policy_map)))];
