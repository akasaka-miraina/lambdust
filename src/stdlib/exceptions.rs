...
type EvaluatorExceptionHandler = Arc<dyn Fn(&mut crate::eval::evaluator::Evaluator, &ExceptionObject) -> Result<ContinuationResult> + Send + Sync>;
...
impl std::fmt::Debug for HandlerEntry {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HandlerEntry")
            ...
            .field("id", &self.id)
            .field("handler", match &self.handler {
...
impl ExceptionSystem {
        ...
        EXCEPTION_HANDLERS.with(|handlers| {
            ...
            if stack.depth() >= self.config.max_stack_depth {
                return Err(Box::new(DiagnosticError::runtime_error(
                    format!("Exception handler stack overflow (max depth: {})", self.config.max_stack_depth),
                    None,
                )));
            }
    ...
    where
        F: Fn(&mut crate::eval::evaluator::Evaluator, &ExceptionObject) -> Result<ContinuationResult> + Send + Sync + 'static,
    {
        EXCEPTION_HANDLERS.with(|handlers| {
            let mut stack = handlers.borrow_mut();
    ...
    pub fn raise_exception(&self, exception: ExceptionObject) -> Result<Value> {
        match self.handle_exception(&exception)? {
            ...
            ContinuationResult::Reraise(new_exception) => {
                self.raise_exception(new_exception)
            },
...
fn bind_exception_raising(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define("raise-continuable".to_string(), Value::Primitive(Arc::new(PrimitiveProcedure {
        ...
        arity_min: 1,
        arity_max: Some(1),
        implementation: PrimitiveImpl::EvaluatorIntegrated(primitive_raise_continuable_evaluator),
        effects: vec![Effect::Error],
    })));
...
fn bind_exception_handling(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define("with-exception-handler".to_string(), Value::Primitive(Arc::new(PrimitiveProcedure {
        ...
        arity_min: 2,
        arity_max: Some(2),
        implementation: PrimitiveImpl::EvaluatorIntegrated(primitive_with_exception_handler_evaluator),
        effects: vec![Effect::Error],
    })));
}
...
fn bind_exception_system_procedures(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define("exception-stack-depth".to_string(), Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define("exception-stack-empty?".to_string(), Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn primitive_raise_evaluator(evaluator: &mut crate::eval::evaluator::Evaluator, args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!("raise expects 1 argument, got {}", args.len()),
            None,
    ...
    match exception_system().raise_exception(exception.clone()) {
        ...
        Err(handler_error) => {
            if !has_handlers {
                ...
                Err(Box::new(DiagnosticError::exception(non_continuable_exception)))
            } else {
...
/// Scheme procedures or when complex evaluation is involved.
fn primitive_raise_continuable_evaluator(evaluator: &mut crate::eval::evaluator::Evaluator, args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!("raise-continuable expects 1 argument, got {}", args.len()),
            None,
...
fn primitive_with_exception_handler_evaluator(evaluator: &mut crate::eval::evaluator::Evaluator, args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
    ...
    let handler_is_proc = matches!(handler_proc, 
        Value::Procedure(_) | 
        Value::CaseLambda(_) | 
        Value::Primitive(_) | 
    ...
    let thunk_is_proc = matches!(thunk,
        Value::Procedure(_) | 
        Value::CaseLambda(_) | 
        Value::Primitive(_) | 
        Value::Continuation(_)
    ...
    let handler_fn = move |eval: &mut crate::eval::evaluator::Evaluator, exception: &ExceptionObject| -> Result<ContinuationResult> {
        ...
        match eval.apply_procedure(handler_proc_clone.clone(), args, None) {
            ...
            crate::eval::evaluator::EvalStep::TailCall { procedure, args, location } => {
    ...
    let handler_id = exception_system.push_evaluator_handler(handler_fn, true)
...
fn execute_thunk_with_exception_handling(
    ...
    loop {
        match current_step {
            ...
            crate::eval::evaluator::EvalStep::Error(error) => {
                ...
                match error {
                    DiagnosticError::Exception { exception, .. } => {
                        ...
                        match exception_system().handle_exception_with_evaluator(&exception, evaluator) {
                            Ok(continuation_result) => {
                                match continuation_result {
                                    ...
                                    ContinuationResult::Reraise(new_exception) => {
                                        ...
                                        return Err(Box::new(DiagnosticError::exception(new_exception)));
                                    }
                                }
                            },
                            Err(handler_error) => {
            ...
            crate::eval::evaluator::EvalStep::CallContinuation { continuation, value } => {
                // Call the continuation
                current_step = evaluator.call_continuation(continuation, value);
            },
            crate::eval::evaluator::EvalStep::NonLocalJump { value, target_stack_depth: _ } => {
...
fn primitive_exception_stack_depth(args: &[Value]) -> Result<Value> {
    if !args.is_empty() {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!("exception-stack-depth expects 0 arguments, got {}", args.len()),
            None,
        )));
    }
...
fn primitive_exception_stack_empty_p(args: &[Value]) -> Result<Value> {
    if !args.is_empty() {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!("exception-stack-empty? expects 0 arguments, got {}", args.len()),
            None,
        )));
    }
...
mod tests {
    ...
    fn test_raise_continuable() {
        ...
        let handler = |_exception: &ExceptionObject| -> Result<ContinuationResult> {
            Err(Box::new(DiagnosticError::runtime_error("not handled", None)))
        };
    ...
    fn test_exception_system_simple_handling() {
        ...
        let test_exception = ExceptionObject::new(
            "test".to_string(), 
    ...
    fn test_exception_system_handler_chain() {
        ...
        let first_handler = |exception: &ExceptionObject| -> Result<ContinuationResult> {
            if exception.exception_type == "first" {
                ...
                Err(Box::new(DiagnosticError::runtime_error("not my exception", None)))
            }
        };
        ...
        let second_exception = ExceptionObject::new(
            "second".to_string(),
            Value::string("test"),
            false
        );
        ...
        let first_exception = ExceptionObject::new(
            "first".to_string(),
            Value::string("test"),
            false
        );
    ...
    fn test_exception_system_unhandled_exception() {
        ...
        let handler = |_exception: &ExceptionObject| -> Result<ContinuationResult> {
            Err(Box::new(DiagnosticError::runtime_error("can't handle this", None)))
        };
        ...
        if let Err(boxed_err) = result {
            if let DiagnosticError::Exception { exception: returned_exc, .. } = boxed_err.as_ref() {
    ...
    fn test_with_exception_handler_arity_validation() {
        ...
        let result = primitive_with_exception_handler_evaluator(&mut evaluator, &[Value::integer(1)]);
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("expects 2 arguments"));
    ...
    fn test_with_exception_handler_argument_validation() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("first argument must be a procedure"));
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("second argument must be a procedure"));
    }
    ...
    fn test_continuation_result_enum() {
        ...
        let reraise_result = ContinuationResult::Reraise(
            ExceptionObject::new("test".to_string(), Value::Nil, false)
        );
        
        // These should all be different variants
        match (&continue_result, &handle_result, &reraise_result) {
            (ContinuationResult::Continue(_), ContinuationResult::Handle(_), ContinuationResult::Reraise(_)) => {},
    ...
    fn test_handler_type_enum() {
        ...
        let eval_handler = |_: &mut crate::eval::evaluator::Evaluator, _: &ExceptionObject| -> Result<ContinuationResult> {
            Ok(ContinuationResult::Handle(Value::Unspecified))
        };
    ...
    fn test_exception_handler_stack_ordering() {
        ...
        let handler1 = |exception: &ExceptionObject| -> Result<ContinuationResult> {
            if exception.exception_type == "test" {
                ...
                Err(Box::new(DiagnosticError::runtime_error("not handled by 1", None)))
            }
        };
        ...
        let handler2 = |exception: &ExceptionObject| -> Result<ContinuationResult> {
            if exception.exception_type == "test" {
                ...
                Err(Box::new(DiagnosticError::runtime_error("not handled by 2", None)))
            }
        };
    ...
    fn test_raise_continuable_with_multiple_handlers() {
        ...
        let second_handler = |exception: &ExceptionObject| -> Result<ContinuationResult> {
            if exception.exception_type == "exception" && exception.continuable {
                Ok(ContinuationResult::Continue(Value::string("second-continued")))
            } else {
                Err(Box::new(DiagnosticError::runtime_error("not my exception", None)))
            }
        };
    ...
    fn test_raise_continuable_with_reraise() {
        ...
        if let Err(boxed_err) = result {
            if let DiagnosticError::Exception { exception, .. } = boxed_err.as_ref() {
                ...
                assert!(matches!(exception.exception_type.as_str(), "exception" | "modified"));
            } else {
                panic!("Expected exception error");
            }
        } else {
