...
use crate::stdlib::strings::common::{bind_primitive, extract_character, extract_string, validate_string_bounds};
use std::sync::Arc;
...
pub fn primitive_string_copy(args: &[Value]) -> Result<Value> {
    ...
    let result_chars: Vec<char> = chars.into_iter().skip(start).take(actual_end - start).collect();
