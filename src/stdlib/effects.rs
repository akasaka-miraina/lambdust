...
fn bind_effect_handler_functions(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "define-effect-handler".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn primitive_define_effect_handler(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "define-effect-handler expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
fn primitive_effect_pure_p(args: &[Value]) -> Result<Value> {
    ...
    let is_pure = !args[0]
        .as_string()
        .map(|s| s.contains("IO") || s.contains("State") || s.contains("Error"))
        .unwrap_or(false);
