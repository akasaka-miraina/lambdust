...
fn bind_filesystem_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "create-symbolic-link".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_metadata_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "file-modification-time".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_permission_operations(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "set-file-permissions".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
pub fn primitive_create_directory(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "create-directory expects 1 or 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_delete_directory(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "delete-directory expects 1 or 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_create_symbolic_link(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "create-symbolic-link expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_list_directory(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "list-directory expects 1 or 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_current_directory(args: &[Value]) -> Result<Value> {
    match args.len() {
        ...
        _ => Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "current-directory expects 0 or 1 arguments, got {}",
                args.len()
            ),
            None,
        ))),
...
pub fn primitive_walk_directory(args: &[Value]) -> Result<Value> {
    if args.is_empty() || args.len() > 3 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "walk-directory expects 1 to 3 arguments, got {}",
                args.len()
            ),
            None,
        )));
    ...
    let walker = walkdir::WalkDir::new(&path).follow_links(follow_links);
...
pub fn primitive_file_metadata(args: &[Value]) -> Result<Value> {
    ...
    match std::fs::metadata(&path) {
        Ok(metadata) => {
            ...
            // Basic properties
            result.insert(
                Value::Symbol(crate::utils::intern_symbol("size")),
                Value::integer(metadata.len() as i64),
            );
            result.insert(
                Value::Symbol(crate::utils::intern_symbol("is-file")),
                Value::boolean(metadata.is_file()),
            );
            result.insert(
                Value::Symbol(crate::utils::intern_symbol("is-dir")),
                Value::boolean(metadata.is_dir()),
            );
            result.insert(
                Value::Symbol(crate::utils::intern_symbol("is-symlink")),
            ...
            if let Ok(modified) = metadata.modified() {
                if let Ok(duration) = modified.duration_since(UNIX_EPOCH) {
                    result.insert(
                        Value::Symbol(crate::utils::intern_symbol("modified")),
            ...
            if let Ok(accessed) = metadata.accessed() {
                if let Ok(duration) = accessed.duration_since(UNIX_EPOCH) {
                    result.insert(
                        Value::Symbol(crate::utils::intern_symbol("accessed")),
                        Value::integer(duration.as_secs() as i64),
                    );
                }
            }
            ...
            if let Ok(created) = metadata.created() {
                if let Ok(duration) = created.duration_since(UNIX_EPOCH) {
                    result.insert(
                        Value::Symbol(crate::utils::intern_symbol("created")),
                        Value::integer(duration.as_secs() as i64),
                    );
                }
            }
            ...
            {
                result.insert(
                    Value::Symbol(crate::utils::intern_symbol("mode")),
                    Value::integer(metadata.mode() as i64),
                );
                result.insert(
                    Value::Symbol(crate::utils::intern_symbol("uid")),
                    Value::integer(metadata.uid() as i64),
                );
                result.insert(
                    Value::Symbol(crate::utils::intern_symbol("gid")),
                    Value::integer(metadata.gid() as i64),
                );
                result.insert(
                    Value::Symbol(crate::utils::intern_symbol("nlink")),
            ...
            {
                result.insert(
                    Value::Symbol(crate::utils::intern_symbol("file-attributes")),
...
pub fn primitive_file_modification_time(args: &[Value]) -> Result<Value> {
    if args.len() != 1 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "file-modification-time expects 1 argument, got {}",
                args.len()
            ),
            None,
        )));
...
pub fn primitive_set_file_permissions(args: &[Value]) -> Result<Value> {
    if args.len() != 2 {
        return Err(Box::new(DiagnosticError::runtime_error(
            format!(
                "set-file-permissions expects 2 arguments, got {}",
                args.len()
            ),
            None,
        )));
