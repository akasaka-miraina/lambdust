...
fn bind_constructors(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "make-range-generator".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
...
fn bind_utilities(env: &Arc<ThreadSafeEnvironment>) {
    ...
    env.define(
        "generator-concatenate".to_string(),
        Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    env.define(
        ...
        Value::symbol_from_str("*eof-object*"),
    );
}
...
fn primitive_generator_exhausted_p(args: &[Value]) -> LambdustResult<Value> {
    if args.len() != 1 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "%generator-exhausted?: expected 1 argument, got {}",
                args.len()
            ),
            span: None,
        }));
    ...
    match &args[0] {
        Value::Generator(generator) => Ok(Value::boolean(generator.is_exhausted())),
        _ => Err(Box::new(crate::diagnostics::Error::RuntimeError {
...
fn primitive_make_range_generator(args: &[Value]) -> LambdustResult<Value> {
    match args.len() {
        1 => {
            ...
            let start = args[0].as_number().ok_or_else(|| {
        ...
        2 => {
            ...
            let end = args[1].as_number().ok_or_else(|| {
                Box::new(crate::diagnostics::Error::RuntimeError {
        ...
        3 => {
            ...
            let step = args[2].as_number().ok_or_else(|| {
                Box::new(crate::diagnostics::Error::RuntimeError {
        ...
        _ => Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "make-range-generator: expected 1-3 arguments, got {}",
                args.len()
            ),
            span: None,
        })),
...
fn primitive_make_iota_generator(args: &[Value]) -> LambdustResult<Value> {
    match args.len() {
        ...
        3 => {
            ...
            let step = args[2].as_integer().ok_or_else(|| {
                Box::new(crate::diagnostics::Error::RuntimeError {
        ...
        _ => Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "make-iota-generator: expected 0-3 arguments, got {}",
                args.len()
            ),
            span: None,
        })),
...
fn primitive_vector_to_generator(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 3 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "vector->generator: expected 1-3 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
    ...
    // Get vector length
    let vector_len = {
        let vec_guard = vector.try_borrow().map_err(|_| {
            Box::new(crate::diagnostics::Error::RuntimeError {
    ...
    let start = if args.len() > 1 {
        let start_val = args[1].as_integer().ok_or_else(|| {
            Box::new(crate::diagnostics::Error::RuntimeError {
    ...
    let end = if args.len() > 2 {
        let end_val = args[2].as_integer().ok_or_else(|| {
            Box::new(crate::diagnostics::Error::RuntimeError {
...
fn primitive_string_to_generator(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 3 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "string->generator: expected 1-3 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
    ...
    let string = args[0].as_string_owned().ok_or_else(|| {
        Box::new(crate::diagnostics::Error::RuntimeError {
...
fn primitive_generator_to_list(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "generator->list: expected 1-2 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
...
fn primitive_generator_to_vector(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "generator->vector: expected 1-2 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
    ...
    let max_length = if args.len() == 2 {
        Some(
            args[1]
                ...
                .ok_or_else(|| {
                    Box::new(crate::diagnostics::Error::RuntimeError {
                        message: "generator->vector: second argument must be an integer"
                            .to_string(),
                        span: None,
                    })
                })?
...
fn primitive_generator_to_string(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "generator->string: expected 1-2 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
    ...
    let max_length = if args.len() == 2 {
        Some(
            args[1]
                ...
                .ok_or_else(|| {
                    Box::new(crate::diagnostics::Error::RuntimeError {
                        message: "generator->string: second argument must be an integer"
                            .to_string(),
                        span: None,
                    })
                })?
        ...
        match generator.next() {
            Ok(value) => {
                ...
                match value {
                    ...
                    _ => {
                        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
                            message: "generator->string: generator must yield characters"
                                .to_string(),
                            span: None,
                        }));
                    }
...
fn primitive_generator_tabulate(args: &[Value]) -> LambdustResult<Value> {
    if args.is_empty() || args.len() > 2 {
        return Err(Box::new(crate::diagnostics::Error::RuntimeError {
            message: format!(
                "generator-tabulate: expected 1-2 arguments, got {}",
                args.len()
            ),
            span: None,
        }));
