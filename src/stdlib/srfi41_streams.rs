...
use crate::eval::stream::{StreamError, StreamNode, StreamRef};
use crate::eval::value::ThreadSafeEnvironment;
use crate::eval::value::{PrimitiveImpl, PrimitiveProcedure, Promise, Value};
    ...
    pub fn stream_cons(args: &[Value]) -> Result<Value> {
        if args.len() != 2 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "stream-cons expects exactly 2 arguments, got {}",
                    args.len()
                ),
                None,
            )));
...
impl SRFI41Streams {
    ...
    pub fn stream_null_predicate(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "stream-null? expects exactly 1 argument, got {}",
                    args.len()
                ),
                None,
            )));
...
impl SRFI41Streams {
    ...
    pub fn list_to_stream(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "list->stream expects exactly 1 argument, got {}",
                    args.len()
                ),
                None,
            )));
...
impl SRFI41Streams {
    ...
    pub fn stream_take(args: &[Value]) -> Result<Value> {
        if args.len() != 2 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "stream-take expects exactly 2 arguments, got {}",
                    args.len()
                ),
                None,
            )));
    ...
    pub fn stream_drop(args: &[Value]) -> Result<Value> {
        if args.len() != 2 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "stream-drop expects exactly 2 arguments, got {}",
                    args.len()
                ),
                None,
            )));
    ...
    pub fn stream_map(args: &[Value]) -> Result<Value> {
        if args.len() < 2 {
            return Err(Box::new(Error::runtime_error(
                format!(
                    "stream-map expects at least 2 arguments, got {}",
                    args.len()
                ),
                None,
            )));
