...
mod concurrency_impl {
    use crate::concurrency::{
        AtomicCounter, Mutex, SemaphoreSync,
        actors::{EchoActor, global_actor_system},
        channels::{Channel, ChannelConfig, ChannelType},
        distributed::DistributedNode,
        futures::{Future, FutureOps, Promise},
        parallel::{ParallelConfig, ParallelOps},
        scheduler::{Priority, submit_priority_task, submit_task},
    };
    ...
    fn register_future_operations(env: &ThreadSafeEnvironment) {
        ...
        env.define(
            ...
            Value::Primitive(Arc::new(crate::eval::value::PrimitiveProcedure {
                ...
                implementation: crate::eval::value::PrimitiveImpl::RustFn(|args| {
                    if args.len() != 1 {
                        return Err(Box::new(Error::runtime_error(
                            "future-resolved expects 1 argument".to_string(),
                            None,
                        )));
                    }
        ...
        env.define(
            ...
            Value::Primitive(Arc::new(PrimitiveProcedure {
                ...
                implementation: PrimitiveImpl::RustFn(|args| {
                    if args.len() != 1 {
                        return Err(Box::new(Error::runtime_error(
                            "future-rejected expects 1 argument".to_string(),
                            None,
                        )));
                    }
        ...
        env.define(
            ...
            Value::Primitive(Arc::new(PrimitiveProcedure {
                ...
                implementation: PrimitiveImpl::RustFn(|args| {
                    if !args.is_empty() {
                        return Err(Box::new(Error::runtime_error(
                            "promise expects no arguments".to_string(),
                            None,
                        )));
                    }
    ...
    fn register_channel_operations(env: &ThreadSafeEnvironment) {
            ...
            Value::Primitive(Arc::new(crate::eval::value::PrimitiveProcedure {
                ...
                implementation: crate::eval::value::PrimitiveImpl::RustFn(|args| {
                    let config = if args.is_empty() {
                        ...
                        let capacity = args[0].as_number().ok_or_else(|| {
                            Error::runtime_error("Capacity must be a number".to_string(), None)
                        })?;
                        ChannelConfig {
                        ...
                        return Err(Box::new(Error::runtime_error(
                            "make-channel expects 0 or 1 arguments".to_string(),
                            None,
                        )));
                    };

                    let channel = Channel::new(config).map_err(|e| {
                        Error::runtime_error(format!("Failed to create channel: {e}"), None)
                    })?;
        ...
        env.define(
            "make-unbounded-channel".to_string(),
            Value::Primitive(Arc::new(PrimitiveProcedure {
        ...
        env.define(
            "make-broadcast-channel".to_string(),
            Value::Primitive(Arc::new(PrimitiveProcedure {
    ...
    fn register_parallel_operations(env: &ThreadSafeEnvironment) {
        ...
        fn par_map_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 2 {
                return Err(Box::new(Error::runtime_error(
                    "par-map expects 2 arguments".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "par-map".to_string(),
            create_primitive("par-map", 2, Some(2), par_map_impl),
        );
        ...
        fn par_filter_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 2 {
                return Err(Box::new(Error::runtime_error(
                    "par-filter expects 2 arguments".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "par-filter".to_string(),
            create_primitive("par-filter", 2, Some(2), par_filter_impl),
        );
        ...
        fn par_reduce_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 3 {
                return Err(Box::new(Error::runtime_error(
                    "par-reduce expects 3 arguments".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "par-reduce".to_string(),
            create_primitive("par-reduce", 3, Some(3), par_reduce_impl),
        );
    }
    ...
    fn register_sync_operations(env: &ThreadSafeEnvironment) {
        ...
        fn make_mutex_impl(args: &[Value]) -> Result<Value> {
            let value = if args.is_empty() {
                ...
                return Err(Box::new(Error::runtime_error(
                    "make-mutex expects 0 or 1 arguments".to_string(),
                    None,
                )));
            };
        ...
        env.define(
            ...
            create_primitive("make-mutex", 0, Some(1), make_mutex_impl),
        );
        ...
        fn make_semaphore_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 1 {
                return Err(Box::new(Error::runtime_error(
                    "make-semaphore expects 1 argument".to_string(),
                    None,
                )));
            }

            let permits = args[0].as_number().ok_or_else(|| {
                Error::runtime_error("Permits must be a number".to_string(), None)
            })?;
        ...
        env.define(
            "make-semaphore".to_string(),
            create_primitive("make-semaphore", 1, Some(1), make_semaphore_impl),
        );
        ...
        fn make_atomic_counter_impl(args: &[Value]) -> Result<Value> {
            let initial = if args.is_empty() {
                ...
                args[0].as_number().ok_or_else(|| {
                    Error::runtime_error("Initial value must be a number".to_string(), None)
                })? as i64
            } else {
                return Err(Box::new(Error::runtime_error(
                    "make-atomic-counter expects 0 or 1 arguments".to_string(),
                    None,
                )));
            };
        ...
        env.define(
            "make-atomic-counter".to_string(),
            create_primitive("make-atomic-counter", 0, Some(1), make_atomic_counter_impl),
        );
        ...
        fn atomic_counter_increment_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 1 {
                return Err(Box::new(Error::runtime_error(
                    "atomic-counter-increment! expects 1 argument".to_string(),
                    None,
                )));
            }
            ...
            if let Value::AtomicCounter(counter) = &args[0] {
                ...
                Err(Box::new(Error::runtime_error(
                    "Argument must be an atomic counter".to_string(),
                    None,
                )))
            }
        }
        ...
        env.define(
            "atomic-counter-increment!".to_string(),
            create_primitive(
                "atomic-counter-increment!",
                1,
                Some(1),
                atomic_counter_increment_impl,
            ),
        );
        ...
        fn atomic_counter_get_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 1 {
                return Err(Box::new(Error::runtime_error(
                    "atomic-counter-get expects 1 argument".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "atomic-counter-get".to_string(),
            create_primitive("atomic-counter-get", 1, Some(1), atomic_counter_get_impl),
        );
    }
    ...
    fn register_actor_operations(env: &ThreadSafeEnvironment) {
        ...
        fn spawn_actor_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 1 {
                return Err(Box::new(Error::runtime_error(
                    "spawn-actor expects 1 argument".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            ...
            create_primitive("spawn-actor", 1, Some(1), spawn_actor_impl),
        );
        ...
        fn actor_tell_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 2 {
                return Err(Box::new(Error::runtime_error(
                    "actor-tell expects 2 arguments".to_string(),
                    None,
                )));
            }

            let actor_id = args[0].as_number().ok_or_else(|| {
                Error::runtime_error("Actor ID must be a number".to_string(), None)
            })? as u64;
            ...
            {
                actor_ref.tell(message).map_err(|e| {
                    Error::runtime_error(format!("Failed to send message: {e}"), None)
                })?;
        ...
        env.define(
            "actor-tell".to_string(),
            create_primitive("actor-tell", 2, Some(2), actor_tell_impl),
        );
    }
    ...
    fn register_scheduler_operations(env: &ThreadSafeEnvironment) {
        ...
        fn submit_task_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 1 {
                return Err(Box::new(Error::runtime_error(
                    "submit-task expects 1 argument".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "submit-task".to_string(),
            create_primitive("submit-task", 1, Some(1), submit_task_impl),
        );
        ...
        fn submit_priority_task_impl(args: &[Value]) -> Result<Value> {
            if args.len() != 2 {
                return Err(Box::new(Error::runtime_error(
                    "submit-priority-task expects 2 arguments".to_string(),
                    None,
                )));
            }

            let thunk = args[0].clone();
            let priority_num = args[1].as_number().ok_or_else(|| {
                Error::runtime_error("Priority must be a number".to_string(), None)
            })? as u8;
            ...
            let priority = match priority_num {
                ...
                _ => {
                    return Err(Box::new(Error::runtime_error(
                        "Priority must be 0-3".to_string(),
                        None,
                    )));
                }
            };

            let task_id = submit_priority_task(
                move || {
                    // In a real implementation, you'd call the Scheme thunk here
                    Ok(Value::Unspecified)
                },
        ...
        env.define(
            "submit-priority-task".to_string(),
            create_primitive(
                "submit-priority-task",
                2,
                Some(2),
                submit_priority_task_impl,
            ),
        );
    }
    ...
    fn register_distributed_operations(env: &ThreadSafeEnvironment) {
        ...
        fn make_distributed_node_impl(args: &[Value]) -> Result<Value> {
            if !args.is_empty() {
                return Err(Box::new(Error::runtime_error(
                    "make-distributed-node expects no arguments".to_string(),
                    None,
                )));
            }
        ...
        env.define(
            "make-distributed-node".to_string(),
            create_primitive(
                "make-distributed-node",
                0,
                Some(0),
                make_distributed_node_impl,
            ),
        );
    ...
    fn primitive_future_delay(args: &[Value]) -> Result<Value> {
        if args.len() != 2 {
            return Err(Box::new(Error::runtime_error(
                "future-delay expects 2 arguments".to_string(),
                None,
            )));
        }

        let duration_ms = args[0]
            .as_number()
    ...
    fn primitive_future_all(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                "future-all expects 1 argument".to_string(),
                None,
            )));
        }
            ...
            match current {
                Value::Pair(car, cdr) => {
                    if let Value::Future(future) = car.as_ref() {
                        ...
                        return Err(Box::new(Error::runtime_error(
                            "Expected future in list".to_string(),
                            None,
                        )));
                    }
                    current = cdr;
                ...
                _ => {
                    return Err(Box::new(Error::runtime_error(
                        "Expected list of futures".to_string(),
                        None,
                    )));
                }
            }
    ...
    fn primitive_future_race(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                "future-race expects 1 argument".to_string(),
                None,
            )));
        }
    ...
    fn primitive_make_unbounded_channel(args: &[Value]) -> Result<Value> {
        if !args.is_empty() {
            return Err(Box::new(Error::runtime_error(
                "make-unbounded-channel expects no arguments".to_string(),
                None,
            )));
        }
    ...
    fn primitive_make_broadcast_channel(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                "make-broadcast-channel expects 1 argument".to_string(),
                None,
            )));
        }
    ...
    fn primitive_channel_send(args: &[Value]) -> Result<Value> {
        if args.len() != 2 {
            return Err(Box::new(Error::runtime_error(
                "channel-send! expects 2 arguments".to_string(),
                None,
            )));
        }
        ...
        if let Value::Channel(channel) = &args[0] {
            ...
            let future =
                Future::new(async move { sender.send(value).await.map(|_| Value::Unspecified) });

            Ok(Value::Future(Arc::new(future)))
        } else {
            Err(Box::new(Error::runtime_error(
                "First argument must be a channel".to_string(),
                None,
            )))
        }
    }
    ...
    fn primitive_channel_recv(args: &[Value]) -> Result<Value> {
        if args.len() != 1 {
            return Err(Box::new(Error::runtime_error(
                "channel-recv! expects 1 argument".to_string(),
                None,
            )));
        }
        ...
        if let Value::Channel(channel) = &args[0] {
            ...
            Err(Box::new(Error::runtime_error(
                "Argument must be a channel".to_string(),
                None,
            )))
        }
    }
