...
impl OptimizationController {
    ...
    pub fn update_stats(&mut self, name: &str, success: bool, duration: std::time::Duration) {
        if let Some(stats) = self.stats.get_mut(name) {
            ...
            stats.avg_time_us = (stats.avg_time_us * (stats.applications - 1) as u64 + duration_us) / stats.applications as u64;
        }
    }
}
...
impl VerificationSystem {
    ...
    pub fn verify_optimization(&mut self, original: &Expr, optimized: &Expr, reference: &Value) -> Result<bool> {
    ...
    pub fn validate_pattern(&mut self, original: &Expr, transformed: &Expr) -> Result<bool> {
        self.stats.total_verifications += 1;
