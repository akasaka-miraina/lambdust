...
impl CachedEnvironment {
    ...
    pub fn with_cache_size(base_env: Rc<Environment>, cache_size: usize) -> Self {
        Self {
            base_env,
            lookup_cache: Rc::new(std::cell::RefCell::new(lru::LruCache::new(
    ...
    pub fn lookup(&self, name: &str) -> Option<Value> {
        ...
        if let Ok(bindings_ref) = self.flattened_bindings.try_borrow() {
            if let Some(ref bindings) = *bindings_ref {
                if let Some((value, generation)) = bindings.get(name) {
                    ...
                    if *generation >= self.cache_generation.get() {
                        ...
                        self.lookup_cache
                            .borrow_mut()
                            .put(name.to_string(), (value.clone(), *generation));
                        self.cache_stats.borrow_mut().cache_hits += 1;
                        self.update_avg_time(start_time);
                        return Some(value.clone());
                    }
    ...
    fn lookup_with_rebuild(&self, name: &str) -> Option<Value> {
        ...
        while let Some(env) = current_env {
            ...
            if let Ok(bindings) = env.bindings.try_borrow() {
        ...
        *self.flattened_bindings.borrow_mut() = Some(flattened.clone());
        ...
        if let Some(ref value) = result {
            let value_clone: Value = value.clone();
            self.lookup_cache
                .borrow_mut()
                .put(name.to_string(), (value_clone, current_generation));
        }
    ...
    pub fn set(&self, name: &str, value: Value) -> bool {
        ...
        if result {
            ...
            self.lookup_cache
                .borrow_mut()
                .put(name.to_string(), (value, generation));
        }
        result
    }
    ...
    fn update_avg_time(&self, start_time: std::time::Instant) {
        ...
        if total_lookups > 0 {
            stats.avg_lookup_time_ns = (stats.avg_lookup_time_ns * (total_lookups - 1) as f64
                + elapsed_ns)
    ...
    pub fn cache_hit_ratio(&self) -> f64 {
        if let Ok(stats) = self.cache_stats.try_borrow() {
