...
use crate::eval::value_bridge::{BridgeConfig, LegacyValueBridge, OptimizationMetrics};
...
impl ValueOptimizer {
    ...
    pub fn boolean(&self, value: bool) -> Value {
        let start = if self.config.enable_performance_tracking {
            Some(Instant::now())
        } else {
            None
        };
                ...
                if let Some(cached_value) = cache.get(&cache_key) {
                    if let Some(start_time) = start {
                        ...
                        stats.cache_hit_rate =
                            (stats.cache_hit_rate * (total_ops - 1.0) + 1.0) / total_ops;
                    }
    ...
    pub fn pair(&self, car: Value, cdr: Value) -> Value {
        ...
        let result = Value::Pair(Box::new(car), Box::new(cdr));
    ...
    pub fn list(&self, values: Vec<Value>) -> Value {
        ...
        let result = values
            .into_iter()
            .rev()
            .fold(Value::Nil, |acc, val| self.pair_uncounted(val, acc));
    ...
    pub fn vector(&self, values: Vec<Value>) -> Value {
        ...
        let result = Value::Vector(Rc::new(RefCell::new(values)));
    ...
    pub fn estimate_value_size(value: &Value) -> usize {
        match value {
            ...
            Value::Vector(vec_arc) => {
                if let Ok(vec) = vec_arc.try_borrow() {
