...
use crate::eval::value_optimization_core::{
    MemoryAnalysisReport, PerformanceStats, ValueOptimizer,
};
...
impl MemoryMeasurer {
    ...
    pub fn new(enable_allocation_tracking: bool) -> Self {
        Self {
            allocator: if enable_allocation_tracking {
                Some(TrackedAllocator::new())
            } else {
                None
            },
    ...
    pub fn benchmark_optimization<F, G>(
        &self,
        name: &str,
        legacy_fn: F,
        optimized_fn: G,
        ...
        G: Fn() -> Value,
    {
        ...
        let optimized_measurement =
            self.measure_value_creation(&format!("{name}_optimized"), optimized_fn);

        let memory_saved = legacy_measurement
            .memory_used
            .saturating_sub(optimized_measurement.memory_used);
        ...
        let time_improvement = if legacy_measurement.time_taken > optimized_measurement.time_taken {
            let saved = legacy_measurement
                .time_taken
                .saturating_sub(optimized_measurement.time_taken);
            (saved.as_nanos() as f64 / legacy_measurement.time_taken.as_nanos() as f64) * 100.0
        } else {
            0.0
        };

        let arc_reduction = legacy_measurement
            .estimated_arc_count
            .saturating_sub(optimized_measurement.estimated_arc_count);
    ...
    pub fn run_comprehensive_benchmark(&self) -> ComprehensiveBenchmarkReport {
        ...
        report
            .immediate_benchmarks
            .push(self.benchmark_optimization(
                "boolean_true",
        ...
        report.compound_benchmarks.push(self.benchmark_optimization(
            "simple_pair",
            || {
        ...
        report.compound_benchmarks.push(self.benchmark_optimization(
            "string_value",
            || {
                Value::Literal(crate::ast::Literal::String(Box::new(
        ...
        report.complex_benchmarks.push(self.benchmark_optimization(
            "vector_small",
            || {
                let elements = vec![
        ...
        let all_benchmarks: Vec<&BenchmarkResult> = report
            .immediate_benchmarks
            .iter()
        ...
        if !all_benchmarks.is_empty() {
            ...
            report.average_savings_percentage = all_benchmarks
                .iter()
                .map(|b| b.savings_percentage)
                .sum::<f64>()
                / all_benchmarks.len() as f64;
            report.total_arc_reduction = all_benchmarks.iter().map(|b| b.arc_reduction).sum();
            report.average_time_improvement = all_benchmarks
                .iter()
                .map(|b| b.time_improvement)
                .sum::<f64>()
                / all_benchmarks.len() as f64;
        }
    ...
    pub fn get_all_benchmarks(&self) -> Vec<BenchmarkResult> {
        self.benchmarks.try_read().unwrap().clone()
    }
    ...
    pub fn summary(&self) -> String {
        ...
        for benchmark in &self.immediate_benchmarks {
            summary.push_str(&format!(
                ...
                benchmark.name,
                benchmark.memory_saved,
                benchmark.savings_percentage,
    ...
    fn analyze_value_recursive(
        ...
        match value {
            ...
            Value::MutablePair(car, cdr) => {
                ...
                if let (Ok(car_guard), Ok(cdr_guard)) = (car.try_borrow(), cdr.try_borrow()) {
...
mod tests {
    ...
    fn test_value_creation_measurement() {
        ...
        let measurement = measurer
            .measure_value_creation("test_boolean", || Value::Literal(Literal::Boolean(true)));
    ...
    fn test_arc_count_estimation() {
        ...
        let bool_arcs =
            measurer.estimate_arc_count_in_value(&Value::Literal(Literal::Boolean(true)));
        ...
        // Test complex values
        let vector = Value::Vector(Rc::new(RefCell::new(vec![Value::Nil])));
