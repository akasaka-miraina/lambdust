...
impl ArcAllocationTracker {
    ...
    pub fn record_allocation(&self, value_type: &str, memory_size: usize) -> AllocationId {
        ...
        if let Ok(mut stats) = self.detailed_stats.write() {
            ...
            let type_stats = stats.per_type_stats.entry(value_type.to_string()).or_default();
    ...
    pub fn record_deallocation(&self, allocation_id: AllocationId, value_type: &str, memory_size: usize) {
        if !self.is_enabled() {
            return;
        }
        
        self.total_deallocations.fetch_add(1, Ordering::SeqCst);
        let current = self.current_count.fetch_sub(1, Ordering::SeqCst).saturating_sub(1);
        ...
        if let Ok(mut stats) = self.detailed_stats.write() {
            ...
            if let Some(type_stats) = stats.per_type_stats.get_mut(value_type) {
                ...
                type_stats.memory_footprint = type_stats.memory_footprint.saturating_sub(memory_size);
            }
    ...
    fn check_memory_pressure(&self, stats: &mut DetailedAllocationStats, current_count: usize) {
        ...
        if let Some(threshold_time) = one_second_ago {
            let recent_allocations = stats.allocation_timeline.iter()
                .filter(|event| {
    ...
    pub fn get_detailed_report(&self) -> DetailedAllocationReport {
        ...
        let detailed_stats = if let Ok(stats) = self.detailed_stats.read() {
            stats.clone()
        } else {
            DetailedAllocationStats {
                per_type_stats: HashMap::new(),
    ...
    pub fn analyze_patterns(&self) -> AllocationPatternAnalysis {
        ...
        for (type_name, stats) in &report.per_type_stats {
            ...
            analysis.type_efficiency.insert(type_name.clone(), allocation_efficiency);
        ...
        analysis.peak_allocation_rate = report.memory_pressure_events.iter()
            .map(|event| event.allocation_rate)
            .fold(0.0, f64::max);
        
        // Calculate overall efficiency
        analysis.overall_efficiency = if report.basic_stats.total_allocations > 0 {
            report.basic_stats.total_deallocations as f64 / report.basic_stats.total_allocations as f64
        } else {
            1.0
        };
...
mod tests {
    ...
    fn test_tracked_arc() {
        ...
        
        {
            ...
            assert_eq!(current_stats.total_allocations, initial_stats.total_allocations + 1);
    ...
    fn test_value_type_classification() {
        ...
        assert_eq!(nil_class.optimization_potential, OptimizationPotential::High);
        
        let pair_class = ValueTypeAnalyzer::classify_value(&Value::pair(Value::integer(1), Value::integer(2)));
        assert_eq!(pair_class.expected_arc_count, 2);
        assert_eq!(pair_class.optimization_potential, OptimizationPotential::High);
    }
