...
use crate::eval::{
    Environment, Evaluator as StandardEvaluator, Value,
    monadic_architecture::{
        MonadicComputation, MonadicEvaluationInput, MonadicEvaluationOrchestrator,
        MonadicEvaluationResult,
    },
...
impl HybridEvaluator {
    ...
    pub async fn evaluate(
        ...
        match analysis.recommended_strategy {
            ...
            EvaluationStrategy::Adaptive => self.evaluate_adaptively(expr, env).await,
        }
    }
    ...
    async fn evaluate_with_both(
        ...
        match (standard_result, monadic_result) {
            (
                UnifiedEvaluationResult::Standard {
                    value: std_value,
                    metrics: std_metrics,
                },
                UnifiedEvaluationResult::Monadic {
                    computation,
                    result,
                },
            ) => {
                ...
                let performance_comparison = PerformanceComparison {
                    ...
                    performance_ratio: std_metrics.evaluation_time_ns as f64
                        / result.metrics.evaluation_time_ns as f64,
                };
...
impl ExpressionAnalyzer {
        ...
        match expr {
            ...
            Expr::Application { operator, operands } => {
                Self::contains_call_cc(&operator.inner)
                    || operands
                        .iter()
                        .any(|arg| Self::contains_call_cc(&arg.inner))
            }
            Expr::If {
                test: cond,
                ...
                Self::contains_call_cc(&cond.inner)
                    ...
                    || else_branch
                        .as_ref()
                        .is_some_and(|e| Self::contains_call_cc(&e.inner))
            }
            Expr::Lambda { body, .. } => body.iter().any(|e| Self::contains_call_cc(&e.inner)),
            _ => false,
        }
    }
        ...
        match expr {
            Expr::Application { operator, .. } => {
                if let Expr::Identifier(name) = &operator.inner {
                    matches!(
                        name.as_str(),
                        "display" | "write" | "read" | "open-input-file" | "open-output-file"
                    )
                } else {
                    false
                }
    ...
    fn calculate_depth(expr: &Expr) -> usize {
        match expr {
            Expr::Application { operator, operands } => {
                ...
                let max_operand_depth = operands
                    .iter()
        ...
        match expr {
            Expr::Application { operator, operands } => {
                1 + Self::count_nodes(&operator.inner)
                    + operands
                        .iter()
                        .map(|arg| Self::count_nodes(&arg.inner))
                        .sum::<usize>()
            }
            ...
            Expr::If {
                ...
                1 + Self::count_nodes(&cond.inner)
                    ...
                    + else_branch
                        .as_ref()
                        .map_or(0, |e| Self::count_nodes(&e.inner))
            }

            Expr::Lambda { body, .. } => {
                1 + body
                    .iter()
                    .map(|e| Self::count_nodes(&e.inner))
                    .sum::<usize>()
            }
    ...
    fn extract_feature_flags(&self, expr: &Expr) -> u32 {
        ...
        if Self::contains_call_cc(expr) {
            flags |= 1;
        }
        if self.contains_io_operations(expr) {
            flags |= 2;
        }
        if self.contains_state_operations(expr) {
            flags |= 4;
        }
...
impl EvaluatorMetrics {
    ...
    pub fn update(&mut self, evaluation_time_ns: u64, success: bool) {
        ...
        self.success_rate =
            (self.evaluation_count - self.error_count) as f64 / self.evaluation_count as f64;
    }
}
