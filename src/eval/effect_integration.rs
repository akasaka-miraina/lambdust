...
use crate::effects::{
    Effect, EffectContext, EffectHandler, EffectResult, EffectfulComputation, Either, FileHandle,
    FileMode, IO, IOContext, Maybe, Reader, State,
};
use crate::eval::{
    Environment, Value,
    monadic_architecture::{EffectInterpreter, InterpreterConfiguration, MonadicComputation},
};
...
impl UnifiedEffectRegistry {
    ...
    pub fn find_handler(
        &self,
        effect: &Effect,
    ) -> Option<Arc<dyn UnifiedEffectHandler + Send + Sync>> {
...
impl UnifiedEffectHandler for MonadicIOHandler {
                    ...
                    match format!("symbol_{id}", id = op_symbol.id()).as_str() {
                        "write" | "display" => {
                            if let Some(value) = args.get(1) {
                                ...
                                let io_comp: IO<Value> = IO::<()>::write(value_cloned.clone())
                                    .map(move |_| value_cloned.clone());
                        ...
                        "open-input-file" => {
                            if let Some(Value::Literal(crate::ast::Literal::String(filename))) =
                                args.get(1)
                            {
                                // For now, create a mock IO operation since open_file doesn't exist
                                let io_comp: IO<Value> =
                                    IO::<String>::pure(format!("opened: {filename}"))
    ...
    fn metadata(&self) -> EffectHandlerMetadata {
        EffectHandlerMetadata {
            ...
            capabilities: vec![EffectCapability::AsyncSupport, EffectCapability::Streaming],
        }
    }
}
                    ...
                    match format!("symbol_{id}", id = op_symbol.id()).as_str() {
                        "throw" | "error" => {
                            if let Some(error_value) = args.get(1) {
                                ...
                                let error =
                                    Error::runtime_error("Unspecified error".to_string(), None);
                                Ok(MonadicComputation::Either(Either::left(error)))
                            }
                        }
                        ...
                        "try" => {
                            ...
                            if let Some(computation_value) = args.get(1) {
                                ...
                                Ok(MonadicComputation::Either(Either::right(
                                    computation_value.clone(),
                                )))
                            } else {
                                Ok(MonadicComputation::Either(Either::right(
                                    Value::Unspecified,
    ...
    async fn handle_effect(
        ...
        if let Some(Value::Symbol(op_symbol)) = args.first() {
            match format!("symbol_{id}", id = op_symbol.id()).as_str() {
                ...
                "nothing" | "none" => Ok(MonadicComputation::Maybe(Maybe::nothing())),
...
mod tests {
    ...
    async fn test_state_handler() {
        ...
        let args = vec![Value::symbol_from_str("get".to_string())];
        let context = EffectContext::new();
