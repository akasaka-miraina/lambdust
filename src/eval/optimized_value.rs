...
/// A tag that identifies the type of value stored inline.
/// This allows us to store small values directly without allocation.
...
pub enum ValueTag {
    ...
    Nil = 0,
    Boolean = 1,
    Fixnum = 2,      // Small integers (-2^30 to 2^30-1)
    Character = 3,   // Unicode characters  
    Unspecified = 4,
    
    // Allocated values (stored as pointers)
    String = 5,
    Number = 6,      // Large numbers, floats
    Symbol = 7,
    Keyword = 8,
    Pair = 9,
    Vector = 10,
...
/// An optimized Scheme value with tagged union representation.
/// Small values are stored inline to avoid allocation overhead.
#[derive(Clone)]
...
/// The actual data storage for values.
/// Uses a union to minimize memory usage.
#[derive(Clone, Copy)]
pub union ValueData {
...
impl OptimizedValue {
    ...
    /// Creates a small integer value (stored inline)
    #[inline]
    pub fn fixnum(n: i32) -> Self {
        Self {
            tag: ValueTag::Fixnum,
...
mod tests {
    ...
    fn test_immediate_values() {
        ...
        let num = OptimizedValue::fixnum(42);
    ...
    fn test_list_operations() {
        let list = OptimizedValue::list(vec![
            OptimizedValue::fixnum(1),
            OptimizedValue::fixnum(2), 
            OptimizedValue::fixnum(3),
        ]);
    ...
    fn test_environment_operations() {
        ...
        env.define("x".to_string(), OptimizedValue::fixnum(42));
        ...
        assert!(env.set("x", OptimizedValue::fixnum(24)));
        assert_eq!(env.lookup("x").unwrap().as_integer(), Some(24));
        
        // Test setting non-existent variable
        assert!(!env.set("z", OptimizedValue::fixnum(99)));
    }
}