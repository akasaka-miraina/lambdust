...
mod tests {
    ...
    fn test_symbol_provider_creation() {
        ...
        let provider = SymbolProvider::new(interpreter);
        assert!(provider.is_ok());
    }

    #[test]
    fn test_define_form_parsing() {
        let interpreter = LambdustInterpreter::new();
        let provider = SymbolProvider::new(interpreter).unwrap();
        
        let symbol = provider.parse_define_form("(define x 42)", 0, "test.scm").unwrap();
    ...
    #[test]
    fn test_function_define_parsing() {
        let interpreter = LambdustInterpreter::new();
        let provider = SymbolProvider::new(interpreter).unwrap();
        
        let symbol = provider.parse_define_form("(define (square x) (* x x))", 0, "test.scm").unwrap();
        assert!(symbol.is_some());
    ...
    fn test_fuzzy_matching() {
        ...
        assert!(provider.fuzzy_match("string-append", "strapp"));
        assert!(provider.fuzzy_match("list-ref", "lref"));
        assert!(!provider.fuzzy_match("cons", "xyz"));
    }

    #[test]
    fn test_symbol_search() {
        let interpreter = LambdustInterpreter::new();
        let mut provider = SymbolProvider::new(interpreter).unwrap();
        ...
        provider.workspace_symbols.insert("test-function".to_string(), vec![symbol]);
        
        let results = provider.search_workspace_symbols("test").unwrap();
        assert!(!results.is_empty());
        assert_eq!(results[0].name, "test-function");
    }
}