...
mod tests {
    ...
    fn test_message_router_creation() {
        ...
        let server = Arc::new(RwLock::new(
            LambdustLanguageServer::new(config).unwrap()
        ));
        
        let router = MessageRouter::new(server);
        assert!(!router.context.server.try_read().is_err());
    }
    ...
    fn test_handler_context() {
        ...
        let context = HandlerContext { server };
        assert!(!context.server.try_read().is_err());
    }

    #[test]
    fn test_utils_position_conversion() {
        let lsp_pos = lsp_types::Position::new(10, 20);
        let internal_pos = utils::convert_position(&lsp_pos);
        let back_to_lsp = utils::convert_position_to_lsp(internal_pos);
        
        assert_eq!(lsp_pos, back_to_lsp);
    }

    #[test]
    fn test_utils_range_conversion() {
        let lsp_range = lsp_types::Range::new(
            lsp_types::Position::new(0, 0),
            lsp_types::Position::new(1, 10),
        );
        let internal_range = utils::convert_range(&lsp_range);
        let back_to_lsp = utils::convert_range_to_lsp(internal_range);
        
        assert_eq!(lsp_range, back_to_lsp);
    }
}