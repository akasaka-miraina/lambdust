...
use lambdust::diagnostics::{
    UnifiedError, UnifiedResult, ErrorCategory, ErrorSeverity,
    error_convert, propagate_error, validate_arity, jit_error
};
use lambdust::jit::unified_jit_errors::{JitErrorKind, JitUnifiedError, PerformanceImpact, ResourceOverhead};
...
fn demonstrate_error_conversion() {
    ...
    fn old_parse_number(s: &str) -> lambdust::diagnostics::Result<f64> {
        s.parse::<f64>()
            .map_err(|e| lambdust::diagnostics::Error::runtime_error(
...
fn demonstrate_jit_errors() {
    ...
    println!("  Performance Impact: {:.1}x degradation", 
        jit_error.performance_impact.as_ref().unwrap().degradation_factor);
...
fn demonstrate_performance() {
    ...
    for i in 0..ITERATIONS {
        let _error = lambdust::diagnostics::Error::runtime_error(
            format!("Error {}", i),
            None
        );
    }
