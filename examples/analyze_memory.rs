use std::mem;
use lambdust::evaluator::{Evaluator, Continuation};
use lambdust::value::{Value, Procedure};
...
fn main() {
    ...
    println!("  Box<Continuation>: {} bytes", mem::size_of::<Box<Continuation>>());
    println!("  Rc<Environment>: {} bytes", mem::size_of::<std::rc::Rc<Environment>>());
    ...
    println!("  Boolean:         {} bytes (stack)", mem::size_of_val(&boolean_val));
    println!("  Number:          {} bytes (stack)", mem::size_of_val(&number_val));
    println!("  String:          {} bytes (stack)", mem::size_of_val(&string_val));
    println!("  Nil:             {} bytes (stack)", mem::size_of_val(&nil_val));
    
    // Continuationのvariantサイズ分析
    println!("\nContinuation variant analysis:");
    let identity_cont = Continuation::Identity;
    println!("  Identity:        {} bytes", mem::size_of_val(&identity_cont));
    
    // Box allocations
    println!("\nHeap allocation sizes:");
    println!("  Box<Continuation>: {} bytes per allocation", mem::size_of::<Continuation>());
    ...
    println!("  Box overhead per level: ~{} bytes", mem::size_of::<Box<Continuation>>());
    println!("  Environment sharing: Rc<Environment> = {} bytes", mem::size_of::<std::rc::Rc<Environment>>());
    
    // パフォーマンス影響の推定
    println!("\nPerformance impact estimation:");
    println!("  Stack frame vs Box allocation: ~{}x slower", 
             "10-100"); // Box allocation is typically much slower than stack allocation
