...
fn main() {
    ...
    let benchmarks = vec![
        ...
        ("Simple recursion (factorial 5)", r#"
(begin
  (define (factorial n)
    (if (<= n 1)
        1
  ...
  (deep-call 10))"#),
    ...
    for (name, code) in benchmarks {
        ...
        if success_count > 0 {
            ...
            println!("{:>8.2} μs/op ({} iterations)", per_iteration, success_count);
        }
    }
