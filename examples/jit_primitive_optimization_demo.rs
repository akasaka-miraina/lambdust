...
fn demonstrate_code_size_reduction() {
    ...
    println!("  ✓ Generated {} optimized implementations", registry.primitive_names().len());
}
...
fn benchmark_arithmetic_performance(registry: &GenericPrimitiveRegistry) {
    ...
    println!("  Addition (+): {} ns/op ({} ops/sec)", 
        ns_per_op, 
    ...
    println!("  Multiplication (*): {} ns/op ({} ops/sec)", 
        ns_per_op,
        1_000_000_000 / ns_per_op.max(1)
    );
...
fn benchmark_type_predicate_performance(registry: &GenericPrimitiveRegistry) {
    ...
    println!("  number? (true): {} ns/op ({} ops/sec)", 
        ns_per_op,
        1_000_000_000 / ns_per_op.max(1)
    );
    ...
    println!("  string? (false): {} ns/op ({} ops/sec)", 
        ns_per_op,
        1_000_000_000 / ns_per_op.max(1)
    );
}
    ...
    for (i, spec) in type_specs.iter().enumerate() {
        ...
        if !spec.simd_opportunities.is_empty() {
            for simd in &spec.simd_opportunities {
                println!("    SIMD: {} {}-bit vectors ({:.1}x speedup)", 
                    simd.instruction_set,
                    simd.vector_width,
                    simd.speedup_factor
                );
            }
...
mod tests {
    ...
    fn test_all_primitives_work() {
        ...
        let result = equal.evaluate(&[Value::string("a".to_string()), Value::string("a".to_string())]).unwrap();
        assert!(result.as_boolean().unwrap());
    }
    ...
    fn test_performance_is_reasonable() {
        ...
        assert!(ns_per_op < 50, "Type predicate too slow: {} ns/op", ns_per_op);
    }
    ...
    fn test_memory_efficiency() {
        ...
        assert!(registry_size < 42 * 1000, "Registry too large: {} bytes", registry_size);
    }
}