...
fn demonstrate_type_system_registry() {
    ...
    if let Some(dep_caps) = registry.capabilities("dependent-types") {
        println!("  Dependent: dependent_types={}, catt_support={}, categorical={}", 
    ...
    let catt_systems = registry.find_systems_with_capabilities(
        TypeSystemCapabilities::catt_compatible()
    );
...
fn demonstrate_hindley_milner_system() {
    match HindleyMilnerSystem::new() {
        Ok(hm_system) => {
            ...
            let poly_func = HMType::Function(
                Box::new(type_var_a.clone()),
                Box::new(type_var_b.clone())
            );
            let forall_type = HMType::Forall(
                vec![
                    HMTypeVariable { id: 0, name: Some("a".to_string()) },
                    HMTypeVariable { id: 1, name: Some("b".to_string()) }
                ],
                Box::new(poly_func)
            );
...
fn demonstrate_monad_aware_system() {
    match MonadAwareSystem::new() {
        Ok(monad_system) => {
            ...
            let maybe_number = MonadAwareType::monadic(
            ...
            println!("    Transformer stack: {}", transformer_stack.display_type());
...
fn demonstrate_dependent_type_system() {
    match DependentTypeSystem::new() {
        Ok(dep_system) => {
            println!("Dependent type system created: {}", dep_system.system_name());
            ...
            println!("    Has monad structure: {}", cat_morphism.has_monad_structure());
...
fn demonstrate_generic_inference() {
    ...
    println!("    Unification algorithm: {:?}", config.unification_algorithm);
    println!("    Generalization strategy: {:?}", config.generalization_strategy);
    println!("    Caching enabled: {}", config.enable_caching);
    println!("    Max unification depth: {}", config.max_unification_depth);
    ...
    for (name, system_result) in systems {
        match system_result {
            Ok(system) => {
                ...
                println!("    {}: CaTT support = {}, categorical = {}, monadic = {}", 
