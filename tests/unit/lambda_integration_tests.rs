...
fn test_nested_lambda_with_map() {
    let result = eval_str_formal("(map (lambda (x) (+ x 1)) (map (lambda (x) (* x 2)) '(1 2 3)))").unwrap();
    assert_eq!(result, Value::from_vector(vec![
        Value::from(3i64),
        Value::from(5i64),
        Value::from(7i64)
...
fn test_fold_right_with_lambda() {
    let result = eval_str_formal("(fold-right (lambda (x acc) (cons x acc)) '() '(1 2 3))").unwrap();
    assert_eq!(result, Value::from_vector(vec![
        Value::from(1i64),
        Value::from(2i64),
        Value::from(3i64)
...
fn test_fold_vs_fold_right() {
    ...
    let right_result = eval_str_formal("(fold-right (lambda (x acc) (- x acc)) 0 '(1 2 3))").unwrap();
