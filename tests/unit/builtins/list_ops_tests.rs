...
mod basic_list_operations_tests {
    ...
    fn test_car() {
        ...
        assert_eq!(call_builtin("car", vec![single_list]).unwrap(), string("hello"));
        
        // Car of nested list
        let nested = list(vec![list(vec![int(1), int(2)]), int(3)]);
        assert_eq!(call_builtin("car", vec![nested]).unwrap(), list(vec![int(1), int(2)]));
    ...
    fn test_cdr() {
        ...
        // Cdr of list
        let test_list = list(vec![int(10), int(20), int(30)]);
        assert_eq!(call_builtin("cdr", vec![test_list]).unwrap(), list(vec![int(20), int(30)]));
        ...
        assert_eq!(call_builtin("cdr", vec![two_list]).unwrap(), list(vec![int(2)]));
        
        // Cdr of nested list
        let nested = list(vec![int(1), list(vec![int(2), int(3)])]);
        assert_eq!(call_builtin("cdr", vec![nested]).unwrap(), list(vec![list(vec![int(2), int(3)])]));
    ...
    fn test_cons() {
        ...
        assert_eq!(call_builtin("cons", vec![int(1), int(2)]).unwrap(), pair(int(1), int(2)));
        
        // Cons with nil to create single-element list
        assert_eq!(call_builtin("cons", vec![int(42), nil()]).unwrap(), list(vec![int(42)]));
        
        // Cons to extend a list
        let existing_list = list(vec![int(2), int(3)]);
        assert_eq!(call_builtin("cons", vec![int(1), existing_list]).unwrap(), list(vec![int(1), int(2), int(3)]));
        
        // Cons with different types
        assert_eq!(call_builtin("cons", vec![string("hello"), int(42)]).unwrap(), pair(string("hello"), int(42)));
        
        // Cons with nested structures
        let inner_list = list(vec![int(10), int(20)]);
        assert_eq!(call_builtin("cons", vec![inner_list.clone(), int(30)]).unwrap(), pair(inner_list, int(30)));
    ...
    fn test_list() {
        ...
        // Single element list
        assert_eq!(call_builtin("list", vec![int(42)]).unwrap(), list(vec![int(42)]));
        
        // Multi-element list
        assert_eq!(call_builtin("list", vec![int(1), int(2), int(3)]).unwrap(), list(vec![int(1), int(2), int(3)]));
        
        // Mixed types
        assert_eq!(call_builtin("list", vec![int(1), string("hello"), symbol("world")]).unwrap(), 
                   list(vec![int(1), string("hello"), symbol("world")]));
        
        // Nested lists
        let inner = list(vec![int(1), int(2)]);
        assert_eq!(call_builtin("list", vec![inner.clone(), int(3)]).unwrap(), 
    ...
    fn test_length() {
        ...
        // Single element
        assert_eq!(call_builtin("length", vec![list(vec![int(42)])]).unwrap(), int(1));
        
        // Multi-element list
        assert_eq!(call_builtin("length", vec![list(vec![int(1), int(2), int(3)])]).unwrap(), int(3));
        ...
        let nested = list(vec![list(vec![int(1), int(2)]), list(vec![int(3), int(4), int(5)])]);
    ...
    fn test_append() {
        ...
        assert_eq!(call_builtin("append", vec![single.clone()]).unwrap(), single);
        
        // Two lists
        let list1 = list(vec![int(1), int(2)]);
        let list2 = list(vec![int(3), int(4)]);
        assert_eq!(call_builtin("append", vec![list1, list2]).unwrap(), list(vec![int(1), int(2), int(3), int(4)]));
        
        // Multiple lists
        let list1 = list(vec![int(1)]);
        let list2 = list(vec![int(2), int(3)]);
        let list3 = list(vec![int(4), int(5), int(6)]);
        assert_eq!(call_builtin("append", vec![list1, list2, list3]).unwrap(), 
                   list(vec![int(1), int(2), int(3), int(4), int(5), int(6)]));
        
        // Append with empty lists
        let empty = nil();
        let non_empty = list(vec![int(1), int(2)]);
        assert_eq!(call_builtin("append", vec![empty.clone(), non_empty.clone()]).unwrap(), non_empty);
        assert_eq!(call_builtin("append", vec![non_empty.clone(), empty]).unwrap(), non_empty);
        
        // Append with nil as last argument
        let list1 = list(vec![int(1), int(2)]);
        assert_eq!(call_builtin("append", vec![list1.clone(), nil()]).unwrap(), list1);
        ...
        assert_eq!(call_builtin("car", vec![call_builtin("cdr", vec![result.clone()]).unwrap()]).unwrap(), int(2));
        assert_eq!(call_builtin("cdr", vec![call_builtin("cdr", vec![result]).unwrap()]).unwrap(), int(42));
        
        // Mixed types
        let list1 = list(vec![string("hello")]);
        let list2 = list(vec![symbol("world")]);
        assert_eq!(call_builtin("append", vec![list1, list2]).unwrap(), 
        ...
        assert!(call_builtin("append", vec![list(vec![int(1)]), int(42), list(vec![int(3)])]).is_err());
    }
    ...
    fn test_reverse() {
        ...
        assert_eq!(call_builtin("reverse", vec![single.clone()]).unwrap(), single);
        
        // Two elements
        let two = list(vec![int(1), int(2)]);
        assert_eq!(call_builtin("reverse", vec![two]).unwrap(), list(vec![int(2), int(1)]));
        
        // Multiple elements
        let multi = list(vec![int(1), int(2), int(3), int(4), int(5)]);
        assert_eq!(call_builtin("reverse", vec![multi]).unwrap(), 
                   list(vec![int(5), int(4), int(3), int(2), int(1)]));
        
        // Mixed types
        let mixed = list(vec![string("a"), int(2), symbol("c")]);
        assert_eq!(call_builtin("reverse", vec![mixed]).unwrap(), 
                   list(vec![symbol("c"), int(2), string("a")]));
        
        // Nested lists
        let nested = list(vec![list(vec![int(1), int(2)]), list(vec![int(3), int(4)])]);
        assert_eq!(call_builtin("reverse", vec![nested]).unwrap(), 
...
mod destructive_operations_tests {
    ...
    fn test_set_cdr() {
        ...
        assert_eq!(call_builtin("cdr", vec![test_pair]).unwrap(), list(vec![int(2), int(3)]));
...
mod predicate_tests {
    ...
    fn test_null_predicate() {
        // True cases
        assert_eq!(call_builtin("null?", vec![nil()]).unwrap(), Value::Boolean(true));
        
        // False cases
        assert_eq!(call_builtin("null?", vec![list(vec![int(1)])]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("null?", vec![pair(int(1), int(2))]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("null?", vec![int(42)]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("null?", vec![string("hello")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("null?", vec![symbol("test")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("null?", vec![Value::Boolean(true)]).unwrap(), Value::Boolean(false));
    ...
    fn test_pair_predicate() {
        // True cases
        assert_eq!(call_builtin("pair?", vec![pair(int(1), int(2))]).unwrap(), Value::Boolean(true));
        assert_eq!(call_builtin("pair?", vec![list(vec![int(1)])]).unwrap(), Value::Boolean(true)); // Single-element list
        assert_eq!(call_builtin("pair?", vec![list(vec![int(1), int(2)])]).unwrap(), Value::Boolean(true)); // Multi-element list
        
        // Dotted pair
        let dotted = pair(int(1), int(2)); // (1 . 2)
        assert_eq!(call_builtin("pair?", vec![dotted]).unwrap(), Value::Boolean(true));
        
        // False cases
        assert_eq!(call_builtin("pair?", vec![nil()]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("pair?", vec![int(42)]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("pair?", vec![string("hello")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("pair?", vec![symbol("test")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("pair?", vec![Value::Boolean(true)]).unwrap(), Value::Boolean(false));
    ...
    fn test_list_predicate() {
        // True cases
        assert_eq!(call_builtin("list?", vec![nil()]).unwrap(), Value::Boolean(true)); // Empty list
        assert_eq!(call_builtin("list?", vec![list(vec![int(1)])]).unwrap(), Value::Boolean(true)); // Single-element
        assert_eq!(call_builtin("list?", vec![list(vec![int(1), int(2), int(3)])]).unwrap(), Value::Boolean(true)); // Multi-element
        
        // Nested lists
        let nested = list(vec![list(vec![int(1)]), int(2)]);
        assert_eq!(call_builtin("list?", vec![nested]).unwrap(), Value::Boolean(true));
        
        // False cases (improper lists/dotted pairs)
        assert_eq!(call_builtin("list?", vec![pair(int(1), int(2))]).unwrap(), Value::Boolean(false)); // Dotted pair
        
        // Non-list types
        assert_eq!(call_builtin("list?", vec![int(42)]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("list?", vec![string("hello")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("list?", vec![symbol("test")]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("list?", vec![Value::Boolean(true)]).unwrap(), Value::Boolean(false));
...
mod edge_case_tests {
    ...
    fn test_nested_structures() {
        ...
        assert_eq!(rest, list(vec![list(vec![int(4), list(vec![int(5), int(6)])])]));
        
        // Test length of nested structure
        let nested = list(vec![list(vec![int(1), int(2), int(3)]), list(vec![int(4), int(5)])]);
    ...
    fn test_circular_reference_safety() {
        ...
        assert_eq!(call_builtin("car", vec![test_pair.clone()]).unwrap(), int(1));
    ...
    fn test_type_consistency() {
        ...
        let mixed_list = list(vec![int(1), string("hello"), symbol("world"), Value::Boolean(true)]);
        
        // All operations should work with mixed types
        assert_eq!(call_builtin("length", vec![mixed_list.clone()]).unwrap(), int(4));
        assert_eq!(call_builtin("car", vec![mixed_list.clone()]).unwrap(), int(1));
        
        let reversed = call_builtin("reverse", vec![mixed_list.clone()]).unwrap();
        assert_eq!(call_builtin("car", vec![reversed]).unwrap(), Value::Boolean(true));
    ...
    fn test_large_lists() {
        ...
        assert_eq!(call_builtin("length", vec![large_list.clone()]).unwrap(), int(1000));
        assert_eq!(call_builtin("car", vec![large_list.clone()]).unwrap(), int(0));
    ...
    fn test_empty_list_operations() {
        ...
        // Predicates
        assert_eq!(call_builtin("null?", vec![empty.clone()]).unwrap(), Value::Boolean(true));
        assert_eq!(call_builtin("pair?", vec![empty.clone()]).unwrap(), Value::Boolean(false));
        assert_eq!(call_builtin("list?", vec![empty.clone()]).unwrap(), Value::Boolean(true));
        ...
        assert_eq!(call_builtin("append", vec![empty.clone(), empty.clone()]).unwrap(), empty);
    ...
    fn test_improper_lists() {
        ...
        assert_eq!(call_builtin("pair?", vec![dotted.clone()]).unwrap(), Value::Boolean(true));
        assert_eq!(call_builtin("list?", vec![dotted.clone()]).unwrap(), Value::Boolean(false));
        ...
        assert_eq!(call_builtin("pair?", vec![semi_proper.clone()]).unwrap(), Value::Boolean(true));
        assert_eq!(call_builtin("list?", vec![semi_proper.clone()]).unwrap(), Value::Boolean(false));
