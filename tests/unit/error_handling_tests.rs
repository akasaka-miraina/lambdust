...
fn expect_error(interpreter: &mut Interpreter, code: &str) -> LambdustError {
    match interpreter.eval(code) {
        Ok(value) => panic!("Expected error but got result: {:?} for code: {}", value, code),
        Err(err) => err,
    }
}
...
fn expect_success(interpreter: &mut Interpreter, code: &str) -> Value {
    match interpreter.eval(code) {
        Ok(value) => value,
        Err(err) => panic!("Expected success but got error: {:?} for code: {}", err, code),
    }
}
...
mod boundary_value_tests {
    ...
    fn test_list_boundary_values() {
        ...
        expect_success(&mut interpreter, "
            (define (make-small-list n acc)
              (if (= n 0)
                  acc
                  (make-small-list (- n 1) (cons 'x acc))))
        ");
        expect_success(&mut interpreter, "(define big-list (make-small-list 10 '()))");
    ...
    fn test_vector_boundary_values() {
        ...
        expect_success(&mut interpreter, "(define test-vec (vector 'a 'b 'c 'd 'e))");
        ...
        expect_success(&mut interpreter, "(define large-vec (vector 'a 'b 'c 'd 'e 'f 'g 'h 'i 'j))");
...
mod edge_case_error_recovery_tests {
    ...
    fn test_nested_error_contexts() {
        ...
        expect_success(&mut interpreter, "(define (safe-car x) (if (pair? x) (car x) 'error))");
    ...
    fn test_complex_data_structure_errors() {
        ...
        expect_success(&mut interpreter, "(define mixed (list 1 \"hello\" 'symbol (vector 'a 'b 'c) '(x y z)))");
    ...
    fn test_evaluation_order_edge_cases() {
        ...
        expect_success(&mut interpreter, "(define (inc!) (set! counter (+ counter 1)) counter)");
...
mod resource_management_tests {
    ...
    fn test_large_computation_stability() {
        ...
        for i in 1..=10 {
            expect_success(&mut interpreter, &format!(r#"(string-append "prefix" "{}" "suffix")"#, i));
        }
        
        // Test list operations for stability
        for i in 1..=10 {
            expect_success(&mut interpreter, &format!("(list {} {} {})", i, i+1, i+2));
        }
    }
    ...
    fn test_garbage_collection_safety() {
        ...
        for i in 1..=20 {
            ...
            expect_success(&mut interpreter, &format!("(define temp{} (list 'a 'b 'c 'd 'e))", i));
            expect_success(&mut interpreter, &format!(r#"(define str{} "temporary string {}")"#, i, i));
        }
        ...
        match vector_test {
            Ok(_) => {
                ...
                for i in 1..=10 {
                    expect_success(&mut interpreter, &format!("(define vec{} (vector 'temp{} 'temp{}))", i, i, i+1));
                }
            }
            Err(_) => {
                // Vectors not implemented, create more lists
                for i in 21..=30 {
                    expect_success(&mut interpreter, &format!("(define list{} (list {} {} {}))", i, i, i+1, i+2));
                }
            }
        }
