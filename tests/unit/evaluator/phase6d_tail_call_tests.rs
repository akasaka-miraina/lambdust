...
use lambdust::evaluator::{
    Evaluator, OptimizationLevel, TailCallAnalyzer, TailCallContext, TailCallOptimizer,
};
...
fn test_tail_call_analysis_simple_function_call() {
    ...
    assert!(result
        .optimizations
        .iter()
        .any(|opt| matches!(opt, lambdust::evaluator::OptimizationHint::TailCall)));
...
fn test_tail_call_optimization_cache() {
    ...
    let result1 = optimizer
        .optimize_tail_call(&expr, &context, &mut evaluator)
        .unwrap();
        ...
        Value::Procedure(lambdust::value::Procedure::Builtin {
            ...
            func: |args| {
                if args.len() == 1 {
                    ...
                    Err(lambdust::error::LambdustError::runtime_error(
                        "test requires 1 argument".to_string(),
                    ))
                }
            },
            arity: Some(1),
...
fn test_cache_clearing() {
    ...
    // Create cache entry
    let _optimization1 = optimizer
        .optimize_tail_call(&expr, &context, &mut evaluator)
        .unwrap();
    assert_eq!(optimizer.get_stats().cache_misses, 1);

    // Use cache
    let _optimization2 = optimizer
        .optimize_tail_call(&expr, &context, &mut evaluator)
        .unwrap();
