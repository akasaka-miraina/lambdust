...
mod tests {
    ...
    fn test_semantic_equivalence_failure() {
        ...
        let goal = ProofGoal {
            statement: Statement::SemanticEquivalence(
                Expr::Literal(Literal::Number(SchemeNumber::Integer(42))),
                Expr::Literal(Literal::Number(SchemeNumber::Integer(43))),
            ),
            goal_type: GoalType::Equivalence,
            expressions: vec![],
    ...
    fn test_proof_methods() {
        ...
        let computation = ProofMethod::Computation;
        let rewrite = ProofMethod::Rewrite("theorem_name".to_string());
        let induction = ProofMethod::Induction("variable".to_string());
        let case_analysis = ProofMethod::CaseAnalysis;
