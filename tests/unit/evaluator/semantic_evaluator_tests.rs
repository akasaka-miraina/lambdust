...
fn test_variable_lookup_defined() {
    ...
    // Define a variable
    env.define("test_var".to_string(), Value::Number(SchemeNumber::Integer(99)));
...
fn test_variable_lookup_from_closure() {
    ...
    outer_env.define("outer_var".to_string(), Value::Number(SchemeNumber::Integer(77)));
...
fn test_builtin_addition() {
    ...
    env.define("+".to_string(), Value::Procedure(
        lambdust::value::Procedure::Builtin {
            name: "+".to_string(),
...
fn test_error_handling_invalid_lambda_arity() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("requires at least"));
}
...
fn test_error_handling_invalid_define_variable() {
    ...
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("first argument must be variable"));
}
...
fn test_literal_value_conversion() {
    ...
    assert!(matches!(number_result.unwrap(), Value::Number(SchemeNumber::Integer(123))));
    ...
    let nil_result = evaluator.eval_pure(
