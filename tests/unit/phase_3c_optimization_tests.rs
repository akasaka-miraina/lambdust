...
use lambdust::adaptive_memory::{AdaptiveMemoryManager, MemoryConfig, MemoryPressure, AllocationStrategy};
use lambdust::cps_inlining::{CpsInliner, InliningDecision, ChainStrategy};
use lambdust::evaluator::Continuation;
use lambdust::environment::Environment;
use lambdust::lexer::SchemeNumber;
use lambdust::memory_pool::{PoolStats, ContinuationPoolStats};
use lambdust::stack_monitor::{StackMonitor, StackFrameType, OptimizationRecommendation};
...
fn test_cps_inliner_operation_inlining() {
    ...
    let result = inliner.try_inline_operation(&assignment, Value::Number(SchemeNumber::Integer(123)));
    assert!(matches!(result, Some(Value::Number(_))));
...
fn test_cps_inliner_pattern_generation() {
    ...
    let values = Continuation::Values {
        values: vec![Value::Number(SchemeNumber::Integer(1)), Value::Number(SchemeNumber::Integer(2)), Value::Number(SchemeNumber::Integer(3))],
        parent: Box::new(Continuation::Identity),
    };
...
fn test_phase_3c_performance_regression_prevention() {
    ...
    let result = cps_inliner.try_inline_operation(&identity, Value::Number(SchemeNumber::Integer(42)));
    assert!(matches!(result, Some(Value::Number(_))));
}