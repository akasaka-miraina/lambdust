...
fn test_character_set_operations_r7rs_compliance() {
    ...
    assert_eq!(result, Value::integer(3), "char-set should create set with 3 elements");
    
    // Test list->char-set
    let result = eval_scheme(&mut runtime, 
        "(char-set-size (list->char-set '(#\\a #\\b #\\c #\\a)))");
    assert_eq!(result, Value::integer(3), "list->char-set should deduplicate characters");
    
    // Test string->char-set
    let result = eval_scheme(&mut runtime, 
        "(char-set-size (string->char-set \"abca\"))");
    assert_eq!(result, Value::integer(3), "string->char-set should deduplicate characters");
    
    // Test char-set-contains?
    let result = eval_scheme(&mut runtime, 
        "(char-set-contains? (char-set #\\a #\\b #\\c) #\\b)");
    assert_eq!(result, Value::boolean(true), "char-set-contains? should find contained character");
    
    let result = eval_scheme(&mut runtime, 
        "(char-set-contains? (char-set #\\a #\\b #\\c) #\\d)");
    assert_eq!(result, Value::boolean(false), "char-set-contains? should not find absent character");
}
...
fn test_character_set_error_conditions() {
    ...
    assert!(result.is_err(), "char-set? with too many arguments should error");
    
    // Test char-set-contains? with wrong types
    let result = runtime.evaluate_string("(char-set-contains? 'not-a-charset #\\a)");
    assert!(result.is_err(), "char-set-contains? with non-charset should error");
    
    let result = runtime.evaluate_string("(char-set-contains? char-set:digit 'not-a-char)");
    assert!(result.is_err(), "char-set-contains? with non-character should error");
}
...
fn test_character_set_conversion_roundtrip() {
    ...
    let result = eval_scheme(&mut runtime, &format!(
        "(char-set-size (string->char-set \"{}\"))", original
    ));
    assert_eq!(result, Value::integer(original.len() as i64),
        "Round-trip conversion should preserve character count");
    
    // Test that conversions preserve character membership
    let result = eval_scheme(&mut runtime, &format!(
        "(char-set-contains? (string->char-set \"{}\") #\\c)", original
    ));
...
fn test_hex_digit_character_set() {
    ...
    for ch in hex_chars.chars() {
        ...
        assert!(result, "char-set:hex-digit should contain hex digit '{}'", ch);
    }
    ...
    for ch in non_hex_chars.chars() {
        let result = charset_contains_scheme(&mut runtime, "char-set:hex-digit", ch);
        assert!(!result, "char-set:hex-digit should not contain non-hex character '{}'", ch);
    }
}
...
fn test_blank_vs_whitespace_distinction() {
    ...
    assert!(!charset_contains_scheme(&mut runtime, "char-set:blank", '\n'));
    assert!(!charset_contains_scheme(&mut runtime, "char-set:blank", '\r'));
    
    // Whitespace should contain all blank characters plus others
    assert!(charset_contains_scheme(&mut runtime, "char-set:whitespace", ' '));
    assert!(charset_contains_scheme(&mut runtime, "char-set:whitespace", '\t'));
    assert!(charset_contains_scheme(&mut runtime, "char-set:whitespace", '\n'));
    assert!(charset_contains_scheme(&mut runtime, "char-set:whitespace", '\r'));
    
    // Test subset relationship
    let result = eval_scheme(&mut runtime, "(char-set<= char-set:blank char-set:whitespace)");
    assert_eq!(result, Value::boolean(true), "char-set:blank should be subset of char-set:whitespace");
}
...
fn test_graphic_vs_printing_character_sets() {
    ...
    for ch in '!'..'~' {  // ASCII printable except space
        assert!(charset_contains_scheme(&mut runtime, "char-set:graphic", ch),
            "char-set:graphic should contain graphic character '{}'", ch);
        assert!(charset_contains_scheme(&mut runtime, "char-set:printing", ch),
            "char-set:printing should contain printing character '{}'", ch);
    }
...
fn test_character_set_size_consistency() {
    ...
    assert_eq!(result, Value::integer(10), "char-set:digit should have exactly 10 characters");
    
    // Hex digits: exactly 22 (0-9, A-F, a-f)
    let result = eval_scheme(&mut runtime, "(char-set-size char-set:hex-digit)");
    assert_eq!(result, Value::integer(22), "char-set:hex-digit should have exactly 22 characters");
    
    // Blank: exactly 2 (space and tab)
    let result = eval_scheme(&mut runtime, "(char-set-size char-set:blank)");
    assert_eq!(result, Value::integer(2), "char-set:blank should have exactly 2 characters");
    
    // Empty: exactly 0
    let result = eval_scheme(&mut runtime, "(char-set-size char-set:empty)");
    assert_eq!(result, Value::integer(0), "char-set:empty should have exactly 0 characters");
    
    // ASCII: exactly 128
    let result = eval_scheme(&mut runtime, "(char-set-size char-set:ascii)");
    assert_eq!(result, Value::integer(128), "char-set:ascii should have exactly 128 characters");
}
...
fn test_ucs_range_character_sets() {
    ...
    let result = eval_scheme(&mut runtime, 
        "(char-set-size (ucs-range->char-set 48 58))");  // '0' is 48, '9'+1 is 58
    assert_eq!(result, Value::integer(10), "UCS range for digits should create set of size 10");
    
    // Test that the range matches the digit character set
    let result = eval_scheme(&mut runtime,
        "(char-set= char-set:digit (ucs-range->char-set 48 58))");
    assert_eq!(result, Value::boolean(true), "UCS range 48-58 should equal digit character set");
    
    // Test empty range
    let result = eval_scheme(&mut runtime,
        "(char-set-size (ucs-range->char-set 58 48))");  // Invalid range
    assert_eq!(result, Value::integer(0), "Invalid UCS range should create empty set");
}
...
"#;
    ...
    let result = eval_scheme(&mut runtime, 
        "(char-set-contains? my-vowels #\\a)");
    assert_eq!(result, Value::boolean(true), "Custom character set should work correctly");
}
