...
mod condition_system_tests {
    ...
    fn test_condition_type_registration() {
        ...
        assert!(
            condition_type.is_some(),
            "Standard &condition type should be registered"
        );

        let message_type = system.registry.get_type_by_name("&message");
        assert!(
            message_type.is_some(),
            "Standard &message type should be registered"
        );

        let error_type = system.registry.get_type_by_name("&error");
        assert!(
            error_type.is_some(),
            "Standard &error type should be registered"
        );
    }
    ...
    fn test_condition_type_hierarchy() {
        ...
        assert!(system.registry.is_subtype(
            system.standard_types.message,
            system.standard_types.condition
        ));
        assert!(
            system
                .registry
                .is_subtype(system.standard_types.error, system.standard_types.serious)
        );
        assert!(system.registry.is_subtype(
            system.standard_types.serious,
            system.standard_types.condition
        ));
        assert!(system.registry.is_subtype(
            system.standard_types.violation,
            system.standard_types.condition
        ));
        ...
        assert!(
            !system
                .registry
                .is_subtype(system.standard_types.violation, system.standard_types.error)
        );
    }
    ...
    fn test_condition_extraction() {
        ...
        let error_conditions =
            compound.extract_conditions(system.standard_types.error, &system.registry);
        assert_eq!(error_conditions.len(), 1);
        assert_eq!(
            error_conditions[0].condition_type,
            system.standard_types.error
        );

        // Extract serious conditions (error is a subtype of serious)
        let serious_conditions =
            compound.extract_conditions(system.standard_types.serious, &system.registry);
        assert_eq!(serious_conditions.len(), 1);
        assert_eq!(
            serious_conditions[0].condition_type,
            system.standard_types.error
        );

        // Extract message conditions
        let message_conditions =
            compound.extract_conditions(system.standard_types.message, &system.registry);
        assert_eq!(message_conditions.len(), 1);
        assert_eq!(
            message_conditions[0].condition_type,
            system.standard_types.message
        );

        // Extract non-existent type
        let violation_conditions =
            compound.extract_conditions(system.standard_types.violation, &system.registry);
...
mod condition_procedures_tests {
    ...
    fn test_compound_condition_creation() {
        ...
        let message_condition =
            primitive_make_message_condition(&[Value::string("Error".to_string())]).unwrap();
    ...
    fn test_condition_has_type() {
        ...
        let has_error_type =
            primitive_condition_has_type_p(&[error_condition.clone(), error_type]).unwrap();
        ...
        let has_serious_type =
            primitive_condition_has_type_p(&[error_condition.clone(), serious_type]).unwrap();
        ...
        let has_message_type =
            primitive_condition_has_type_p(&[error_condition, message_type]).unwrap();
    ...
    fn test_condition_ref() {
        ...
        let condition =
            primitive_make_message_condition(&[Value::string(message.to_string())]).unwrap();

        // Access message field
        let message_symbol = system.registry.interner.intern("message");
        let field_value =
            primitive_condition_ref(&[condition, Value::symbol(message_symbol)]).unwrap();
    ...
    fn test_make_condition_with_fields() {
        ...
        let args = vec![Value::string("&message".to_string()), field_bindings];
...
mod integration_tests {
    ...
    fn test_exception_condition_integration() {
        ...
        if let Value::Condition(condition) = condition_value {
            let exception = lambdust::stdlib::exceptions::ExceptionObject::from_condition(
                condition.clone(),
                false,
            );
    ...
    fn test_comprehensive_condition_workflow() {
        ...
        let message_condition =
            primitive_make_message_condition(&[Value::string("File not found".to_string())])
                .unwrap();
        let error_condition = primitive_make_error(&[]).unwrap();
        ...
        assert_eq!(
            primitive_condition_p(&[compound.clone()]).unwrap(),
            Value::boolean(true)
        );
        assert_eq!(
            primitive_error_p(&[compound.clone()]).unwrap(),
            Value::boolean(true)
        );
        assert_eq!(
            primitive_message_condition_p(&[compound.clone()]).unwrap(),
            Value::boolean(true)
        );
        ...
        let error_type = Value::ConditionType(system.standard_types.error);
        let has_error_type =
            primitive_condition_has_type_p(&[compound.clone(), error_type]).unwrap();
        assert_eq!(has_error_type, Value::boolean(true));
        ...
        let message_symbol = system.registry.interner.intern("message");
        let message_value =
            primitive_condition_ref(&[compound, Value::symbol(message_symbol)]).unwrap();
        match message_value {
    ...
    fn test_standard_condition_hierarchy_completeness() {
        ...
        let types = [
            ...
            (
                ...
                system.standard_types.implementation_restriction,
            ),
        ];
        ...
        for (name, id) in types {
            ...
            assert!(
                registered_type.is_some(),
                "Type {} should be registered",
                name
            );
            assert_eq!(
                registered_type.unwrap().id,
                id,
                "Type {} should have correct ID",
                name
            );
        }
        ...
        assert!(system.registry.is_subtype(
            system.standard_types.assertion,
            system.standard_types.violation
        ));
        assert!(system.registry.is_subtype(
            system.standard_types.violation,
...
mod error_handling_integration_tests {
    ...
    fn test_raise_with_condition() {
        ...
        // Create a condition
        let condition =
            primitive_make_message_condition(&[Value::string("Test error".to_string())]).unwrap();
