...
fn test_string_generator() {
    ...
    if let Value::Generator(gen_ref) = &generator {
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('a'))
        );
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('b'))
        );
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('c'))
        );
        assert!(gen_ref.is_exhausted());
    }
    ...
    if let Value::Generator(gen_ref) = &unicode_generator {
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('α'))
        );
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('β'))
        );
        assert_eq!(
            gen_ref.next().unwrap(),
            Value::Literal(lambdust::ast::Literal::Character('γ'))
        );
        assert!(gen_ref.is_exhausted());
    }
}
