...
fn test_value_to_type_conversion_literals() {
    ...
    let bool_type = integration
        .value_to_type(&bool_value)
    ...
    let string_type = integration
        .value_to_type(&string_value)
...
fn test_type_to_value_conversion() {
    ...
    let universe_value = integration
        .type_to_value(&universe_type)
...
fn test_symbol_and_null_processing() {
    ...
    let symbol_type = integration
        .value_to_type(&symbol_value)
    ...
    let null_type = integration
        .value_to_type(&null_value)
...
fn test_type_compatibility_checking() {
    ...
    let int_type1 = integration
        .value_to_type(&int_value1)
        .expect("Failed to convert first integer");
    let is_compatible = integration
        .is_value_compatible(&int_value2, &int_type1)
    ...
    let is_incompatible = integration
        .is_value_compatible(&bool_value, &int_type1)
...
fn test_cache_functionality_and_performance() {
    ...
    let _type1 = integration
        .value_to_type(&test_value)
...
fn test_expression_type_inference() {
    ...
    let inferred_type = integration
        .infer_expression_type(&literal_expr, &env)
    ...
    let inferred_type = integration
        .infer_expression_type(&id_expr, &env)
...
fn test_performance_comprehensive() {
    ...
    let test_values = vec![
        ...
        Value::Literal(Literal::String(Box::new("performance".to_string()))),
    ...
    assert!(
        cache_size <= test_values.len(),
        "Cache should not exceed number of unique values"
    );
...
fn test_martin_lof_type_theory_correctness() {
    ...
    let type1 = integration
        .value_to_type(&int_value1)
        .expect("Failed conversion 1");
    let type2 = integration
        .value_to_type(&int_value2)
        .expect("Failed conversion 2");
    ...
    match (&type1, &type2) {
        (
            ...
            assert_eq!(
                n1, n2,
                "Same Scheme types should have same dependent type name"
            );
...
fn test_comprehensive_scheme_integration() {
    ...
    for original_value in original_values {
        ...
        let dep_type = integration
            .value_to_type(&original_value)
            .expect("Failed to convert to dependent type");
        let _converted_value = integration
            .type_to_value(&dep_type)
    ...
    assert!(
        final_cache_size > 0,
        "Cache should contain entries after operations"
    );
