...
use lambdust::stdlib::charset::{CharSet, StandardCharSets, CharSetConfig, CHARSET_CONFIG};
...
fn test_optimization_correctness() {
    ...
    for test_str in test_cases.iter() {
        ...
        assert_eq!(optimized.size(), legacy.size(), "Size mismatch for: {}", test_str);
        assert_eq!(optimized.is_empty(), legacy.is_empty(), "Empty check mismatch for: {}", test_str);
        ...
        for c in test_str.chars() {
            assert_eq!(
                ...
                legacy.contains(c),
                "Membership mismatch for '{}' in: {}", c, test_str
            );
        }
        ...
        assert_eq!(opt_chars, leg_chars, "Character set mismatch for: {}", test_str);
    }
...
fn test_ascii_optimization() {
    ...
    assert!(digits_display.contains("type=ascii"), "Digits should use ASCII optimization: {}", digits_display);
...
fn test_configuration_system() {
    ...
    assert!(display_str.contains("type=legacy"), "Should use legacy representation: {}", display_str);
    ...
    assert!(!opt_display.contains("type=legacy"), "Should use optimized representation: {}", opt_display);
...
fn test_performance_regression() {
    ...
    for test_str in test_strings.iter() {
        ...
        CHARSET_CONFIG.with(|config| {
            *config.borrow_mut() = CharSetConfig { enable_optimizations: false, force_representation: None };
        });
        let legacy = CharSet::from_string(test_str);
        
        CHARSET_CONFIG.with(|config| {
            *config.borrow_mut() = CharSetConfig { enable_optimizations: true, force_representation: None };
        });
        let optimized = CharSet::from_string(test_str);
