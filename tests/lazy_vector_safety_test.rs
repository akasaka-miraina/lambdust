...
fn test_lazy_vector_runtime_error_prevention() {
    ...
    let huge_storage =
        VectorStorage::new(1_000_000_000, Value::Number(SchemeNumber::Integer(1))).unwrap();
...
fn test_lazy_vector_gradual_materialization() {
    ...
    let mut storage =
        VectorStorage::new(1_000_000, Value::Number(SchemeNumber::Integer(42))).unwrap();
    ...
    for i in (0..1_000_000).step_by(100_000) {
        storage
            .set(i, Value::Number(SchemeNumber::Integer(99)))
            .unwrap();
    }
...
fn test_lazy_vector_memory_efficiency() {
    ...
    storage
        .set(10, Value::Number(SchemeNumber::Integer(1)))
        .unwrap();
    storage
        .set(20, Value::Number(SchemeNumber::Integer(2)))
        .unwrap();
    storage
        .set(30, Value::Number(SchemeNumber::Integer(3)))
        .unwrap();
