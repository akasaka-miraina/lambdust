...
use lambdust::stdlib::control::{primitive_values, evaluator_call_with_values};
use lambdust::stdlib::srfi71_let_syntax::{uncons, unlist, values_to_list, list_to_values};
use std::sync::Arc;
...
fn test_values_procedure_no_errors() {
    ...
    // Multiple arguments
    let args = &[Value::integer(1), Value::string("hello"), Value::boolean(true)];
...
fn test_unlist_error_conditions() {
    ...
    let list = Value::list(&[Value::integer(1), Value::integer(2)]);
    let result = unlist(&[list, Value::Literal(lambdust::ast::Literal::ExactInteger(5))]);
    assert!(result.is_err());
}
