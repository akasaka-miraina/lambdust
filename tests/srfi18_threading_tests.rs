...
use lambdust::eval::parameter::{capture_parameter_bindings, inherit_parameter_bindings, ParameterFrame};
...
fn test_mutex_creation_and_predicates() {
    ...
    let name = Value::Literal(lambdust::ast::literal::Literal::String(Box::new("test-mutex".to_string())));
...
fn test_parameter_inheritance_basic() {
    ...
    bindings.push((1u64, Value::Literal(lambdust::ast::literal::Literal::String(Box::new("test-value".to_string())))));
...
fn test_concurrent_mutex_operations() {
    ...
    let handles: Vec<_> = (0..4).map(|i| {
        ...
        thread::spawn(move || {
            ...
            for _ in 0..10 {
                // Acquire mutex
                if mutex_clone.lock(thread_id, Some(Duration::from_millis(100))).is_ok() {
                    // Critical section
                    {
    ...
    assert!(final_count <= 40, "Final count should be at most 40, got {}", final_count);
    assert!(final_count > 0, "Final count should be greater than 0, got {}", final_count);
}
...
fn test_performance_benchmarks() {
    ...
    let start = Instant::now();
    for i in 0..1000 {
        let _thread = SchemeThread::new(
            Some(format!("thread-{}", i)),
            Arc::new(Vec::new()),
        );
    }
    let thread_creation_time = start.elapsed();
    println!("Thread creation: {:?} per thread", thread_creation_time / 1000);
    ...
    println!("Mutex lock/unlock: {:?} per operation", mutex_ops_time / NUM_OPERATIONS as u32);
    ...
    println!("Condition variable notify: {:?} per operation", notify_time / NUM_OPERATIONS as u32);
}
...
fn test_thread_statistics() {
    ...
    assert_eq!(thread.stats.procedure_calls.load(std::sync::atomic::Ordering::SeqCst), 0);
    assert_eq!(thread.stats.parameter_accesses.load(std::sync::atomic::Ordering::SeqCst), 0);
    assert_eq!(thread.stats.mutex_operations.load(std::sync::atomic::Ordering::SeqCst), 0);
}
...
fn test_mutex_statistics() {
    ...
    // Initially, all statistics should be zero
    assert_eq!(mutex.stats.lock_count.load(std::sync::atomic::Ordering::SeqCst), 0);
    assert_eq!(mutex.stats.unlock_count.load(std::sync::atomic::Ordering::SeqCst), 0);
    
    // Perform operations and check statistics
    mutex.lock(thread_id, None).unwrap();
    assert_eq!(mutex.stats.lock_count.load(std::sync::atomic::Ordering::SeqCst), 1);
    
    mutex.unlock(thread_id).unwrap();
    assert_eq!(mutex.stats.unlock_count.load(std::sync::atomic::Ordering::SeqCst), 1);
}
...
fn test_condition_variable_statistics() {
    ...
    assert_eq!(condvar.stats.notify_count.load(std::sync::atomic::Ordering::SeqCst), 0);
    
    // Perform notifications and check statistics
    condvar.notify_one();
    assert_eq!(condvar.stats.notify_count.load(std::sync::atomic::Ordering::SeqCst), 1);
    
    condvar.notify_all();
    assert_eq!(condvar.stats.notify_count.load(std::sync::atomic::Ordering::SeqCst), 2);
}
