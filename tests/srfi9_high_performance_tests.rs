...
use lambdust::eval::record_arena::{RecordArena, SizeClass, GLOBAL_RECORD_ARENA};
use lambdust::eval::record_instance::{RecordInstance, RecordRef, value_to_nan_boxed, nan_boxed_to_value};
use lambdust::eval::record_type::{
    RecordTypeDescriptor, RecordTypeId, GLOBAL_RECORD_REGISTRY,
};
use lambdust::eval::value::{ThreadSafeEnvironment, Value};
use lambdust::stdlib::srfi9_optimization::{
    RecordOperation, GLOBAL_RECORD_PERFORMANCE_MONITOR,
    get_performance_report, apply_optimizations, optimize_memory,
};
use lambdust::stdlib::srfi9_records::{install_srfi9_procedures, GLOBAL_SRFI9_REGISTRY};
use std::sync::Arc;
...
fn test_field_mutation() {
    ...
    unsafe {
        ...
        instance_ref.set_field(0, NanBoxedValue::small_integer(50)).unwrap();
        assert_eq!(instance_ref.get_field_fast(0), NanBoxedValue::small_integer(50));
        
        // Test field mutation by name
        instance_ref.set_field_by_name("y", NanBoxedValue::small_integer(150)).unwrap();
        assert_eq!(instance_ref.get_field_fast(1), NanBoxedValue::small_integer(150));
    }
}
...
fn test_field_access_caching() {
    ...
    let start = Instant::now();
    let value1 = cache.get_field_cached(
    ...
    let start = Instant::now();
    let value2 = cache.get_field_cached(
        unsafe { instance.as_ref() },
        "field1",
        site_id,
    ).unwrap();
...
fn test_type_checking_performance() {
    ...
    for i in 0..1000 {
        if i % 2 == 0 {
            GLOBAL_TYPE_CHECKER.is_type_cached(
                unsafe { instance1.as_ref() },
                type1,
                call_site_id2,
            );
...
fn test_performance_monitoring() {
    ...
    GLOBAL_RECORD_PERFORMANCE_MONITOR.record_type_performance(
        type_id,
        RecordOperation::BulkOperation { count: 8 },
    );
    ...
    let type_perf = report.type_performance.iter()
...
fn test_optimization_system() {
    ...
    for _ in 0..500 {
        GLOBAL_RECORD_PERFORMANCE_MONITOR.record_type_performance(
            type_id,
            RecordOperation::FieldAccess { access_time_ns: 8 },
        );
    }
    ...
    for _ in 0..100 {
        GLOBAL_RECORD_PERFORMANCE_MONITOR.record_type_performance(
            type_id,
            RecordOperation::BulkOperation { count: 4 },
        );
    }
    ...
    assert!(optimization_results.applied + optimization_results.failed + optimization_results.skipped > 0);
}
    ...
    for original_value in test_values {
        ...
        match &original_value {
            ...
            _ => {
                ...
                println!("Complex type conversion: {:?} -> {:?}", original_value, converted_back);
            }
        }
    }
}
...
fn test_stdlib_integration() {
    ...
    let type_def = registry.define_record_type(
