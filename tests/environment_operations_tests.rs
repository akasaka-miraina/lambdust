...
fn test_eval_basic_expression() {
    ...
    crate::lambdust::stdlib::arithmetic::create_arithmetic_bindings(&env);
    ...
    let expr = Value::pair(
        plus_symbol, 
        Value::pair(one, Value::pair(two, Value::Nil))
    );
...
fn test_eval_error_handling() {
    ...
    assert!(result.is_err(), "Should error with non-environment argument");
}
...
fn test_environment_bound_error_handling() {
    ...
    let result = primitive_environment_bound_p(&[
        Value::integer(1), 
        Value::integer(2), 
        Value::integer(3)
    ]);
    ...
    assert!(result.is_err(), "Should error with non-symbol first argument");
}
...
fn test_r7rs_compliance_integration() {
    ...
    assert!(env.lookup("environment").is_some(), "environment should be available");
    assert!(env.lookup("environment-bound?").is_some(), "environment-bound? should be available");
    assert!(env.lookup("scheme-report-environment").is_some(), "scheme-report-environment should be available");
    assert!(env.lookup("null-environment").is_some(), "null-environment should be available");
    assert!(env.lookup("interaction-environment").is_some(), "interaction-environment should be available");
}
...
fn test_environment_security() {
    ...
    assert!(result.is_ok(), "Safe expression should evaluate successfully");
}
...
mod property_based_tests {
    ...
    #[quickcheck]
    fn test_environment_bound_consistency(var_name: String) -> TestResult {
    ...
    fn test_eval_literal_identity(n: i64) -> bool {
        ...
        match (&literal, &result) {
            (Value::Literal(lit1), Value::Literal(lit2)) => {
                lit1.to_i64() == lit2.to_i64()
            }
            _ => false,
        }
    }
...
mod benchmark_tests {
    ...
    fn benchmark_environment_creation() {
        ...
        println!("Environment creation: {} operations in {:?} ({:.2} ops/sec)", 
    ...
    fn benchmark_environment_bound_checks() {
        ...
        for _ in 0..iterations {
            let _result = primitive_environment_bound_p(&[symbol_value.clone(), env_value.clone()]).unwrap();
        }
        
        let duration = start.elapsed();
        println!("Environment bound checks: {} operations in {:?} ({:.2} ops/sec)", 
                iterations, duration, iterations as f64 / duration.as_secs_f64());
    }
}