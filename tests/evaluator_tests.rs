...
mod formal_evaluator_tests {
    ...
    fn test_special_forms() {
        ...
        // Test begin
        let result = interpreter.eval("(begin (define x 1) (define y 2) (+ x y))").unwrap();
    ...
    fn test_do_loops() {
        ...
        let result = interpreter.eval("(do ((i 0 (+ i 1))) ((>= i 5) i))").unwrap();
        assert_eq!(result, Value::from(5i64));
        
        // Loop with accumulator
        let result = interpreter.eval("(do ((i 0 (+ i 1)) (sum 0 (+ sum i))) ((>= i 5) sum))").unwrap();
    ...
    fn test_multi_value_system() {
        ...
        let result = interpreter.eval("(call-with-values (lambda () (values 1 2)) +)").unwrap();
        assert_eq!(result, Value::from(3i64));
    }
    ...
    fn test_evaluation_order_independence() {
        ...
        interpreter.eval("(define (inc!) (set! counter (+ counter 1)) counter)").unwrap();
