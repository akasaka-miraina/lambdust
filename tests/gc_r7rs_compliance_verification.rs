...
use lambdust::ast::{Literal, RationalLiteral};
use lambdust::eval::value::Value;
use lambdust::runtime::LambdustRuntime;
use lambdust::runtime::gc::{GcConfigBuilder, GcSystem};
...
fn test_42_core_primitives_with_gc() {
    ...
    assert!(
        allocated_obj.is_ok(),
        "Basic allocation with GC should work"
    );
...
fn test_equality_predicates_with_gc_object_movement() {
    ...
    let str1 = Arc::new(Value::Literal(Literal::String(Box::new(
        "hello".to_string(),
    ))));
    let str2 = Arc::new(Value::Literal(Literal::String(Box::new(
        "hello".to_string(),
    ))));
    ...
    assert_eq!(
        ...
        "equal? semantics changed after GC"
    );
...
fn test_tail_recursion_with_gc() {
    ...
    let fibonacci_tail = |n: i64| -> i64 {
        fn fib_helper(n: i64, a: i64, b: i64, gc_system: &Arc<GcSystem>) -> i64 {
            if n == 0 {
                ...
                for _ in 0..10 {
                    let _ = gc_system.allocate(Value::Literal(Literal::InexactReal(n as f64)), 64);
                }
    ...
    assert!(
        stats.minor_collections > 0 || stats.major_collections > 0,
...
fn test_exact_arithmetic_preservation() {
    ...
    let exact_values = [
        ...
        Value::Literal(Literal::Rational(Box::new(RationalLiteral::new(1, 3)))),
        Value::Literal(Literal::Rational(Box::new(RationalLiteral::new(-22, 7)))),
    ];

    let allocated_objects: Vec<_> = exact_values
        .iter()
    ...
    for (original, gc_obj) in exact_values.iter().zip(allocated_objects.iter()) {
        match (original, gc_obj.value.as_ref()) {
            ...
            (Value::Literal(Literal::Rational(rat1)), Value::Literal(Literal::Rational(rat2))) => {
                assert_eq!(
                    (rat1.numerator, rat1.denominator),
                    (rat2.numerator, rat2.denominator),
                    "Rational number corrupted by GC"
                );
            }
...
fn test_string_mutability_with_gc() {
    ...
    // Modify the string
    {
        let mut chars = mutable_string.borrow_mut();
        chars[0] = 'H';
    }
...
fn test_large_object_handling() {
    ...
    let large_vector = Value::Vector(Rc::new(RefCell::new(large_vector_data)));
...
fn test_closure_environment_capture_with_gc() {
    ...
    for i in 0..50 {
        ...
        let env_objects: Vec<_> = captured_vars
            .iter()
            .map(|v| gc_system.allocate(v.clone(), 64).unwrap())
            .collect();
...
fn test_stress_allocation_and_collection() {
    ...
    for batch in 0..100 {
        ...
        for i in 0..100 {
            let value = Value::Pair(
                Box::new(Value::Literal(Literal::InexactReal(batch as f64))),
                Box::new(Value::Literal(Literal::InexactReal(i as f64))),
            );
    ...
    assert!(
        allocation_count > 9000,
        "Should have allocated many objects"
    );
    assert!(
    ...
    assert!(
        stats.is_healthy(),
        "GC should remain healthy under stress: {stats:?}"
    );

    // Verify survivors are still valid
    for obj in &survived_objects {
        assert!(
            matches!(obj.value.as_ref(), Value::Pair(_, _)),
            "Survivor object corrupted"
        );
    }

    println!(
        "Stress test stats: allocations={}, collections={}/{}, health_score={:.2}",
        allocation_count,
...
fn test_deterministic_behavior_preservation() {
    ...
    let results: Vec<_> = (0..3)
        .map(|_run| {
            ...
            for i in 0..100 {
                let value = Value::Pair(
                    Box::new(Value::Literal(Literal::InexactReal(i as f64))),
                    Box::new(Value::Literal(Literal::Boolean(i % 2 == 0))),
                );
    ...
    assert_eq!(
        results[0], results[1],
        "GC introduced non-deterministic behavior"
    );
    assert_eq!(
        results[1], results[2],
        "GC introduced non-deterministic behavior"
    ...
    assert_eq!(
        results[0].len(),
        100,
        "Operations should complete deterministically"
    );
}
...
fn test_performance_regression_detection() {
    ...
    println!(
        "Performance: baseline={baseline_time:?}, gc={gc_time:?}, overhead={overhead_ratio:.2}x"
    );
...
fn test_gc_statistics_accuracy() {
    ...
    assert_eq!(
        final_stats.minor_collections,
        initial_minor + 2,
        "Minor collection count incorrect"
    );
    assert_eq!(
        final_stats.major_collections,
        initial_major + 1,
    ...
    assert!(
        final_stats.allocation_rate >= 0.0,
        "Allocation rate should be non-negative"
    );
    assert!(
        final_stats.young_utilization >= 0.0 && final_stats.young_utilization <= 100.0,
        "Young generation utilization out of range"
    );
    assert!(
        final_stats.old_utilization >= 0.0 && final_stats.old_utilization <= 100.0,
...
mod integration_tests {
    ...
    fn test_full_r7rs_compliance_suite() {
        ...
        for value in test_values {
            ...
            assert!(
                allocated.is_ok(),
                "Should be able to allocate basic values with GC"
            );
        }
