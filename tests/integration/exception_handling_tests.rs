...
mod exception_tests {
    ...
    fn test_raise_basic() {
        ...
        if let Err(e) = result {
            assert!(e.to_string().contains("Exception raised: \"This is an error\""));
        }
        ...
        assert!(result.is_err());
        if let Err(e) = result {
            assert!(e.to_string().contains("Exception raised: error-symbol"));
        }
    }
    ...
    fn test_with_exception_handler_syntax() {
        ...
        let result = interpreter.eval("(with-exception-handler (lambda (obj) 'handled) (lambda () 42))");
        assert!(result.is_ok());
    ...
    fn test_guard_syntax_basic() {
        ...
        let result = interpreter.eval("(guard (e ((string? e) 'string-error) (else 'other-error)) 42)");
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::from(42i64));
    ...
    fn test_exception_handling_arity_errors() {
        ...
        let result = interpreter.eval("(with-exception-handler (lambda (x) x) (lambda () 1) extra)");
        assert!(result.is_err());
    }
...
mod exception_integration_tests {
    ...
    fn test_exception_with_user_defined_functions() {
        ...
        interpreter.eval("(define (safe-divide a b) (if (= b 0) (raise 'division-by-zero) (/ a b)))").unwrap();
