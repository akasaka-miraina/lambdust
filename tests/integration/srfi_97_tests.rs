...
mod srfi_97_integration_tests {
    ...
    fn test_srfi_name() {
        ...
        assert_eq!(result, Value::String("Primitives for Expressing Iterative Lazy Algorithms".to_string()));
        
        // Test SRFI 46
        let result = interpreter.eval("(srfi-name 46)").unwrap();
        assert_eq!(result, Value::String("Basic Syntax-rules Extensions".to_string()));
    ...
    fn test_srfi_parts() {
        ...
        if let Value::Vector(parts) = result {
            ...
            assert!(parts.iter().any(|p| matches!(p, Value::String(s) if s == "records")));
        }
        ...
        if let Value::Vector(parts) = result {
            ...
            assert!(parts.iter().any(|p| matches!(p, Value::String(s) if s == "inquiry")));
        }
