...
mod tests {
    ...
    fn test_srfi_69_library_available() {
        ...
        let functions = vec![
            ...
            "hash-table-walk", "hash-table-fold", "hash-table-copy", "hash-table-merge!",
    ...
    fn test_hash_table_set_and_ref_integration() {
        ...
        interpreter.eval_string("(hash-table-set! ht \"key\" 42)").unwrap();
        
        // Get value
        let result = interpreter.eval_string("(hash-table-ref ht \"key\")").unwrap();
    ...
    fn test_hash_table_exists_integration() {
        ...
        let result = interpreter.eval_string("(hash-table-exists? ht \"test\")").unwrap();
        assert_eq!(result, Value::Boolean(false));

        // Set value
        interpreter.eval_string("(hash-table-set! ht \"test\" 42)").unwrap();
    ...
    fn test_hash_table_delete_integration() {
        ...
        // Delete value
        let result = interpreter.eval_string("(hash-table-delete! ht \"test\")").unwrap();
        assert_eq!(result, Value::Boolean(true));
    ...
    fn test_alist_to_hash_table_integration() {
        ...
        let result = interpreter.eval_string("(alist->hash-table '((\"key\" . 42) (\"other\" . 24)))").unwrap();
        assert!(result.is_hash_table());
    }
    ...
    fn test_hash_table_copy_integration() {
        ...
        interpreter.eval_string("(define ht-copy (hash-table-copy ht))").unwrap();
        let result = interpreter.eval_string("(hash-table-ref ht-copy \"test\")").unwrap();
        assert_eq!(result, Value::Number(SchemeNumber::Integer(42)));
    }
    ...
    fn test_hash_table_ref_with_default_integration() {
        ...
        let result = interpreter.eval_string("(hash-table-ref/default ht \"missing\" 99)").unwrap();
